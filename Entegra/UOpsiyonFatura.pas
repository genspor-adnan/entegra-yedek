...
{  if (ComboDijitBr.Text <> OncekiDigitSay) and (ComboDijitBr.ItemIndex >= 0) and
     (Application.MessageBox(PChar(KGeri_donusu_yok),
      PChar('Uyar�/Onay'), MB_YESNO + MB_ICONQUESTION)= IDYES) then
  begin
...
begin
  ...
  Tablo.GENINI.WriteInteger(Ops_OpsiyonFatura_VarsayilanKlasor,VarsayilanKlasor.Tag); //OpsiyonFatura VarsayilanKlasor Dokuman i�in
  Tablo.GENINI.WriteInteger(Ops_OpsiyonSiparis_VarsayilanKlasor,VarsayilanKlasorSiparis.Tag); //OpsiyonSiparis VarsayilanKlasor Dokuman i�in
...
begin
   if Application.MessageBox(PChar('"'+GelenFaturaDetaySablonList.Items[GelenFaturaDetaySablonList.ItemIndex]+'" ve t�m alt ��eler silinecektir. Onayl�yor musunuz?'),PChar('ONAY'),MB_YESNO) = IDYES then begin
...
begin
   if Application.MessageBox(PChar('"'+GelenSiparisDetaySablonList.Items[GelenSiparisDetaySablonList.ItemIndex]+'" ve t�m alt ��eler silinecektir. Onayl�yor musunuz?'),PChar('ONAY'),MB_YESNO) = IDYES then begin
...
begin
   if Application.MessageBox(PChar('"'+GidenFaturaDetaySablonList.Items[GidenFaturaDetaySablonList.ItemIndex]+'" ve t�m alt ��eler silinecektir. Onayl�yor musunuz?'),PChar('ONAY'),MB_YESNO) = IDYES then begin
...
begin
   if Application.MessageBox(PChar('"'+GidenSiparisDetaySablonList.Items[GidenSiparisDetaySablonList.ItemIndex]+'" ve t�m alt ��eler silinecektir. Onayl�yor musunuz?'),PChar('ONAY'),MB_YESNO) = IDYES then begin
...
begin
  ...
  try
    Dlg.Title := 'XSLT Dosyas� Se�';
    Dlg.Filter := 'XSLT Dosyalar� (*.xslt;*.xsl)|*.xslt;*.xsl|T�m Dosyalar (*.*)|*.*';
    Dlg.DefaultExt := 'xslt';
    ...
    try
      try
        ...
        Icerik.LoadFromFile(Dlg.FileName);   // fallback � default encoding
      end;
