CREATE PROC [dbo].[Sp_Prg_Devir_Cari] (@BasTar Datetime,@AktarilacakYil int,@Kur varchar(5),@IslemKur varchar(10),@DevirleriAl Bit)
AS BEGIN
SET NOCOUNT ON
...
else (SELECT TOP 1 case when @IslemKur='ALIS' THEN ISNULL(D.ALIS,0)
...
FROM DOVIZ D WHERE D.CINSI=KUR ORDER BY ABS(DATEDIFF(HOUR,@YilBasiTarihi,D.TARIH)))end),
-- SELECT * FROM DOVIZ
DOVIZ_KURU=@Kur
...
from(
                Select
                       K.REHBERID,
                       BORC = case when EKSTREDEKULLAN=1 AND BORC>0 then K.DOVIZ_TUTARI else K.BORC end,
                       ALACAK = case when EKSTREDEKULLAN=1 AND ALACAK>0 then K.DOVIZ_TUTARI else K.ALACAK end,
                       KUR = case when EKSTREDEKULLAN=1 then K.DOVIZ_KURU else K.KUR end,
                ...
                Select
                                  CH.REHBERID,
                                  BORC  = Case when CH.ISLEM in(140,131,132,133,134,137) then (case when CH.EKSTREDEKULLAN=1 then CH.TUTAR else isnull(C.TUTAR,0)end) else 0 end,
                                  ALACAK= Case when CH.ISLEM in(130,141) then (case when CH.EKSTREDEKULLAN=1 then CH.TUTAR else isnull(C.TUTAR,0)end) else 0 end,
                ...
                Select
                                  S.REHBERID,
                                  BORC=Case when S.TUR=34 then (case when EKSTREDEKULLAN=1 then S.DOVIZ_TUTARI else isnull(S.TUTAR,0)end) else 0 end,
                                  ALACAK=Case when S.TUR=24 then (case when EKSTREDEKULLAN=1 then S.DOVIZ_TUTARI else isnull(S.TUTAR,0)end) else 0 end,
                                  KUR=case when EKSTREDEKULLAN=1 then S.DOVIZ_KURU else isnull(S.KUR,'TL') end,
                ...
                SELECT
                                  F.REHBERID,
                                  BORC=case when F.TUR in (8,11,12,13) then 0.0 else (case when EKSTREDEKULLAN=1 then DOVIZ_TUTARI else FATURA_TUTARI end) end ,
                                  ALACAK=case when F.TUR in (15,16,17) then 0.0 else (case when EKSTREDEKULLAN=1 then DOVIZ_TUTARI else FATURA_TUTARI end) end,
                                  KUR = case when EKSTREDEKULLAN=1 then DOVIZ_CINSI else KUR end,
...
DOVIZ_TUTARI=(case when SUM(BORC-ALACAK)>0 then SUM(BORC-ALACAK) else SUM(ALACAK-BORC) end)*(case when isnull(KUR,@Kur)=@Kur then 1.0 
                                   ...
                                   when @IslemKur='SATIS' THEN ISNULL(D.SATIS,0) 
                                         when @IslemKur='EFALIS' THEN ISNULL(D.EFALIS,0)
                                         when @IslemKur='EFSATIS' THEN ISNULL(D.EFSATIS,0)
                                         END  FROM DOVIZ D WHERE D.CINSI=KUR ORDER BY ABS(DATEDIFF(HOUR,@YilBasiTarihi,D.TARIH)))end),
DOVIZ_KURU=@Kur
from(
                Select K.REHBERID,
                       BORC = case when EKSTREDEKULLAN=1 AND BORC>0 then K.DOVIZ_TUTARI else K.BORC end,
