  ...
  object SQLDonusCikanHedef_2: TMemo
    ...
    Lines.Strings = (
      'insert into :TabloAdi'
        ...
        ' SELECT  SI1.STOKID,SSL.SERINO,DURUM=abs(SI1.KALAN),KALAN=abs(SI' +
        '1.KALAN),SEC=0,SSL.LOTNO,SSL.SKT, SSL.URT, '
      ' SI1.ID, SI1.BASLIKID,SI1.SATIRID,SI1.DONUSID, SERILOTID=SSL.ID '
      ' from STOKIZLEME SI1 '
      ' INNER JOIN [STOKSERILOT] SSL ON SI1.SERILOTID=SSL.ID '
      ' where '
      ' SI1.SATIRID=@SatirID and abs(isnull(SI1.KALAN,0)) > 0.0001')
    TabOrder = 7
    Visible = False
    WordWrap = False
  end
