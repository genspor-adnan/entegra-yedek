...
begin
  if (Sender as TMenuItem).Tag = 1 then  begin//se�iliyi g�ster
    if StrToIntDef(VarToStrDef(SIPARISDETAY.FieldByName('URETIMPLANDETAYID').Value,'0'),0) < 0 then
      ...
      ShowMessage('Bu sat�r g�sterimde ya da kullan�mda. ��lem ger�ekle�tirilemiyor.');
  end;
  ...
  if (Sender as TMenuItem).Tag = 3 then  begin  //se�iliyi sakla
    if StrToIntDef(VarToStrDef(SIPARISDETAY.FieldByName('URETIMPLANDETAYID').Value,'0'),0) = 0 then
      ...
      ShowMessage('Bu sat�r sakl� ya da kullan�mda. ��lem ger�ekle�tirilemiyor.');
  end;
...
begin
  ...
  while not SIPARISDETAY.Eof do begin
    if SIPARISDETAY.FieldByName('URETIMPLANDETAYID').Value <> plandetID then begin
      ShowMessage('Kullan�lm�� ya da kapat�lm�� sat�rlar mevcut, l�tfen her sat�r i�in ayr� i�lem uygulay�n.');
      Exit(False);
    end;
    SIPARISDETAY.Next;
  end;
...
begin
  ...
  if SIPARIS.FieldByName('PROJEID').AsInteger > 0 then
    if Tablo.UyariGoster('Proje Se�imi','Se�mi� oldu�unuz proje, belgenizin t�m sat�rlar�na uygulans�n m�?',2)=MrYes then
...
begin
  ...
  if SIPARIS.FieldByName('GIRISDEPO').AsInteger = VarsDepo then begin
     ShowMessage('Giri� Deposu "Ana Depo" se�ilemez!');
     Abort;
  end;
...
begin
  ...
  if cbKdvDurum.Text = 'Hari�' then begin
...
begin
  if StrToIntDef(SIPARIS.FieldByName('ONAYLAYAN').AsString,0) <> 0 then begin
    if Tablo.UyariGoster(Uyari,'Yapt���n�z de�i�iklik sipari� onay�n� kald�racakt�r, devam etmek ister misiniz?',2)=mrYes then
  ...
  case IslemOp of
    'E':begin
       ...
       Tablo.TablodanSorguAc(1,'INSERT INTO [SIPARIS] ([TARIH],[TUR],[TIPI],[REHBERID],[SIPARISTARIH],[SIPARISSERI],'+
        ...
        ''',''Hari�'',0,0,0,''TL'',0,''TL'','+IntToStr(TabNo_SATINALMA)+','+inttoStr(SatinAlmaID)+','+inttoStr(REHBERILETID)+','+inttoStr(SubeID)+') select scope_identity() ' );
...
//  cbSatici.Text := Tablo.AciklamaGetir('REHBER','FIRMA', SIPARIS.FieldByName('SATICIKODU').AsString);
  ...
  SIPARIS.FieldByName('KDVDURUM').AsString := 'Hari�';
