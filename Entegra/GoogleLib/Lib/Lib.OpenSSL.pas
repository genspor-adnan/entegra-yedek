...
type
  { Callback events }
  TOpenSSLNotify = procedure of object;
  TOpenSSLData = procedure(const ABuffer: Pointer; const ASize: Integer) of object;
  { OpenSSL protocol handler }
  TOpenSSL = class(TObject)
  protected
    FOnConnected: TOpenSSLNotify;
    FOnRead: TOpenSSLData;
    FOnWrite: TOpenSSLData;
  private
  ...
  public
    ...
    property OnConnected: TOpenSSLNotify read FOnConnected write FOnConnected;
    { Fired when decrypted SSL data is ready to be read }
    property OnRead: TOpenSSLData read FOnRead write FOnRead;
    { Fired when encrypted SSL data is ready to be sent }
    property OnWrite: TOpenSSLData read FOnWrite write FOnWrite;
  end;
  ...
  public
    ...
    class procedure SetCertificate(ctx: PSSL_CTX; const ACertificateFile, APrivateKeyFile: UnicodeString;
      const APassword: UnicodeString = ''); overload;
  public
    class function ExtractPEMPrivateKey_FromPKCS12(const APFX: TBytes; out APEM: TBytes): Boolean;
...
function TOpenSSL.Connect(const AALPN: Boolean): Boolean;
begin
  Result := False;
  { create ssl context }
  ...
  begin
    ...
    begin
      if FPrivateKey = nil then
        TSSLHelper.SetCertificate(FSSLContext, FCertificate, FCertificate, FPassword)
      else
        TSSLHelper.SetCertificate(FSSLContext, FCertificate, FPrivateKey, FPassword);
...
procedure TOpenSSL.Read(const ABuffer: Pointer; const ASize: Integer);
var
  Bytes: Integer;
  Error: Integer;
begin
...
function TOpenSSL.Write(const ABuffer: Pointer; const ASize: Integer): Boolean;
var
  Bytes: Integer;
  Error: Integer;
begin
...
function TOpenSSL.ALPN: Boolean;
var
...
{ TgoSSLHelper }
class procedure TSSLHelper.LoadSSL;
begin
...
class procedure TSSLHelper.UnloadSSL;
begin
...
class procedure TSSLHelper.SetCertificate(ctx: PSSL_CTX; const ACertificate, APrivateKey: TBytes;
...
class procedure TSSLHelper.SetCertificate(ctx: PSSL_CTX; const ACertificateFile, APrivateKeyFile: UnicodeString;
  const APassword: UnicodeString = '');
var
...
class function TSSLHelper.Sign_RSASHA256(const AData: TBytes; const APrivateKey: TBytes;
  out ASignature: TBytes): Boolean;
var
  BIOPrivateKey: PBIO;
  PrivateKey: PEVP_PKEY;
...
class function TSSLHelper.HMAC_SHA256(const AKey, AData: RawByteString): String;
const
  EVP_MAX_MD_SIZE = 64;
var
...
class function TSSLHelper.ExtractPEMPrivateKey_FromPKCS12(
  const APFX: TBytes; out APEM: TBytes): Boolean;
var
  B: PBIO;
  pKey: PEVP_PKEY;
  pCert: PX509;
  ...
  KeyLength: Integer;
begin
  B:= BIO_new_mem_buf(@APFX[0], Length(APFX));
  ...
  try
    ...
    try
      CertChain := nil;
      if PKCS12_parse(P12,  PAnsiChar('notasecret'), pKey, pCert, CertChain) <> 1 then begin
        Exit(False);
      end;
      try
        PrivateKey := BIO_new(BIO_s_mem);
        try
          PEM_write_bio_PKCS8PrivateKey(PrivateKey, pKey, nil, nil, 0, nil, nil);
          KeyLength := BIO_pending(PrivateKey);
          SetLength(APEM, KeyLength);
          BIO_read(PrivateKey, @APEM[0], KeyLength);
          Result := True;
        finally
          BIO_free(PrivateKey);
        end;
...
class function TSSLHelper.HMAC_SHA1(const AKey, AData: RawByteString): TBytes;
const
  EVP_MAX_MD_SIZE = 20;
var
  MessageAuthCode: PByte;
...
initialization
  ...
  SSL_library_init;
  _MemBufferPool := TMemoryPool.Create(DEFAULT_BLOCK_SIZE);
finalization
  TSSLHelper.UnloadSSL;
  _MemBufferPool.Free;
end.