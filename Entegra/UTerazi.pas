...
var
  ...
  i, p, bas:integer;
  Buffer: PChar;
begin       //ST,GS- 64.680,kg      dönen değer
               //     s 64.680
               //ST,GS 15    iki satır halinde geliyor
               //4.020,kg
  ComPort.ReadStr(s, 25);
  //Memo1.Lines.Add(IntToStr(Count)+'s>' + s);
  Okunan := Okunan+s;   //2 satır halinde okuduğu için sonu kg olana kadar topluyoruz
...
begin
  //dara kodlarından liste için sql oluşturulur
  s:='';
  ...
  if Tablo.HizliGirisListedenBilgiGetir('Dara (Boş Ağırlık) Seçimi','select ID, STOKADI,BIRIM2MIKTAR from STOKLAR where '+s+' union all select ID=-1, STOKADI=''Elle Giriş'',BIRIM2MIKTAR=null',SonucListe,False,[False, True, True],[])then begin
     Adet:=AdetGetir('Adet giriniz','1',0, 0, False);
     if SonucListe[0]='-1' then //elle giriş
        s:=FloatToStr(AdetGetir('Dara (Boş Ağırlık) Giriniz', '100', 0, 0, False))
     else
        s:= SonucListe[2];
