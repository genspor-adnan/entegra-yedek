...
begin        // EGITIM KULLANICI YER=2
  ...
  ' WHERE  R.DURUM=1 AND K.DURUM=1 AND R.ID NOT IN (SELECT REHID FROM KALITEKULLANICI KK WHERE KK.REHID=R.ID AND KK.YER=2 AND KK.YERID='+TabEgitim.FieldByName('ID').AsString+')', [nil, nil, nil, Tablo.RepCariGrup, Tablo.RepCariBolum, Tablo.RepCariSinif], ['Id', 'Kullanıcı', 'Rol', 'Grup', 'Kategori', 'Sınıf']);
...
begin
  ...
  if (ComboFaaliyet.ItemIndex =1) and (not BoslukKontrol(ComboMusteri.text, 'Müşteri')) then Abort;
  if not BoslukKontrol(EditEgitimAcan.text, 'Eğitim Açan') then Abort;
  ...
  if not BoslukKontrol(EditEgitimSorumlu.text, 'Eğitim Sorumlusu') then Abort;
  if not BoslukKontrol(MemDofNeden.text, 'Eğitim Açıklama') then Abort;
