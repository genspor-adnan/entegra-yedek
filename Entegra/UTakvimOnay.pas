...
begin
   TabKasa.Close;
   if not(TUR  in [61, 71]) then begin   //plan giren / çıkan
      KaydetTus.Visible := False;
   ...
   if not (TUR in [71,72]) then begin //Ödeme planı değilse müşteri hesap bilgileri görünmesin
...
begin
   case TUR of
    71 :  TakvimOnayDlg.Tag := 21; //gelir planı --> nakit tahsilat
    72 :  TakvimOnayDlg.Tag := 31; //gider planı --> nakit ödeme
...
begin
  ...
  //Müşteri bölümü
   //RehberId den kod ve adı bulup getirelim
  ...
  //Bizim bölümümüz
   //HesapId den kod ve adı bulup getirelim
