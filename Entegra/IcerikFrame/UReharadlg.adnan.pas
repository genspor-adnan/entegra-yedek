...
uses
  ...
  Dialogs, Buttons, Mask, Menus, IniFiles, ComCtrls, UFDCompatHelpers, cxStyles,
...
type
  TRehberAraDlg = class(TFrame, IIcerikBilgiFrame, IBilgiFrame,IPopupDialog )
    ...
    hracat1: TMenuItem;
    hraKaytl1: TMenuItem;
    SerbestMeslekMakbuzuMenu1: TMenuItem;
    Kira1: TMenuItem;
    GiderPusulas1: TMenuItem;
    thalat1: TMenuItem;
    MutabakatEkleMenu: TMenuItem;
    MutabakatKaydi: TMenuItem;
    MutabakatKaydGir1: TMenuItem;
    CariGridViewTEMAS: TcxGridDBColumn;
    CariGridViewEKLEMETARIHI: TcxGridDBColumn;
    CariGridViewIL: TcxGridDBColumn;
    CariGridViewGSM: TcxGridDBColumn;
    CariGridViewULKE: TcxGridDBColumn;
    CariGridViewVERGINO: TcxGridDBColumn;
//    N7: TMenuItem;
    ...
    procedure MutabakatEkleMenuClick(Sender: TObject);
...
uses FetaUtil, UCombo, UAnaForm,UGirisKutusuEx,UKodAgaci,URehberBilgiDuzenle, UGenNotificationUtils,
  ...
  UKasalarListeFrame, UGenelAnaSekmeFrame, URaporAraclari, UGenSifre,UBekletme, UGorevDlg, UMutabakat,
...
procedure TRehberAraDlg.RehEkranInit;
var
  gf : TCariGorevFrame;
begin
...
begin
   ...
   else if TabRehberIlgili.RecordCount=0 then
      Exit;
...
begin
  if TabRehberIlgili.FieldByName('DURUM').AsInteger = 1 then begin
     Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'Update REHBER Set STATU=0 Where BAGID=&RehID',['&RehID'],[REHBER.FieldByName('ID').AsInteger]);
     Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'Update REHBER Set STATU=1 Where BAGID=&RehID and ID=&PersID',['&RehID','&PersID'],[REHBER.FieldByName('ID').AsInteger,TabRehberIlgili.FieldByName('ID').AsInteger]);
...
begin
   BankaDuzenleTus.Visible   := TabBankaHesaplar.RecordCount>0;
...
begin
   ...
   DegisTus.Visible := (REHBER.RecordCount>0) and (Tablo.YetkiVarmi(2201,YetkiTur_Degistirme));
   ...
   PageControlSekme.Visible := REHBER.RecordCount>0;
...
begin
   IlgiliDuzenleTus.Visible := TabRehberIlgili.RecordCount>0;
   IlgiliSilTus.Visible := IlgiliDuzenleTus.Visible;
end;

procedure TRehberAraDlg.TabRehberIlgiliBeforeDelete(DataSet: TDataSet);
begin
   Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, ' delete from KULLANICI where REHBERID='+TabRehberIlgili.FieldByName('ID').AsString+' and PERSONEL=0', [], []);
end;
...
begin
   if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
      ...
      //kendisi silinir
      Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, ' delete from TEKLIFDETAY where TEKLIFID=&id ',['&id'],[TabTeklifler.Fields[0].AsInteger]);
      Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, ' delete from TEKLIF where ID=&id ',['&id'],[TabTeklifler.Fields[0].AsInteger]);
...
begin
  ...
  begin
    ...
    if Tablo.Query4.RecordCount > 0 then TurAdi := Tablo.Query4.Fields[0].AsString
...
begin
   ...
   KapatAc(CariGridViewSEKTOR, FArama.CheckDetay.Checked);
   ...
   KapatAc(CariGridViewGSM, FArama.CheckDetay.Checked);
   KapatAc(CariGridViewIL, FArama.CheckDetay.Checked);
   KapatAc(CariGridViewILCE, FArama.CheckDetay.Checked);
   KapatAc(CariGridViewULKE, FArama.CheckDetay.Checked);
   ...
   KapatAc(CariGridViewEKLEMETARIHI, FArama.CheckDetay.Checked);
   KapatAc(CariGridViewVERGINO, FArama.CheckDetay.Checked);
...
begin
  Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'update REHBER set STATU=0 , DURUM='+IntToStr(TMenuItem(Sender).Tag)+' where ID=&ID',['&ID'],[TabRehberIlgili.FieldByName('ID').AsInteger]);
  Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'update KULLANICI set DURUM=0 where REHBERID=&ID ',['&ID'],[TabRehberIlgili.FieldByName('ID').AsInteger]);
...
begin
  ...
  if TGirisKutusuEx.BilgiAlEx(RDAdSoyad, ctrls) = mrOk then begin
    ...
    Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'insert into REHBERBILGI(YERI,YER_ID,SIRA,ETIKET,BILGI,SUBEID) '+
...
var
  ...
  Uzunluk:integer;
begin
  Paramst :=' R.ID, R.KOD, R.FIRMA, ADSOYAD=P.FIRMA, R.GRUP, R.TEMAS, R.SEKTOR, R.KATEGORI, R.SINIF, R.DURUM, R.OZELKOD';
  if (FArama.ComboCariAnaliz.Visible)and(FArama.ComboCariAnaliz.Itemindex in [1..5]) then
        Paramst :=Paramst + ' ,TOPLAM_BORC,TOPLAM_ALACAK,KUR,BAKIYE=TOPLAM_BORC-TOPLAM_ALACAK'
  else if FArama.CheckDetay.Checked then begin
        Paramst :=Paramst + ',ALTSEKTOR=(select ANAHTAR from GENINI G where G.DIL=-1 and G.DEGER = R.ALTSEKTOR AND G.BOLUM=convert(int,''-2204''+convert(varchar(10),R.SEKTOR))),TEMSILCIAD = R2.FIRMA,ILLER= X.BILGI, '+
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
     ...
     Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, ' delete from BANKAHESAPLAR where ID=&id ',['&id'],[TabBankaHesaplar.Fields[0].AsInteger]);
     PageControlSekmeChange(Self);
  end;
end;
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
     Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, ' delete from REHBERBILGI where YERI='+IntToStr(TabNo_EKIPMANREHBER)+' and YER_ID=&id ',['&id'],[TabEkipmanlar.FieldByName('ID').AsInteger]);
     Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, ' delete from EKIPMANREHBER where ID=&id ',['&id'],[TabEkipmanlar.FieldByName('ID').AsInteger]);
...
var Sonuc:variant;
begin
  Sonuc := Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'select isnull((select top 1 TUTAR from REHBER_KOTA Where REHBERID=&RehID),0.0) ',['&RehID'],[REHBER.FieldByName('ID').AsInteger],True);
  if TGirisKutusuEx.BilgiAlEx('Risk Limiti Bilgisi Giriniz.',TGirdiDenetimleri.Create.CurrencyEdit(CariDoviz+' : ',@Sonuc,2)) = mrOk then begin
     Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'delete from REHBER_KOTA Where REHBERID=&RehID ',['&RehID'],[REHBER.FieldByName('ID').AsInteger]);
     Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'insert into REHBER_KOTA(REHBERID,TUTAR,KUR) values (&RehID,&Tutar,&Kur) ',
...
begin
  if State = dsDragLeave then begin //
     ...
     if not TreeHitTest.HitAtNode then begin
        Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'update EKIPMANREHBER set USTID = 0 where ID='+TabEkipmanlar.Fields[0].AsString,[],[]);
        TabloYenile(TabEkipmanlar,[REHBER.FieldByName('ID').AsInteger]);
     end;
  end
  else begin
  ...
  try
   if Nodes.Count = 1 then begin  //Projeler kapal? ve liste di?er listenin alt?na gelecekse
      ...
      if (DragId>0)and(dropId>0) then begin
         ...
         Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'update EKIPMANREHBER set USTID = '+IntToStr(abs(dropId))+' where ID='+IntToStr(abs(dragId)),[],[]);
         TabloYenile(TabEkipmanlar,[REHBER.FieldByName('ID').AsInteger]);
      end;
    end;
  finally
...
begin
   if GridYorum.ActiveView = GridYorumDBCardView1 then begin
      ...
      if (TabYorum2.RecordCount>0)and((TamYetkili)or(Kullanan = TabYorum2.FieldByName('EKLEYEN').AsString)) then begin
...
begin
  ...
  if (Tur in [111,112,113,114,115,117,118])or(Tur=-111) then begin
     if Tur<0 then Tipi:=-1
     else Tipi:=Tur-110;
     Tur := 11;
  end else
  if ((Tur>=151)and(Tur<=159))or(Tur=1526)or(Tur=-115) then begin
     if Tur<0 then Tipi:=-1
     else if Tur=1526 then Tipi:=26 //ihracat
     else Tipi:=Tur-150;
     Tur := 15;
  end;
  ...
  case Tur of
     ...
     61,71  : begin  DateSaat:=Tablo.GENINI.BugunTrhSaat;
                     ID := Tablo.KasaSihirbazBaslat('E', -1, Tur, 0, REHBER.Fields[0].AsInteger,DateSaat ,DateSaat,0,0,'','');
              end;
  end;
  if ID > 0 then
    PageControlSekmeChange(Self);
...
begin
   ...
   Sil1.Visible := TabCariListe.RecordCount > 0;
...
begin
  ...
  // ID := CariGridView.Controller.SelectedRecords[i].Values[CariGridViewID.Index];
   if (not REHBER.Active)or(REHBER.Active and (REHBER.RecordCount<1)) then
...
//  fb : TIcerikFrameBilgi;
  select,s2:string;
begin
  if (REHBER.Active)and(REHBER.RecordCount>0)and(CalendarEkstreBas.EditValue<>null)and(CalendarEkstreBit.EditValue<>null) then begin
      TabCariListe.Close;
...
//  if rehberdetayaktif then
  ...
  PageControlSekme.Visible := (REHBER.Active)and(REHBER.RecordCount>0);
  //cxSplitter1.OpenSplitter;
//   PageControlSekmeChange(Self);
...
begin
  ...
  if Tablo.Query1.RecordCount = 0 then begin
    Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, ' insert into KULLANICI (REHBERID,SIFRE,KOD,ROLID,DURUM,EKLEYEN,EKLEMETARIHI,SUBEID,DIL) values '+
...
begin
  {TabRehberIlgili.Close;
  TabRehberIlgili.Parameters[0].Value := REHBER.Fields[0].AsInteger;
  TabRehberIlgili.Parameters[1].Value := '%'+Trim(IlgiliAraEdit.Text)+'%';
  TabRehberIlgili.Open; }
  TabloYenile(TabRehberIlgili, [REHBER.Fields[0].AsInteger, '%'+Trim(IlgiliAraEdit.Text)+'%']);
  if TabRehberIlgili.RecordCount>0 then begin
    cxGridPersoneller.DataController.FocusedRecordIndex:=0;
    cxGridPersoneller.ViewData.Records[0].Selected:=True;
  end;

  IlgiliSilTus.Visible := TabRehberIlgili.RecordCount>0;
...
//                        ' from REHBER R left outer join REHBERPERSONEL P on R.ID=P.REHBERID ';
  if (FArama.ComboCariAnaliz.Visible)and(FArama.ComboCariAnaliz.Itemindex in [1..5]) then begin
     ...
     if FArama.ComboCariAnaliz.ItemIndex in [1,3] then
        s := s + ' and  isnull(R.ID,'''')<>'''' and (ISNULL(TOPLAM_BORC,0.0) - ISNULL(TOPLAM_ALACAK,0.0)) > 1.0 '
...
begin
   ...
   if EditKOD <> '0' then begin
      Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'Update REHBER Set GRUP=120, KOD='''+EditKOD+''' Where ID=&RehID',['&RehID'],[REHBER.FieldByName('ID').AsInteger]);
      CariGridView.DataController.DeleteRecord(CariGridView.DataController.FocusedRecordIndex);
   end;
end;

procedure TRehberAraDlg.MutabakatEkleMenuClick(Sender: TObject);
begin
  {Application.CreateForm(TMutabakatDlg, MutabakatDlg);
  MutabakatDlg.Showmodal;
  MutabakatDlg.Destroy; }
...
begin
   ...
   if Id>1 then
      Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'UPDATE EKIPMANREHBER SET REHBERID='+IntToStr(Id)+' WHERE ID = &ID', ['&ID'],[IntToStr(EID)]);
...
begin
  ...
  if not(REHBER.Active)or(REHBER.RecordCount<1) then
    Exit;
...
begin
  BorcAlacakKapamaMenu.Enabled :=(REHBER.Active)and(REHBER.RecordCount>0)and(REHBER.FieldByName('GRUP').AsInteger<>1) ;
...
begin
   ...
   if Veritabani.VeriVarMi(Tablo.FDCnn, 'select ID from CEKLER where REHBERID =  &RId', ['&RId'],[REHBER.FieldByName('ID').AsInteger]) then
    ...
    Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'Update REHBER Set GRUP=1, KOD='''+REHBER.FieldByName('ID').AsString+''' Where ID=&RehID',['&RehID'],[REHBER.FieldByName('ID').AsInteger]);
    CariGridView.DataController.DeleteRecord(CariGridView.DataController.FocusedRecordIndex);
end;
