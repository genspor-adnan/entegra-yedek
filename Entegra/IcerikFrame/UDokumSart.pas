...
uses
  ...
  SynHighlighterSQL, ImgList, StdActns, ActnList, FireDAC.Comp.Client, cxGridLevel, printers,
...
begin
   ...
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE DOKUMLER SET YAZICI=''' + YaziciAd + ''',KOPYASAY='+s+' WHERE ID =' +DtsDokumler.dataset.Fields[0].AsString, [], []);
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then
     //Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, ' delete from REHERINI where BOLUM=''&kod''',['&kod'],
