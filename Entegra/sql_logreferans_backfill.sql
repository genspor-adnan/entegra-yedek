...
-- Kasa tanimi (480): KASALAR
INSERT INTO GENDEPO.dbo.LOGREFERANS(TABLOID,KAYITID,AD,KOD,SILINDI,SONISLEM)
SELECT 480, k.ID, LEFT(k.KASAADI,200), LEFT(NULLIF(LTRIM(RTRIM(k.KASAKODU)),''),60), 0, GETDATE()
FROM KASALAR k
WHERE LTRIM(RTRIM(ISNULL(k.KASAADI,''))) <> ''
  AND NOT EXISTS(SELECT 1 FROM GENDEPO.dbo.LOGREFERANS l
                 WHERE l.TABLOID=480 AND l.KAYITID=k.ID AND l.AD=k.KASAADI COLLATE Turkish_CI_AS);

-- Kredi karti (46): KREDIKARTI
INSERT INTO GENDEPO.dbo.LOGREFERANS(TABLOID,KAYITID,AD,KOD,SILINDI,SONISLEM)
SELECT 46, k.ID, LEFT(k.ADI,200), LEFT(NULLIF(LTRIM(RTRIM(k.KODU)),''),60), 0, GETDATE()
FROM KREDIKARTI k
WHERE LTRIM(RTRIM(ISNULL(k.ADI,''))) <> ''
  AND NOT EXISTS(SELECT 1 FROM GENDEPO.dbo.LOGREFERANS l
                 WHERE l.TABLOID=46 AND l.KAYITID=k.ID AND l.AD=k.ADI COLLATE Turkish_CI_AS);

-- POS (69): POS
INSERT INTO GENDEPO.dbo.LOGREFERANS(TABLOID,KAYITID,AD,KOD,SILINDI,SONISLEM)
SELECT 69, p.ID, LEFT(p.ADI,200), LEFT(NULLIF(LTRIM(RTRIM(p.KODU)),''),60), 0, GETDATE()
FROM POS p
WHERE LTRIM(RTRIM(ISNULL(p.ADI,''))) <> ''
  AND NOT EXISTS(SELECT 1 FROM GENDEPO.dbo.LOGREFERANS l
                 WHERE l.TABLOID=69 AND l.KAYITID=p.ID AND l.AD=p.ADI COLLATE Turkish_CI_AS);

-- Krediler (47): KREDILER
INSERT INTO GENDEPO.dbo.LOGREFERANS(TABLOID,KAYITID,AD,KOD,SILINDI,SONISLEM)
SELECT 47, k.ID, LEFT(k.ADI,200), LEFT(NULLIF(LTRIM(RTRIM(k.KREDIKODU)),''),60), 0, GETDATE()
FROM KREDILER k
WHERE LTRIM(RTRIM(ISNULL(k.ADI,''))) <> ''
  AND NOT EXISTS(SELECT 1 FROM GENDEPO.dbo.LOGREFERANS l
                 WHERE l.TABLOID=47 AND l.KAYITID=k.ID AND l.AD=k.ADI COLLATE Turkish_CI_AS);

-- Banka hesabi (7): BANKAHESAPLAR
INSERT INTO GENDEPO.dbo.LOGREFERANS(TABLOID,KAYITID,AD,KOD,SILINDI,SONISLEM)
SELECT 7, b.ID, LEFT(b.HESAPADI,200), LEFT(NULLIF(LTRIM(RTRIM(b.HESAPKODU)),''),60), 0, GETDATE()
FROM BANKAHESAPLAR b
WHERE LTRIM(RTRIM(ISNULL(b.HESAPADI,''))) <> ''
  AND NOT EXISTS(SELECT 1 FROM GENDEPO.dbo.LOGREFERANS l
                 WHERE l.TABLOID=7 AND l.KAYITID=b.ID AND l.AD=b.HESAPADI COLLATE Turkish_CI_AS);
