...
-- Hangi kayıttan kaç tane geldiğini görelim
WITH CTE AS (
SELECT DISTINCT 
    R.ID, 
    R.KOD, 
    ...
    FATBASLIK = X1.BILGI, 
    R.GRUP, 
    -- R.TEMAS, R.SEKTOR, R.KATEGORI, R.SINIF, R.DURUM, R.OZELKOD, 
    TEMSILCIAD = R2.FIRMA,
...
LEFT OUTER JOIN REHBERILETISIM RI WITH (NOLOCK) ON R.ID = RI.REHBERID AND RI.VARSAYILAN = 1 
LEFT OUTER JOIN REHBERBILGI RB WITH (NOLOCK) ON RB.YER_ID = RI.ID AND RB.YERI = 1 
LEFT OUTER JOIN REHBERAYAR RA WITH (NOLOCK) ON RA.SIRA = RB.SIRA AND RA.YERI = 1 AND RA.VARSAYILAN = 8
