...
uses
  ...
  dxCoreGraphics, dxDateRanges, dxScrollbarAnnotations, FireDAC.Stan.Intf,
...
begin
  ...
  Tablo.ListedenBilgiGetir('İşlem Yapanı Seçiniz','SELECT	DISTINCT R.ID, R.FIRMA AS [İşlem Yapan]	 FROM	'+
...
begin
   if (TabLog.FieldByName('TABLOID').AsInteger=43)and(TabLog.FieldByName('TUR').AsInteger=4) then begin//tablo kasa ve değişme ise
...
begin
  if Trim(txtTablo.Text) <> '' then begin
    ...
    List := TStringList.Create;
    Tablo.ListedenBilgiGetir('İşlem Yapanı Seçiniz','SELECT DISTINCT G.DEGER AS ID,G.ANAHTAR AS TABLOADI,LH.TABLOALANADI AS ALANADI FROM LOG L'+
...
begin
  List := TStringList.Create;
  Tablo.ListedenBilgiGetir('İşlem Yapanı Seçiniz','SELECT DISTINCT G.DEGER AS ID, G.ANAHTAR AS TABLOADI FROM '+
...
begin
  ...
  if TabloId>0 then begin  //belli bir satir görünecekse
     //önce tablodan ekleyen vekleme tar. bulalım
