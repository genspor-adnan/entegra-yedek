...
$adimlar = @(
    ...
    @{ Ad = "OLAYLAR.BILGINO int (guncelleme kilitlenmesi)";       Dosya = "GenUpdate\olaylar_bilgino_int.sql" },

    @{ Ad = "OLAYLAR negatif komut no onarimi (16-bit tasma)";    Dosya = "GenUpdate\olaylar_negatif_versiyon_onar.sql" },
    ...
    @{ Ad = "Belge listeleri: Son Aranan altyapisi (index)";       Dosya = "GenUpdate\GenDepoUpdate66.sql" },

    # VKN/TCKN ayrac temizligi: SADECE rapor verir (RAPOR MODU). Duzeltmek icin

    #   dosyadaki @Uygula = 1 yapilip ELLE calistirilmali - hangi kayitlarin

    #   duzelecegi musteride goz ile gorulmeli (icine unvan/XXX yazilmis olanlara dokunmaz).

    @{ Ad = "VKN/TCKN ayrac temizligi (RAPOR)";                    Dosya = "GenUpdate\vergino_temizle.sql" },
