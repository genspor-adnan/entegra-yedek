                ...
                begin
                  ...
                  begin
                    firstStr := jVData_message.Value;
                        ...
                        firstStr := StringReplace(firstStr,'Ad ve soyadı'#10,'Ad ve soyadı:',[rfReplaceAll]);
                        firstStr := StringReplace(firstStr,'Telefon numarası'#10,'Telefon numarası:',[rfReplaceAll]);
                    ...
                    if _platform='INSTAGRAM' then
                     ...
                     begin
                        ...
                        Result.message_UserName  := ParseByTag('Ad ve soyadı:',firstStr);
                        Result.message_Phone  := ParseByTag('Telefon numarası:',firstStr);
                        Result.message_email    := ParseByTag('E-posta:',firstStr);
                        Result.PageName :=  ParseFirst( firstStr);
                        Result.Title := ParseFirst( firstStr);
                      end;
                  end;
                  // Sağ/Sol boşluklar temizlenmeli
  ...
  //https://graph.facebook.com/v18.0/1569153207222534?fields=organic_leads_count,name,id,page,created_time,status,leads{id},expired_leads_count
{
  ...
  "name": "Granit'(n)in 2 Aralık 2023 Cumartesi 14:31 tarihinde oluşturduğu form",
  "id": "1569153207222534",
  "page": {
    "name": "Granit",
    "id": "164408166754533"
                  ...
                  begin
                    ...
                    if GetJSONValue(jo,'name')<>nil then
                     begin
                       ...
                       if (tempName = 'FULL_NAME') or(tempName='adı_soyadı') then Result.FULL_NAME := tempVal; //'adı_soyadı'
                       if (tempName = 'EMAIL') or(tempName='e-posta') then Result.EMAIL := tempVal; //'e-posta'
                       if (tempName = 'PHONE') or(tempName='telefon_numarası') then Result.PHONE := tempVal; //'telefon_numarası'
                       if (tempName='diger_telefon_numarası') then Result.PHONE2 := tempVal; //'telefon_numarası' , 'diger_telefon_numarası'
                       ...
                       if (tempName = 'CITY') or(tempName='şehir') then Result.CITY:= tempVal;     //'şehir'
...
Kalıcı User Acess Token     EAAJFC5NMtMcBO3ittBKkTLDZC5XgqevELoeMnEk5zReNvScIpP6xuf1ZAaiPXCInrVaqFZCqxEmHwgvKCly4ZAoc3TxSNYtZBEfZCUoiicSj3Y3OnurJVV01iDIbvBLeateM0oj0ljEVlhoEkeIqRro2JZAZC5Mc6lw9hVz4RfD8zG82dIaEPI6TuMxJBoS6ZCLmwtRBIzv5M
Kalıcı Page Acess Token     EAAJFC5NMtMcBOZCpHHvfn7L0YSCjuk1C1GjUE4TDV9MXeZCZCZCYKZBu78w2bWqHZA8nHu8Np67DNweEpK4zPKOwn12UBn4vqdwK9DKQ4HxZCHaSxudTHNvIZCtTp8jlm88wgTk8LCVlppjP0Lmawz1ww7ALRwkZCI1h1E1eW0XIKsouZCjiPyvdsE1g7fyUHGOu9dHNad3kLglZACu82kZD
