...
begin
  // qryMetaCollect ADO dataset'ten dxMemCollect Memory dataset'e aktarım.
  ...
  try
    ...
    try
       while dxMemCollect.RecordCount>0 do
        ...
        {$IFDEF 3Dparty} _LogEkle('ADOTOMEM', qryMetaCollect.RecordCount.ToString+ ' Kayıt Aktarıldı'); {$endif}
...
begin
   {
     META_Collect Contact_RehberID eşleştirilmiş, fakat zaman içinde "REHBER" tablosundan silinmiş olabilir.
     Dolayısıyla İçeri alınmış gibi görünmeye devam edecektir.
     Form başlatılıdğında veya "Yenileme" sonrasinda mutlaka güncellenmeli
   }
...
begin
  ...
  FacebookLoggerMethod := LogSocialMedia; // her hangi bir method eşitlenebilir.
  ...
  Rehber_Potansiyel_Proc := TFDStoredProc.Create(Self);
   try
      ...
      AddParam(Rehber_Potansiyel_Proc.Params, '@ResultID',       ftInteger,    10, ptOutput);
       ...
       {Selected kayıtlar için dönecek}
      ...
      try
         ...
         for donguSecim := 0 to MetaView.Controller.SelectedRowCount-1 do
          begin
             gridRow := MetaView.Controller.SelectedRows[donguSecim];
             // Daha önce aktarılmışlar tekrar aktarılmıyor!
             if Not gridRow.Values[MetaViewImported.Index] then
              ...
              begin
                ...
                if Not TryStrToInt(Kullanan, Ekleyen) then
                  Ekleyen := -1;
                //Ekleyen := KullananID; //-1; //Ekleyen Kullanıcı Uygulamayı Açan kişi olmalı
                Temsilci := -1; // Henüz atanmış kimse yok
                Sinif := 0;
                ...
                { Memory Dataset verilerinden "Rehber_Potansiyel_Proc" çalıştırldı
...
begin
  //
  labelInfo.Caption := 'Mails Modül oluşturuluyor.';
  ...
  { MulitThread yordamlar ADO bağlantısı için ayrı bir kopyasıyla çalışacaktır
    MainThread içinde kullanıdığımız ADOconnection beklenmeyen sonuçlara ve programın donmasına neden olur
    Geçici olarak yeni bir ADOconnection ile çalışılacaktır.
  }
  ...
  try
    ...
    labelInfo.Caption := 'Mails sorgusu hazırlanıyor';
    qryData:= TFDQuery.Create(Self);
    //IMAP_UID max değeri alınıyor
    ...
    qryData.Close;
    {Hiç kaydedilmiş IMAP_UID değeri yoksa "0" gelecektir, bu koşulda ilk IMAP_UID değeri en az "1" olmalı
     Aksi durumda Son Kaydedilmiş IMAP_UID değerinden devam etmeli }
    if (sonUID = 0) then
      sonUID := sonUID + 1;
    ...
    if ImapModule.Connected then
     begin
       ...
       labelInfo.Caption := 'Mails IMAP4 yeni postalar sorgulanıyor';
      ...
      {UID numarasından önce "UID" deyimi çok önemli!
       UID SEARCH 267:* yazılması halinde Sequence numarasından başlar
       UID SEARCH UID 267:* yazılması UID numarasından başlar. istenen şey!
      }
       SearchStr := SearchStr + 'UID '+sonUID.ToString+':*';
       {$IFDEF 3DParty} LogSocialMedia('SEARCH STR = '+SearchStr,'SOCIAL_IMAP'); {$ENDIF}
       {
        UID SEARCH UID 267:*
        Arama kriterlerine uygun Liste oluştu
       }
       ...
       { Arama Listesi kadar döngü kurulup
         ...
         Tüm postaların toplu halde gelmesi durumunda Uzun bir string dönebilir!!
       }
       ...
       for i := 0 to SearchList.Count-1 do
         begin
           labelInfo.Caption := 'Mails ('+SearchList[i]+') ePosta Verisi alınıyor';
       ...
       begin
         labelInfo.Caption := 'IMAP ePosta bağlantısı sağlanamadı!';
         raise Exception.Create('IMAP ePosta bağlantısı sağlanamadı!');
       end;

  finally
    labelInfo.Caption := 'Posta işleme tamamlandı';
    if Assigned(ImapModule) then
     begin
       if ImapModule.Connected then
...
begin
  ...
  if PageID='' then
    raise Exception.Create('Sayfa ID tanımsız doğru bilgileri tanımlayınız');
  ...
  try
    ...
    begin
    {$IFDEF 3DParty}LogSocialMedia('1.LongLiveToken alındı'); {$ENDIF}

    {$REGION 'Conversations'}
    Conversations.Clear;
    labelInfo.Caption := 'Messenger verileri alnıyor';
    GetConversationsList(PageID,'MESSENGER',LongLiveToken, Conversations);
    {$IFDEF 3DParty}LogSocialMedia('2.GetConversationsList MESSENGER Sonrası');{$ENDIF}
    labelInfo.Caption := 'Instagram verileri alnıyor';
    GetConversationsList(PageID,'INSTAGRAM',LongLiveToken, Conversations);
      {$IFDEF 3DParty}LogSocialMedia('3.GetConversationsList INSTAGRAM Sonrası');{$ENDIF}
    ...
    {$IFDEF 3DParty}LogSocialMedia('4.qryMetaCollect.Open');{$ENDIF}
      for i := 0 to Conversations.Count-1 do
       begin
         ...
         for y := 0 to Messages.Count-1 do
            begin
              labelInfo.Caption := 'Messenger/Instagram verileri işleniyor ('+i.ToString+')';
              ...
              begin
                // Contact_ID benzeri kayıt olmamalı
     ...
     {--}
      ...
      {$REGION 'FormLeads'}
      labelInfo.Caption := 'Forms Listesi alınıyor verileri alnıyor';
      ...
      begin
        ...
        begin
          FormLeads.Clear;
          labelInfo.Caption := 'Forms('+i.ToString+') '+PageForms[i].Text+' locale : '+PageForms[i].Locale+' Etkileşim alınıyor...';
          ...
          for y := 0 to FormLeads.Count-1 do
           begin
            ...
            // begin
               labelInfo.Caption := 'Forms('+i.ToString+') '+PageForms[i].Text+' Etkileşim : ('+y.ToString+') Verisi alınıyor';
    ...
    {}       // META_Collect ADO Veritabanı dxMemCollect Memory tabloya aktarılacak !
    //AdoToMem; Bunu Thread bitiminde çağır
    {}
  end;
end;
...
begin
  // DoWork...
   ...
   FillChar(SosyalEklentiler, 10 , True);
  {
    Meta Verileri sürekli sorgulanmasın , OPT/CRM Meta sekmesinde parametrik bir değere bağlı olarak,
    Zaman aralığına uygun davransın (15, 30, 120 dakika vs)

    GENINI "Ops_SocialMedia_MetaGuncelleme" anahtarı ile kaydedilen Dakika aralığına uygunluğu gözetlenecektir.
    Uygun Zaman gelmemiş ise boşuna META verilerini çekmesin
  }
  ...
  // UNUTMA remark kaldırılacak

  //(1) Meta işleri yapılacak
   if Tablo.GENINI.ReadBoolean(Ops_SocialMedia_Meta, False) and Initialized and SosyalEklentiler[1] then
    begin
      ...
      labelInfo.Caption := 'Meta Güncelleme Tamamlandı';

    end;

    if Not MetaZamanUygunmu then
      labelInfo.Caption := 'Güncelleme zaman kıstası uygulandı';
    Sleep(1000);
    ...
    begin
      ...
      labelInfo.Caption := '"Web Form" IMAP Güncelleme Tamamlandı';
      Sleep(1000);
    end;
...
begin
  {$IFDEF 3DParty}LogSocialMedia('Thread Sonlandı');{$ENDIF}
  labelInfo.Caption := 'Güncelleme Tamamlandı';
end;
...
begin
  ...
  if (dxMemCollect.FieldByName('Imported').AsBoolean) {or (dxMemCollect.FieldByName('Import_Date').AsString<>'')} then
     begin
        Error := True;
        ErrorText := 'Daha önce içeri alınmış bir kayıt Seçilemez';
        dxMemCollect.Cancel;
     end;
  *)
end;
...
begin
   {Gruplama yapıldığında hata olmasın, çünkü Gruplama anına Grup Satırı için AItem NULL gelecetir}
   if Not Assigned(AItem) then
     Exit;

   // Imported = İçeri aktarılmış ise
...
begin
   ...
   if INITarih = 0 then
    begin
      Result := True;
      {$IFDEF 3DParty}LogSocialMedia('INI Tanımı olmadığından Varsayılan TRUE');{$ENDIF}
    end
    else
     begin
...
begin
  ...
  if (AppId='') or (AppSecret='') or (PageID='') or (PageToken='') then
    raise Exception.Create('Parametreler boş olamaz!');
   if AppName='' then
     AppName := 'DefaultApp';
   {
