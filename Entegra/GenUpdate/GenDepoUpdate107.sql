-- ============================================================
-- GenDepoUpdate107.sql
-- SP ZINCIRI YENI SERI/LOT KABUL EDER (giris donusumleri)
--
-- GenDepoUpdate103 izlemeler[] icin "serilotId" desteklemisti - yani lotun
--   KART OLARAK ZATEN VAR OLDUGUNU varsayiyordu. Cikista dogru (depodan
--   secilir) ama GIRISTE degil: siparis -> alis irsaliyesi/faturasi
--   donusumunde lot TEDARIKCIDEN gelir ve sistemde HENUZ YOKTUR.
--
-- Ekran artik serilotId yerine seriNo/lotNo/skt/urt gonderebiliyor; bu surumde
--   kart donusumun KENDI TRANSACTION'INDA acilir (once aranir, yoksa eklenir).
--   Boylece donusum yarida kalirsa lot karti da geri sarilir.
--
    ...
    UPDATE I SET I.SerilotId = SSL.ID
      FROM #DonusumIzlemeSecim I
           INNER JOIN dbo.STOKSERILOT SSL ON SSL.STOKID = I.UrunId
                  AND ISNULL(SSL.SERINO, N'') = ISNULL(I.SeriNo, N'')
                  AND ISNULL(SSL.LOTNO,  N'') = ISNULL(I.LotNo,  N'')
...
BEGIN
    ...
    CREATE TABLE #DonusumIzlemeSecim (
        ...
        SeriNo NVARCHAR(64) NULL, LotNo NVARCHAR(50) NULL,
    ...
    BEGIN TRY
        ...
        DECLARE @Sec TABLE (SatirId INT, StokIzlemeId INT, SerilotId INT NULL,
                            UrunId INT, SeriNo NVARCHAR(64), LotNo NVARCHAR(50),
