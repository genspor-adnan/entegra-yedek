-- ============================================================
-- GenDepoUpdate80.sql
-- sp_Api_Belge_Kaydet_Json : ISLEMLOG (ekleme/degistirme/silme) yazsin
--   + dbo.fn_Api_Belge_TabNo : TUR -> kart/detay TABLOID eslemesi (tek kaynak)
--
-- NEDEN: Kaydet SP'si hic log yazmiyordu. Silme (sp_Api_Belge_Sil_Json) loglu
--   oldugu icin UInfo'da silme goruluyor ama Ubelgegiris ile olusturulan belgeler
--   log ekraninda HIC gorunmuyordu (kullanici bildirdi: belge 114045/114046 ->
--   sifir log satiri). Delphi sihirbazlarinda ekleme logunu Pascal yaziyor
--   (ULog.LogKartEkle); API ile olusan belgede o yol calismiyor.
--
-- NE YAZILIR (ULog.LogYaz ile ayni kolonlar; sp_Api_Log_Yaz_Ic uzerinden)
--   kart   : yeni belgede ISLEMTIPI=1 (ekle), guncellemede 2 (degistir)
--   satir  : eklenen 1, guncellenen 2, SILINEN 0 -- silme logu satir SILINMEDEN
--            ONCE yazilir (Geri Al buna bagli).
--   TABLOID esleme fn_Api_Belge_TabNo ile; sp_Api_Belge_Sil_Json'daki ile AYNI.
--
-- FARK (bilincli): ULog "degistir" logunu ALAN BAZLI diff olarak yazar
--   (LogDiffKaydet). Burada tam satir anlik goruntusu yazilir - SP eski degeri
--   bilmiyor. UInfo'da islem gorunur; alan bazli diff icin sihirbaz yolu gerekir.
-- ============================================================
SET ANSI_NULLS ON;
