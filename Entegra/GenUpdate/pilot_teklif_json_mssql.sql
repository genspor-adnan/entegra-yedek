-- ============================================================
-- PILOT: sp_Prog_Teklif_Liste_Json — tek JSON parametre (MSSQL)
--   Tipli-param sp_Prog_Teklif_Liste ile AYNI govde; sadece arayuz JSON.
--   @Filtre = '{"Mod":4,"TarihBas":"2026-01-01",...}' -> JSON_VALUE ile yerel degiskenlere.
--   Amac: JSON vs tipli-param karsilastirmasi (parite + sure + kod).
-- ============================================================
CREATE OR ALTER PROCEDURE dbo.sp_Prog_Teklif_Liste_Json
    @Filtre NVARCHAR(MAX)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SelectList        NVARCHAR(MAX) = ISNULL(JSON_VALUE(@Filtre,'$.SelectList'), N'');
    DECLARE @TopN              INT           = ISNULL(TRY_CAST(JSON_VALUE(@Filtre,'$.TopN')  AS INT), 0);
    DECLARE @Mod               SMALLINT      = ISNULL(TRY_CAST(JSON_VALUE(@Filtre,'$.Mod')   AS SMALLINT), 4);
    DECLARE @TarihBas          DATE          = TRY_CAST(JSON_VALUE(@Filtre,'$.TarihBas') AS DATE);
    DECLARE @TarihBit          DATE          = TRY_CAST(JSON_VALUE(@Filtre,'$.TarihBit') AS DATE);
    DECLARE @Hazirlayan        NVARCHAR(200) = JSON_VALUE(@Filtre,'$.Hazirlayan');
    DECLARE @Musteri           NVARCHAR(200) = JSON_VALUE(@Filtre,'$.Musteri');
    DECLARE @Konusu            NVARCHAR(200) = JSON_VALUE(@Filtre,'$.Konusu');
    DECLARE @Turu              INT           = TRY_CAST(JSON_VALUE(@Filtre,'$.Turu') AS INT);
    DECLARE @BelgeNo           NVARCHAR(100) = JSON_VALUE(@Filtre,'$.BelgeNo');
    DECLARE @Stok              NVARCHAR(200) = JSON_VALUE(@Filtre,'$.Stok');
    DECLARE @Durumu            INT           = TRY_CAST(JSON_VALUE(@Filtre,'$.Durumu') AS INT);
    DECLARE @Revize            BIT           = ISNULL(TRY_CAST(JSON_VALUE(@Filtre,'$.Revize') AS BIT), 0);
    DECLARE @Kabul             BIT           = ISNULL(TRY_CAST(JSON_VALUE(@Filtre,'$.Kabul') AS BIT), 0);
    DECLARE @Reddedilenler     BIT           = ISNULL(TRY_CAST(JSON_VALUE(@Filtre,'$.Reddedilenler') AS BIT), 0);
    DECLARE @SubeYetkiList     NVARCHAR(MAX) = JSON_VALUE(@Filtre,'$.SubeYetkiList');
    DECLARE @HazirlayanZorunlu INT           = TRY_CAST(JSON_VALUE(@Filtre,'$.HazirlayanZorunlu') AS INT);
    DECLARE @SubeZorunlu       INT           = TRY_CAST(JSON_VALUE(@Filtre,'$.SubeZorunlu') AS INT);
    DECLARE @KulId             INT           = TRY_CAST(JSON_VALUE(@Filtre,'$.KulId') AS INT);
    DECLARE @Modul             INT           = TRY_CAST(JSON_VALUE(@Filtre,'$.Modul') AS INT);
    DECLARE @OrderBy           NVARCHAR(200) = JSON_VALUE(@Filtre,'$.OrderBy');
