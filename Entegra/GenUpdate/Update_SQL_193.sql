-- Update_SQL_193.sql
-- Belge donusumunde (siparis -> irsaliye/fatura, irsaliye -> fatura) kaynak belgenin
-- alttaki ACIKLAMA'si ("Not 1" memo) hedef belgeye tasinir (kullanici: "siparisten
-- irsaliyeye ve irsaliyeden faturaya alttaki fatura aciklamasi gecsin").
-- Update_SQL_190 ile alan bos birakiliyordu; simdi YENI hedef belgede kaynak
-- SIPARIS.ACIKLAMA / FATBASLIK.ACIKLAMA doluysa LEFT(...,300) olarak yazilir.
-- Var olan hedefe ekleme (@HedefBaslikID > 0) dokunulmaz; kaynak bos ise bos kalir.
-- Baslik JSON'undaki Aciklama degil, Kaydet sonrasi dogrudan UPDATE kullanilir:
-- sp_Api_Belge_Kaydet_Json basligi 200 karakterde keser, FATBASLIK.ACIKLAMA 300.
--
-- Degisen tek sey sp_Prog_BelgeDonusum_Kaydet'teki ANAKAYITID blogunun yanina eklenen
-- ACIKLAMA guncellemesidir; SP'nin geri kalani Update_SQL_190.sql ile ayni.
-- Mevcut kayitlar degismez. Idempotent (CREATE OR ALTER).
...
BEGIN
    ...
    -- Update193: kaynak belgenin alttaki aciklamasi (Not 1) yeni hedef belgeye gecsin.
    IF @YeniBelgeOut = 1
    BEGIN
        DECLARE @KaynakAciklama NVARCHAR(600) =
            CASE WHEN @KaynakBaslikTablo = 'SIPARIS'
                 THEN (SELECT ACIKLAMA FROM SIPARIS   WHERE ID = @BaslikId)
                 WHEN @KaynakBaslikTablo = 'FATBASLIK'
                 THEN (SELECT ACIKLAMA FROM FATBASLIK WHERE ID = @BaslikId)
                 ELSE NULL END;
        IF LTRIM(RTRIM(ISNULL(@KaynakAciklama, N''))) <> N''
            UPDATE FATBASLIK SET ACIKLAMA = LEFT(@KaynakAciklama, 300)
             WHERE ID = @HedefBaslikOut;
    END
