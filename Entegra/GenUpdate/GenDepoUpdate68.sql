	...
	SELECT
		...
		DEGER = CASE 
					WHEN ISNULL(FB.FATURADOVIZI,'TL')  = 'TL' THEN
						...
						ELSE CAST((KDV*((((F.BIRIMFIYAT*ADET)*(100.0-ROUND(ISKONTO,G.DEGER))/100))/100.0)) AS decimal(18,6)) END),2)
					ELSE
						ROUND((SUM(CASE WHEN KDVDURUM ='Dahil' THEN ((DOVIZ_BIRIMFIYAT*ADET)*(100.0-ROUND(ISKONTO,G.DEGER))/100)-((((DOVIZ_BIRIMFIYAT*ADET)*(100.0-ROUND(ISKONTO,G.DEGER))/100))*(100.0/(100.0+KDV))) 
								...
								ELSE CAST((KDV*((case when F.DOVIZ_KURU=FB.RAPORDOVIZ then ((DOVIZ_BIRIMFIYAT*ADET)*(100.0-ROUND(ISKONTO,G.DEGER))/100) else F.TUTAR/FB.DOVIZKUR end)/100.0)) AS decimal(18,6))  END)),2)* FB.DOVIZKUR
				END,
		DOVIZTUTARI = ROUND((SUM(CASE WHEN KDVDURUM ='Dahil' THEN ((DOVIZ_BIRIMFIYAT*ADET)*(100.0-ROUND(ISKONTO,G.DEGER))/100)-(((DOVIZ_BIRIMFIYAT*ADET)*(100.0-ROUND(ISKONTO,G.DEGER))/100)*(100.0/(100.0+KDV))) 
								when OTVMIKTAR>0.0 then (((case when F.DOVIZ_KURU=FB.RAPORDOVIZ then F.DOVIZ_TUTARI else F.TUTAR/FB.DOVIZKUR end)+
								((CASE WHEN F.OTVYUZDE =0 THEN ADET*OTVMIKTAR ELSE (OTVMIKTAR*(((case when F.DOVIZ_KURU=FB.RAPORDOVIZ then F.DOVIZ_BIRIMFIYAT else F.BIRIMFIYAT/FB.DOVIZKUR end)*F.ADET)/100.0))  END)))*KDV/100.0)
								ELSE CAST((KDV*((case when F.DOVIZ_KURU=FB.RAPORDOVIZ then ((DOVIZ_BIRIMFIYAT*ADET)*(100.0-ROUND(ISKONTO,G.DEGER))/100) else F.TUTAR/FB.DOVIZKUR end)/100.0)) AS decimal(18,6))  END)
								),2),
		FB.KUR,
		FB.RAPORDOVIZ,
		DOVIZKUR,
