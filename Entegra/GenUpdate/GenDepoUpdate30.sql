-- ============================================================
-- GenDepoUpdate30 : sp_Prog_AlisSatis_IrsFatFisKons_Json2  (SELF-CONTAINED - TEK SP)
--   ANA DB. Fatura/Irsaliye/Fis liste. @Baslik (SELECT ek kolon) + @Kosullar (JSON). Wrapper KALDIRILDI.
--   Kullanilmayan sp_Prog_AlisSatis_IrsFatFisKons DROP edilir.
--   PERF: (1) bos filtre 'LIKE %%' EKLEMEZ (sargable+NULL-kolon bug); (2) metin filtreleri
--         PARAMETRELI (REPLACE-concat degil) -> tek plan tekrar kullanilir, injection yuzeyi yok.
...
END
GO
IF OBJECT_ID('dbo.sp_Prog_AlisSatis_IrsFatFisKons','P') IS NOT NULL
    DROP PROCEDURE dbo.sp_Prog_AlisSatis_IrsFatFisKons;
GO
PRINT 'sp_Prog_AlisSatis_IrsFatFisKons_Json2: self-contained + parametreli filtreler; sp_Prog_AlisSatis_IrsFatFisKons kaldirildi';
