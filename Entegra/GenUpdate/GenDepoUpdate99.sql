-- ============================================================
-- GenDepoUpdate99.sql
-- IZLEME EKRANI - ADIM A6: sp_Prog_Izleme_Aday_Json  CIKIS ve GIRIS modlari
--
-- A1'de yalnizca mod='donusum' vardi (digerleri 51001 ile reddediliyordu).
--   Bu surumde ucu de calisiyor. Sayim (eski IslemTur=99) BILINCLI OLARAK YOK:
--   K-A karari ile ayri is akisina cikarildi (veri gerekcesi: sayimda
--   STOKIZLEME'de hic kayit olusmuyor, depo hareketi de yapilmiyor).
--
-- UC YONTEM - hepsi ayni CIKTI semasini verir
--   'tasima' : kaynak belge satirindaki seri/lot kayitlari (donusum, kaynak FATURA)
--   'depo'   : depodaki bakiyeden secim  (cikis belgeleri + kaynagi SIPARIS olan
--              donusumler)   MEVCUT = STOKDURUMIZLEME.KALAN eksi BU SATIRIN
--              kendi hareketi
--   'kendi'  : bu belge satirinin kendi kayitlari (giris belgeleri - kullanici
--              yeni seri/lot girer, mevcutlar isaretli gelir)
--
-- MOD -> YONTEM
--   donusum + kaynak FATURA        -> tasima
--   donusum + kaynak SIPARISDETAY  -> depo
--   cikis                          -> depo
--   giris                          -> kendi
--
-- Eski karsiliklari (UIzleme.dfm):
--   tasima = SQLDonusCikanHedef + SQLDonusCikanHedefUpdate
--   depo   = SQLCikan + SQLCikanUpdate
--   kendi  = SQLGiren
--
