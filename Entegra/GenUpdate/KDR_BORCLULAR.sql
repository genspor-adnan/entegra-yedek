/* ============================================================================
   KDR_BORCLULAR.sql  —  MUSTERI ANA veritabaninda calistirilir

   "KDR_BORÇLULAR" karar destek raporunun DOGRU sorgusunu DOKUMLER tablosuna yazar
   ...
   NOT: Dosya UTF-8 (BOM) kodludur. sqlcmd ile calistirirken  -f 65001  ZORUNLU:
        sqlcmd -S SUNUCU -d MUSTERIDB -U sa -P *** -C -f 65001 -i KDR_BORCLULAR.sql
...
DECLARE @Rapor  sysname       = N'KDR_BORCLULAR';
...
SELECT 
       XXX.KOD,XXX.FIRMA,BAKIYE=convert(float,BAKIYE),KUR,TLKARSILIK=convert(float,(BAKIYE*(select top 1 D.SATIS from DOVIZ D where D.CINSI=XXX.KUR order by D.TARIH desc)))  
FROM (
select  DISTINCT R.SUBEID,R.KOD ,R.FIRMA,cast(TOPLAM_BORC as float) AS BORC ,cast(TOPLAM_ALACAK as float) AS ALACAK,KUR ,BAKIYE=TOPLAM_BORC-TOPLAM_ALACAK 
 ...
 FROM(
    ...
    where  
		ISNULL(RU.KDR_Disi,''False'') = ''False'' and R.ID > 0 and isnull(R.ID,'''')<>'''' and R.GRUP <> 335 
		AND TOPLAM_BORC>TOPLAM_ALACAK          
   ) AS XXX       
 INNER JOIN REHBER R2  ON  XXX.SUBEID=R2.ID
