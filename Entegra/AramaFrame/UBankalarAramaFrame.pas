...
begin
   ...
   if ComboSube.Visible then begin
      ComboSube.Properties.Items := Tablo.imgComboboxInit('Select ID=0,FIRMA='''' union all select R.ID,R.FIRMA from REHBER R inner join YETKI Y on cast((''2598''+cast(-R.ID as varchar(10))) as int)=Y.MODULID where R.ID<0 and Y.ROLID='+RolId+' and Y.HAK=1 ').Items;
