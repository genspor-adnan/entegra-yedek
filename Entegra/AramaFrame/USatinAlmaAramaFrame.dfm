object SatinAlmaAramaFrame: TSatinAlmaAramaFrame
  Left = 0
  Top = 0
  Width = 245
  Height = 338
  Align = alClient
  Color = clWhite
  Font.Charset = TURKISH_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Trebuchet MS'
  Font.Style = []
  ParentBackground = False
  ParentColor = False
  ParentFont = False
  TabOrder = 0
  object YenileTus: TSpeedButton
    Tag = 4
    Left = 4
    Top = 1
    Width = 69
    Height = 22
    Caption = 'Yenile'
    Flat = True
    Glyph.Data = {
      
      0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D}
  end
  object Label1: TcxLabel
    Left = 0
    Top = 105
    Caption = 'M'#252#351'teri'
    ParentFont = False
    Style.Font.Charset = TURKISH_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'Trebuchet MS'
    Style.Font.Style = []
    Style.IsFontAssigned = True
    Transparent = True
  end
  object Label4: TcxLabel
    Left = 0
    Top = 77
    Caption = 'Haz'#305'rlayan'
    ParentFont = False
    Style.Font.Charset = TURKISH_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'Trebuchet MS'
    Style.Font.Style = []
    Style.IsFontAssigned = True
    Transparent = True
  end
  object AraRevize: TcxCheckBox
    Left = 0
    Top = 219
    Caption = 'Revizeleri de g'#246'ster'
    TabOrder = 0
    Transparent = True
  end
  object AraTarihBas: TcxDateEdit
    Left = 77
    Top = 23
    Enabled = False
    Properties.ImmediatePost = True
    TabOrder = 1
    Width = 115
  end
  object AraTarihBit: TcxDateEdit
    Left = 77
    Top = 48
    Enabled = False
    Properties.ImmediatePost = True
    TabOrder = 2
    Width = 115
  end
  object AraMusteri: TcxButtonEdit
    Left = 77
    Top = 104
    ParentShowHint = False
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = '-'
        Hint = 'Temizle'
        Kind = bkText
      end>
    Properties.ReadOnly = False
    Properties.OnButtonClick = AraMusteriPropertiesButtonClick
    ShowHint = True
    TabOrder = 3
    OnKeyDown = AraMusteriKeyDown
    Width = 115
  end
  object AraHazirlayan: TcxButtonEdit
    Left = 77
    Top = 76
    ParentShowHint = False
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = '-'
        Hint = 'Temizle'
        Kind = bkText
      end>
    Properties.ReadOnly = False
    Properties.OnButtonClick = AraHazirlayanPropertiesButtonClick
    ShowHint = True
    TabOrder = 4
    OnKeyDown = AraHazirlayanKeyDown
    Width = 115
  end
  object cxLabel28: TcxLabel
    Left = 0
    Top = 190
    Caption = 'T'#252'r'#252
    ParentFont = False
    Style.Font.Charset = TURKISH_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'Trebuchet MS'
    Style.Font.Style = []
    Style.IsFontAssigned = True
    Transparent = True
  end
  object cxLabel22: TcxLabel
    Left = 0
    Top = 162
    Caption = 'Konusu'
    ParentFont = False
    Style.Font.Charset = TURKISH_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'Trebuchet MS'
    Style.Font.Style = []
    Style.IsFontAssigned = True
    Transparent = True
  end
  object cxLabel1: TcxLabel
    Left = 0
    Top = 134
    Caption = 'Durumu'
    ParentFont = False
    Style.Font.Charset = TURKISH_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'Trebuchet MS'
    Style.Font.Style = []
    Style.IsFontAssigned = True
    Transparent = True
  end
  object AraIkiTarih: TcxCheckBox
    Left = 0
    Top = 36
    Caption = 'Tarih'
    Properties.OnEditValueChanged = AraIkiTarihPropertiesEditValueChanged
    TabOrder = 8
    Transparent = True
  end
  object AraDurumu: TcxImageComboBox
    Left = 77
    Top = 132
    RepositoryItem = Tablo.repTeklifDurumu
    Properties.ClearKey = 46
    Properties.Items = <>
    TabOrder = 11
    Width = 115
  end
  object AraTuru: TcxImageComboBox
    Left = 77
    Top = 188
    RepositoryItem = Tablo.repTeklifTuru
    Properties.Items = <>
    TabOrder = 12
    Width = 115
  end
  object AraKabulEdilenler: TcxCheckBox
    Left = 0
    Top = 247
    Caption = 'Kabul edilenleri de g'#246'ster'
    TabOrder = 13
    Transparent = True
  end
  object AraReddedilenler: TcxCheckBox
    Left = 0
    Top = 272
    Caption = 'Reddedilenleri de g'#246'ster'
    TabOrder = 14
    Transparent = True
  end
  object AraKonusu: TcxLookupComboBox
    Left = 77
    Top = 160
    Hint = 'Teklif_Konusu'
    Properties.ClearKey = 46
    Properties.DropDownListStyle = lsFixedList
    Properties.KeyFieldNames = 'ID'
    Properties.ListColumns = <
      item
        Caption = 'Konusu'
        FieldName = 'KONUSU'
      end>
    Properties.ListSource = DtsSK
    Properties.OnInitPopup = AraKonusuLokPropertiesInitPopup
    TabOrder = 15
    Width = 115
  end
  object TabSK: TFDQuery
    Connection = Tablo.FDCnn
    ParamData = <>
    SQL.Strings = (
      'select null as ID, null as KONUSU'
      'union all '
      
        'select SIRANO as ID, ANAHTAR as KONUSU from REHBERINI where BOLU' +
        'M='#39'Teklif_Konusu'#39' order by 2')
    Left = 351
    Top = 107
  end
  object DtsSK: TDataSource
    DataSet = TabSK
    Left = 350
    Top = 43
  end
end



