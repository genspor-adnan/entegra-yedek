...
uses
  ...
  cxLookAndFeelPainters, cxButtons, CategoryButtons, ImgList, DB, FireDAC.Comp.Client,
  UFrameYoneticisi;
...
begin
  ...
  try
    with (FFramebilgi.IcerikFrameYoneticisi.FrameBul(TDokumDlg).Ornek as TDokumDlg).TabDokum do begin
      Qry.Connection := Tablo.FDCnn;
      Qry.SQL.Assign(SQL);
    end;
    if Assigned(Qry.Params.FindParam('MOD')) then begin
      with Qry.Params.ParamByName('MOD') do begin
        DataType := ftString;
        if Size < 10 then
          Size := 10;
        AsString := FDokumEkranAdi;
      end;
    end;
    if Assigned(Qry.Params.FindParam('ROL')) then begin
      with Qry.Params.ParamByName('ROL') do begin
        DataType  := ftInteger;
        AsInteger := StrToIntDef(RolID, 0);
      end;
    end;
    if Assigned(Qry.Params.FindParam('STNDRT')) then begin
      with Qry.Params.ParamByName('STNDRT') do begin
        DataType := ftSmallint;
        AsSmallInt := Stndrt;
      end;
    end;
    if Assigned(Qry.Params.FindParam('DURUM')) then begin
      with Qry.Params.ParamByName('DURUM') do begin
        DataType := ftSmallint;
        AsSmallInt := Durum;
      end;
    end;
    if Assigned(Qry.Params.FindParam('STN')) then begin
      with Qry.Params.ParamByName('STN') do begin
        DataType := ftSmallint;
        AsSmallInt := Stndrt;
      end;
    end;
    if Assigned(Qry.Params.FindParam('DUR')) then begin
      with Qry.Params.ParamByName('DUR') do begin
        DataType := ftSmallint;
        AsSmallInt := Durum;
      end;
    end;
    if Assigned(Qry.Params.FindParam('DRM')) then begin
      with Qry.Params.ParamByName('DRM') do begin
        DataType := ftSmallint;
        AsSmallInt := Durum;
      end;
    end;
    for j := 0 to Qry.Params.Count - 1 do
      if Qry.Params[j].DataType = ftUnknown then begin
        pName := UpperCase(Qry.Params[j].Name);
        if pName = 'MOD' then begin
          Qry.Params[j].DataType := ftString;
          if Qry.Params[j].Size < 10 then
            Qry.Params[j].Size := 10;
          Qry.Params[j].AsString := FDokumEkranAdi;
        end else if pName = 'ROL' then begin
          Qry.Params[j].DataType := ftInteger;
          Qry.Params[j].AsInteger := StrToIntDef(RolID, 0);
        end else if (pName = 'STN') or (pName = 'STNDRT') then begin
          Qry.Params[j].DataType := ftSmallint;
          Qry.Params[j].AsSmallInt := Stndrt;
        end else if (pName = 'DUR') or (pName = 'DURUM') then begin
          Qry.Params[j].DataType := ftSmallint;
          Qry.Params[j].AsSmallInt := Durum;
        end else begin
          Qry.Params[j].DataType := ftWideString;
          Qry.Params[j].Clear;
        end;
      end;
    Qry.Open;
    if Qry.Eof then   //Eğer tablo boşsa sayfayı direk açsın
      ...
      begin
        Tamam := True;
          ...
          if Qry.FieldByName('GRUBU').AsString = '' then
            with genelKat.Items.Add do begin
              Caption := Qry.FieldByName('RAPORADI').AsString;
              New(yb);
              Data := yb;
              yb^.RaporAdi := Caption;
              ImageIndex := 0;
          ...
          begin
            idx := dokumListesi.Categories.IndexOf(Qry.FieldByName('GRUBU').AsString);
            if idx <> -1 then begin
              ...
              with dokumListesi.Categories.Add do begin
                Caption := Qry.FieldByName('GRUBU').AsString;
