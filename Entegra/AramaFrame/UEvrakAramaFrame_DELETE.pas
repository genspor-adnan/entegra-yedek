...
uses {$IFDEF 3Dparty}
  ...
  {$ENDIF 3Dparty}
   ...
   UGirisKutusuEx, PrjConst, FetaKurulusSiniflari, UDokumanListeFrame, UDokumanYetki, LocOnFly, uEvrakModule, UVeriMotor;
...
//  TabKlasorler.sql.Add(' order by USTID, AD');
  ...
  {
   "DOKUAMNKLASOR"  Evrak yapısı için varsayılan Veriler oluşacak
  }
  InitEvrakKlasorler;
  ...
  //yetkili değilse popupmenü çıkmasın
  if TamYetkili = False then
...
begin
  {if TabKlasorler.FieldByName('ID').AsInteger=-1 then begin
     Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update DOKUMANKLASOR set USTID=ESKIUSTID where USTID=-1',[],[]);
     Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update DOKUMAN set KLASOR=ESKIKLASOR where KLASOR=-1',[],[]);
  end else if TabKlasorler.FieldByName('USTID').AsInteger=-1 then begin
     Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update DOKUMANKLASOR set USTID=ESKIUSTID where ID='+TabKlasorler.FieldByName('ID').AsString,[],[]);
  end;}
  //önce bakalım altına alacağımız üst klasör hala var mı
  if TabKlasorler.FieldByName('ESKIUSTID').AsInteger<>0 then begin
     ...
     if Tablo.Query1.RecordCount > 0 then begin
        ...
        showmessage(Tablo.Query1.Fields[0].asstring+' klasörünün altına geri yüklendi..');
     end else
        Ust:=0;
  end
  else
...
{
 Evrak Klasörleri oluşturacaktır,
 ID     USTID   TUR   RESIM     AD              ...                 DURUM
-1001	-1000	1	7	Gelen Evrak                         1
-1101	-1001	2	5	Bütün Evraklar                      1
-1102	-1001	3	1	Üzerinde çalıştıklarım              1
-1103	-1001	4	12	Teslim almadıklarım                 1
-1104	-1001	5	11	Havale ettiklerim                   1
-1105	-1001	6	19	Dosyaya kaldırdıklarım              1
-1106	-1001	7	0	İptal ettiklerim                    1
-2001	-2000	8	8	Giden Evrak                         1
-2101	-2001	9	20	Bütün Evraklar                      1
-2102	-2001	10	1	Üzerinde çalıştıklarım              1
-2103	-2001	11	3	İmzada Bekleyenler                  1
-2104	-2001	12	18	Havale Ettiklerim                   1
-2105	-2001	13	19	Dosyaya kaldırdıklarım              1
-2106	-2001	14	13	İptal ettiklerim                    1
}
type
...
const
   constEvrakKlasor : array [1..15] of recEvrakKlasor=(
    ...
    (ID:-1102; USTID:-1001; TUR:3;  RESIM:1; AD:'Üzerinde çalıştıklarım'),
    (ID:-1103; USTID:-1001; TUR:4;  RESIM:12;AD:'Teslim almadıklarım'),
    (ID:-1104; USTID:-1001; TUR:5;  RESIM:11;AD:'Havale ettiklerim'),
    (ID:-1105; USTID:-1001; TUR:6;  RESIM:19;AD:'Dosyaya kaldırdıklarım'),
    (ID:-1106; USTID:-1001; TUR:7;  RESIM:0; AD:'İptal ettiklerim'),

    (ID:-2001; USTID:-2000; TUR:8;  RESIM:8; AD:'Giden Evrak'),
    (ID:-2101; USTID:-2001; TUR:9;  RESIM:20;AD:'Bütün Evraklar'),
    (ID:-2102; USTID:-2001; TUR:10; RESIM:1; AD:'Üzerinde çalıştıklarım'),
    (ID:-2103; USTID:-2001; TUR:11; RESIM:1; AD:'Gönderilen İşler'),
    (ID:-2104; USTID:-2001; TUR:12; RESIM:2; AD:'İmzada Bekleyenler'),
    (ID:-2105; USTID:-2001; TUR:13; RESIM:3; AD:'Havale Ettiklerim'),
    (ID:-2106; USTID:-2001; TUR:14; RESIM:4; AD:'DosyDosyaya kaldırdıklarım'),
    (ID:-2107; USTID:-2001; TUR:15; RESIM:5; AD:'İptal ettiklerim ')
   );
...
var
  ...
  insertSQL : string;
begin
     Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'SET IDENTITY_INSERT '+'DOKUMANKLASOR'+' ON',[],[]);
    try
      for i := 1 to High(constEvrakKlasor) do
       begin
         ...
         Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, insertSQL, [],[]);
       end;
    finally
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'SET IDENTITY_INSERT '+'DOKUMANKLASOR'+' OFF',[],[]);
    end;
...
//         SilmeParametre := SilmeParametre + ' ' + IntToStr(Tablo.Query4.RecordCount)+DYBelgeyi;
   ...
   if TabKlasorler.FieldByName('USTID').AsInteger=-1 then begin//çöp kutusu boşaltılır
...
//        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,RecursiveText+' select ID from DOKUMAN where KLASOR in(SELECT ID FROM REACH) ',[],[]);
//        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,RecursiveText+' select ID from IMAJ where YERI=1 and YER_ID in (select ID from DOKUMAN where KLASOR in(SELECT ID FROM REACH)) ',[],[]);
      TabloYenile(TabKlasorler,[]);
    end else begin //çöp kutusuna atılır
         Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,RecursiveText(TabKlasorler.FieldByName('ID').AsInteger, TabKlasorler.FieldByName('USTID').AsInteger)+
...
{  if TabKlasorler.FieldByName('ID').AsInteger = -1 then
    ...
    //  if Application.MessageBox(' Klasörünü boşalt', PChar(SGenotipOnay),MB_YESNO) = IDYES then
  if MessageDlg(TabKlasorler.FieldByName('AD').AsString +' Klasörünü boşalt',mtInformation,[MByes,Mbno],0) = mrYes then Begin
     Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,' update DOKUMAN set ESKIKLASOR=KLASOR , KLASOR=-1 where ID=(select top 1 DOKUMAN.ID from DOKUMANKLASOR  where DOKUMANKLASOR.ID = dokuman.klasor and DOKUMANKLASOR.ID=&DokID)',['&DokID'], [TabKlasorler.FieldByName('ID').AsInteger]);
     Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,' update DOKUMANKISAYOL set ESKIKLASOR=KLASOR , KLASOR=-1 where KLASOR=(select top 1 DOKUMANKLASOR.ID from DOKUMANKLASOR where DOKUMANKLASOR.ID = DOKUMANKISAYOL.KLASOR and DOKUMANKLASOR.ID=&DokID )', ['&DokID'], [TabKlasorler.FieldByName('ID').AsInteger]);
     TabloYenile(TabKlasorler,[]);
  end;  }
end;
...
//  if TabKlasorler.FieldByName('ID').AsInteger <> -1  then begin
    ...
    //KutuyuBosalt.Caption := (TabKlasorler.FieldByName('AD').AsString + ' Klasörünü Boşalt');
...
begin
  ...
  if TreeKlasorler.SelectionCount=1 then
     ...
     DokumanYetki.DokumanYetkiID:=0; //birden fazla seçim varsa klasör ID sıfır alırız
