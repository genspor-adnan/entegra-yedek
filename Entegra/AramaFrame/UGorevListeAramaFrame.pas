        ...
        if Bilgi <> '' then begin
            case Tur  of
              ...
              'I' : begin
                       ...
                       if Tablo.Query1.IsEmpty then
                          Memokontrol.Lines.Add('Tanımsız İlgili:'+Bilgi+'/'+VarToStr(sheet.cells[satir, c_Firma]))
                       end;
            end;
  ...
  if Tablo.OpenDialog1.Execute then begin
     ...
     Application.CreateForm(TBekletmeDlg,BekletmeDlg);
      ...
      for satir := 2 to excelsonsatir(1)+1 do begin
        ...
        Kontrol('L', VarToStr(sheet.cells[satir, c_ListeAdi])); // ListeAdı
        ...
        Kontrol('R', VarToStr(sheet.cells[satir, c_kod])+VarToStr(sheet.cells[satir,c_Firma])); // Müşteri
        Kontrol('I', VarToStr(sheet.cells[satir, c_Ilgili])); // İlgili
        Kontrol('P', VarToStr(sheet.cells[satir, c_Sube])); // Şube
      end;
      ...
      for satir := 2 to excelsonsatir(1)+1 do begin
        ...
        if VarToStr(sheet.cells[satir,1]) <> '' then begin
                ...
                if VarToStr(sheet.cells[satir, c_Atanan])<>'' then //atanan varsa
                   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, ' insert into [GOREVKULLANICI] ([LISTGOREVID],[TUR],[REHBERID],[EKLEYEN])values('+Tablo.Query5.fields[0].AsString+',11,'+
                      Kontrol('P', VarToStr(sheet.cells[satir, c_Atanan]))+','+Kullanan+') ',[],[]);
                if VarToStr(sheet.cells[satir, c_Notlar])<>'' then //notlar varsa
                   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'insert into [GOREVYORUM]([GOREVID],[TUR],[YORUM],[EKLEYEN])values('+Tablo.Query5.fields[0].AsString+',1,'''+StringReplace(VarToStr(sheet.cells[satir,c_Notlar]),'''','',[rfReplaceAll
                   ])+''','+Kullanan+')',[],[]);
                if VarToStr(sheet.cells[satir, c_Yorum])<>'' then //notlar varsa
                   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'insert into [GOREVYORUM]([GOREVID],[TUR],[YORUM],[EKLEYEN])values('+Tablo.Query5.fields[0].AsString+',2,'''+StringReplace(VarToStr(sheet.cells[satir,c_Yorum]),'''','',[rfReplaceAll
                   ])+''','+Kullanan+')',[],[]);
         end;
      end;
...
begin
   ...
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,' insert into [GOREVLER] ([LISTEID],[KONUSU],[BASLAMATARIHI],[BITISTARIHI],[REHBERID],[MUS_ILGILI],[MUS_ILGILI2],'+
...
begin
   if Veritabani.VeriVarMi(Tablo.FDCnn,'SELECT top 1 * FROM GOREVLER WHERE LISTEID='+TabListe.Fields[0].AsString,[],[]) then
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from GOREVLISTE where ID='+TabListe.Fields[0].AsString,[],[]);
   end;
   TabloYenile(TabListe,[StrToInt(Kullanan)]);
end;
...
begin
   ...
   //arama sekmesinde yeni butonu pasif olmalı
...
//adn                 TGorevListeDlg(AnaFrameYoneticisi.AktifFrame.IcerikFrameYoneticisi.AktifFrame.Ornek).GorevGridDBTableView1ID.Index];
    ...
    //Taşınan Node Id si liste drag over dan alınır
...
begin
  if State = dsDragLeave then begin //
     ...
     if not TreeHitTest.HitAtNode then begin
        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update GOREVLISTE set USTID = 0 where ID='+TabListe.Fields[0].AsString,[],[]);
        Listele;
     end;
  end
  else begin
...
begin
  Sender.BeginUpdate;
  try
    if (Nodes.Count = 1)and(PageListeler.ActivePageIndex=1) then begin  //Projeler açıksa
      dropId := TcxDBTreeListNode( AttachNode ).KeyValue;
      if dropId>0 then begin//node değil de proje adı üzerine bırakıldı
      ...
      if DragId>0 then begin
         ...
         Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'insert into [PROJEASAMA] ([PROJEID],[REHBERID],[ASAMA],[BASTAR],[AKTIF],[DURUM],[EKLEYEN])values('+
    ...
    else if Nodes.Count = 1 then begin  //Projeler kapalı ve liste diğer listenin altına gelecekse
      dropId := TcxDBTreeListNode( AttachNode ).KeyValue;
      dragId := TcxDBTreeListNode( Nodes[0] ).Values[0];
      if (DragId>0)and(dropId>0) then begin
         //Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update PROJELER set ASAMA='+IntToStr(abs(dropId))+' where ID='+IntToStr(dragId),[],[]);
         Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update GOREVLISTE set USTID = '+IntToStr(abs(dropId))+' where ID='+IntToStr(abs(dragId)),[],[]);
         Listele;
      end;
    end;
  finally
