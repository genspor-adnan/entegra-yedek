...
const
  // CİHAZLAR
  Nakit: Integer = 5; // NAKİTTOPLAM TUR
  Pos: Integer = 6; // POSTOPLAM TUR
  SatisTutar: Integer = 7; // SATIŞTOPLAMTUTAR TUR
  PosIslemleri: integer = 11; // POSISLEMLERI TUR
  // SAYIMLAR
  NakitSayim: Integer = 20; // NAKİTSAYIM TUR
  MultinetTicketSayim: integer = 30; // MULTINETTICKET TUR
  HediyeSayim: Integer = 40; // HEDİYE TUR
  IadeSayim: Integer = 50; // IADE TUR
  AcilisTutar: Integer = 0; // AÇILIŞ TUTAR TUR
  KapanisTutar: Integer = 100; // KAPANIŞ TUTAR TUR
  // İADE VE HEDİYECEKİ ID'LER
  HediyeID: Integer = -1; // HEDİYE ÇEKİ ID
...
begin
   ...
   begin
    ...
    TabSayimBilgisi.SQL.Text:= 'SELECT KD.KASAACKAPAT_ID,KD.HESAP_ID,''Nakit Tutar'' AS ADI,KD.TUTAR_SISTEM,KD.TUTAR_KAPANIS,KD.KUR' + #13#10 +
                              ...
                              'UNION' + #13#10 +
							                'SELECT KD.KASAACKAPAT_ID,KD.HESAP_ID,''Hediye Çeki Sayım Tutar'' AS ADI,KD.TUTAR_SISTEM,KD.TUTAR_KAPANIS,KD.KUR' + #13#10 +
                              'FROM dbo.KASAACKAPAT_DETAY KD' + #13#10 +
                              'WHERE KASAACKAPAT_ID=:ID3 AND KD.YER=40' + #13#10 +
                              'UNION' + #13#10 +
                              'SELECT KD.KASAACKAPAT_ID,KD.HESAP_ID,''İade Sayım Tutar'' AS ADI,KD.TUTAR_SISTEM,KD.TUTAR_KAPANIS,KD.KUR' + #13#10 +
                              'FROM dbo.KASAACKAPAT_DETAY KD' + #13#10 +
...
begin
  ...
  TabSayimBilgisi.SQL.Text:='IF  EXISTS (SELECT * FROM tempdb.sys.objects WHERE name=''##TEMPKUP'' AND type=''u'')' + #13#10 +
                            ...
                            'INSERT INTO ##TEMPKUP (ADI,TUTAR_KAPANIS,YER) VALUES (''Hediye Çeki Sayım Tutar'',0,40)' + #13#10 +
                            'INSERT INTO ##TEMPKUP (ADI,TUTAR_KAPANIS,YER) VALUES (''İade Sayım Tutar'',0,50)' + #13#10 +
