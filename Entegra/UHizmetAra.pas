...
var
  ...
  //// 0  : tüm stoklar , 1: secili depoya ait stoklar , 2: stok sayim ekranı
implementation
...
begin  //   HizmetAraCagiran 0 : giren fat, 1 çıkış fatura 2: sayım tutanağı 3 : demirbaş 4:transfer  5:teklif  6:servis
  ...
  if (Tablo.YetkiVarmi(MODUL_Stok,YetkiTur_Gorme)) and (cxPageControl1.ActivePageIndex = 1) then begin // Burada Stok Seçimi var
    ...
    if FiyatGoster then
     ...
     //   HizmetAraCagiran 0 : giren fat, 1 çıkış fatura 2: sayım tutanağı 3 : demirbaş
     //     4:transfer  5:teklif  6:servis   7 : iade alış fişi
     //tüm stoklar gelicek
      ...
      1, 4: //çıkış fat ve transfer
        begin // seçili depodaki stoklar gelecek
          QueryIslem.SQL.Text := ' Select distinct ' + TopAramaSayi + ' S.ID as URUNID, S.KOD, STOKADI AS AD, ' +
            ...
            '  ';
          if StokDurumKontrolKurali=0 then  //stok eksiye düşmesine izin yoksa stokdurumu 0 olanlar listeye gelmesin.
...
function UrunVarsaArttir : Boolean;
  begin
    Result:=False;
    //Eğer daha önce girilmişse onun sayısını artıralım
    if (cxPageControl1.ActivePage= shtStokAra) and   (HizmetAraCagiran in [1,2,4]) then begin //(not (TabFatBaslik.FieldByName('TUR').AsInteger in [10,11,12])) then  //stok çıkışı ise skt dahil olacak
      // stok çıkışı yapılıyorsa skt ye göre ve birime göre de arasın
...
begin
//   HizmetAraCagiran 0 : giren fat, 1 çıkış fatura 2: sayım tutanağı 3 : demirbaş 4:transfer  5:teklif  6:servis
//  7 : iade fiş
  if HizmetAraCagiran = 2 then
 {$REGION 'SAYIMTUTANAK KONTROLLERİ'}
  begin // sayım tutanağı ekranından çağrıldıysa
    if QueryIslem.FieldByName('IZLEME').AsInteger = 2 then // skt takibi
      if TGirisKutusuEx.BilgiAlEx(QueryIslem.FieldByName('AD').AsString + ' için SKT Bilgisi', TGirdiDenetimleri.Create.DateTimePicker('SKT Giriniz', @SKT)) <> mrOk then
        Abort;
...
{$ENDREGION}
     ...
     if cxPageControl1.ActivePage = shtHizmetAra then begin
        ...
        TabEkle := QueryIslem;
     if TabEkle.RecordCount < 1 then // 0 veya 1 ise fatura ekranından çağrılmış
     ...
     if HizmetAraCagiran = 7 then
      begin
        if  StrToFloat(Adet.Text)*Tablo.StokCarpan(QueryIslem.FieldByName('URUNID').AsInteger, comboBirim.EditValue ) > QueryIslem.FieldByName('ADET').AsFloat  then
          begin
            Application.MessageBox('Geri Almak istediğiniz ürün miktarı çıkılan miktardan fazla olamaz','U Y A R I',MB_OK+ MB_ICONWARNING);
            Abort;
          end;
      end;
   {$ENDREGION}
     if not( UrunVarsaArttir ) then begin
        ...
        //miktar hesaplaması için tür ve birim bilgisi mutlaka adetten önce atanmalı
        if cxPageControl1.ActivePage= shtStokAra then begin
           TabFatura.FieldByName('TUR').AsInteger := 1;//Stok işlemleri için tür 1 olmalı //TabEkle.FieldByName('TIPI').AsInteger;        // türün stok olduğunu belirtiyor
           if (comboBirim.EditValue<>TabEkle.FieldByName('ANABIRIM').AsInteger) and
              (comboBirim.EditValue<>TabEkle.FieldByName('BIRIM2').AsInteger )  then begin
               Application.MessageBox('Seçmiş olduğunuz birim bilgisi bu ürün için kullanılamaz','H A T A', MB_OK+MB_ICONERROR);
               TabFatura.Cancel;
               Abort;
           end;
        end;
        // miktar hesaplaması için adetten önce tür ve birim bilgisi mutlaka atanmış olmalı
        ...
        if (HizmetAraCagiran in [0,1]) and (cxPageControl1.ActivePage= shtStokAra)  then //giriş çıkış faturalarında izleme alanı dolacak
        ...
        if cxPageControl1.ActivePageIndex = 1 then begin
           if HizmetAraCagiran in [0,1,2,3,4,7] then begin
             ...
             if Tur in [0, 9, 10, 11, 12] then
               ...
               StokCikisIslemi;
             if (HizmetAraCagiran in [0,1,2,3,4,7])and(cbStokDepo.Enabled) then begin//demekki ilk kez çıkış yapılıyor; bunu kaydedelim bir daha depo adı depişemesin
        ...
        if ((s = '-1')or(s='') or (Tur in [9,10,11,12,13]))then begin // fiyat sorulacak
          if (TGirisKutusuEx.BilgiAlEx('Ürün Fiyatını Giriniz('+TabEkle.FieldByName('KUR').AsString+')', TGirdiDenetimleri.Create.CurrencyEdit('Fiyatını girin('+TabEkle.FieldByName('KUR').AsString+')', @Fiyat,Tablo.GENINI.ReadInteger(Ops_FaturaOpsiyon_OndalikDijitSayTut,2))) = mrOk) then
            s := Trim(Fiyat)
          else begin
            TabFatura.Cancel;
            Abort;
...
begin
  ...
  if comboBirim.Properties.Items.Count < 1 then
    ShowMessage('Sisteme birim tanımı girin (Adet vb..)')
  else
...
begin    //   HizmetAraCagiran 0 : giren fat, 1 çıkış fatura 2: sayım tutanağı 3 : demirbaş 4:transfer  5:teklif  6:servis
    // sayım tutanağı ekranı veya transfer ekranı formu cağırmışsa hizmet araması yapılmasın
