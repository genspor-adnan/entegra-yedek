...
//    if Sender.Etiketler.AsBoolean['DökümEkranı'] then
//    begin
...
begin
  ...
  with FFrameBilgi.IcerikFrameYoneticisi.AktifFrame do begin
    ...
    if Etiketler.AsBoolean['DökümEkranı'] then
...
begin
   ...
   if Ekranadi = 'DokumDlg' then begin
     ...
     Ver := TDokumDlg(Dlg).TabDokum.AsString['VERSIYON'];
     dlgSave.FileName := kaynakDokum + '.frd'; //döküm ayarları
     dlgSave.Filter := '.frd';
     RaporId := TDokumDlg(Dlg).TabDokum.AsInteger['ID'];
   end else begin                                                                  //sayfayı kaydetme
     ...
     Delete(kaynakDokum, pos('&',kaynakDokum), 1);
     dlgSave.Filter := '.frs';
     dlgSave.FileName := kaynakDokum+'.frs';//sayfa ayarları
...
begin
   ...
   //eski yazdırma menülerini silelim
...
begin
  ...
  if TGirisKutusuEx.BilgiAlEx(BGYeni_rapor, TGirdiDenetimleri.Create.Edit(BGYeni_ad,@dokumAdi)) = mrOk then begin
    ...
    //    YazdirmaBilgileriniYenile(FFrameBilgi.AktifIcerik);
        ...
        //TB := TToolButton(FFrameBilgi.AktifIcerik.Ornek.FindComponent('YaziciYaz'));  yerine aşağıdaki yapıldı 12/2/2011 ao
        TB :=TToolButton( TPopupMenu(TMenuItem(sender).GetParentComponent).PopupComponent);
    //    TPopupMenu(FFrameBilgi.AktifIcerik.Ornek.FindComponent('PopupMenuYaz')).Items.ItemOperation(moAdd, TB.Caption, RaporSecClick); yerine aşağıdaki yapıldı
...
//    TJvPanel(Sender).Caption := 'Gezinme ve Yazdırma';
