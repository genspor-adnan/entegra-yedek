...
begin
  //sender.tag 9 ise verilen 19 ise alınan sipariş..
  //sender.hint + ise ekleme - ise çıkartma
end;
...
var
  ...
  i:Integer;
begin
  //planlama içeriğine girecek siparişlerin hangileri olacağı belirlenir.. alınan ve verilen siparişler..
  AlinanSip := Tablo.ListedenCokluSecim('Üretim Planına Dahil Edilecek Alınan Siparişler',MemoSiparisAra.Lines.Text+' and S.TUR=19 ' //and S.CIKISDEPO='+IntToStr(DepoID)
                                        ,[nil,nil,nil,nil,Tablo.RepSubelerOrtakTumSubeler,nil,nil,nil,Tablo.repStokOzellik,Tablo.repStokTipi,nil,nil]
                                        ,['ID','Tarih','Seri','No','Şube','Firma','Kod','Stok','Özellik','Tip','Miktar','Reçete']);
  AlinanStr := '0';
  for I := 0 to AlinanSip.Count - 1 do
    AlinanStr := AlinanStr + ',' + AlinanSip[i];

  VarilenSip := Tablo.ListedenCokluSecim('Üretim Planına Dahil Edilecek Verilen Siparişler',MemoSiparisAra.Lines.Text+' and S.TUR=9 and S.GIRISDEPO='+VarToStr(FArama.cbDepo.EditValue)
                                        ,[nil,nil,nil,nil,Tablo.RepSubelerOrtakTumSubeler,nil,nil,nil,Tablo.repStokOzellik,Tablo.repStokTipi,nil,nil]
                                        ,['ID','Tarih','Seri','No','Şube','Firma','Kod','Stok','Özellik','Tip','Miktar','Reçete']);
  VarilenStr:= '0';
  ...
  //olan satırlar için miktar güncellemesi yapalım.. olmayanlar için de insert yapmamız gerekecek..
  Tablo.Query1.Close;
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
    if Veritabani.VeriVarMi(Tablo.FDCnn,'select 1 from FATURA where URETIMPLANDETAYID=&UPID',['&UPID'],[TabUretimPlanlama.FieldByName('ID').AsInteger]) then
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from URETIMOPERASYON where URETIMPLANDETAYID = &UPID',['&UPID'],[TabUretimPlanlama.FieldByName('ID').AsInteger]);
      TabloYenile(FArama.TabPlanlar,[FArama.cbDepo.EditValue]);
      AramaYap(nil);
    end;
  end;
...
begin
  try
    ...
    BekletDlg.Caption := 'Üretim Emirleri Oluşturuluyor...';
    BekletDlg.LabelUstTaraf.Caption := 'Güncellemeler Yapılıyor.';
    BekletDlg.Show;
    //buradan sonra listemizi açıp gerekli üretimi 0 dan büyük olan tüm satırlar için üretim emri oluşturmamız gerekiyor..
    Tablo.Query8.Close;
    ...
    while not Tablo.Query8.Eof do begin
      ...
      if (ReceteID>0)and(Miktar>0.0) then begin
        ...
        //buradan bilgilerin gelip gelmediğini kontrol edeceğiz..
        if (Tablo.Query1.RecordCount>0)and(Tablo.Query2.RecordCount>0)and(Tablo.Query3.RecordCount>0)then begin
          //reçete içerisindeki ürün birimine ve adedine bakılması gerekiyor.. reçetede üretilecek ürünün satırı olup olmadığı da kontrol edilmiş oluyor..
          if Tablo.Query2.Locate('TUR;URUNID',VarArrayOf([1,Tablo.Query3.FieldByName('ID').AsInteger]),[]) then begin
            //başlık bilgilerini kaydedelim..
            Tablo.Query7.Close;
            ...
            //ilk satırı ekledikten sonra döngüye sokabiliriz.. önce ilk satırı ekleyelim..
            Seviye := 0;
            Tablo.Query4.Close;
            ...
            while Seviye < 20 do begin
              //ilgili seviyenin(başlangıç için 0) alt reçeteleri bulunur... sadece bu üretime girecek olan stok bileşenlerin reçeteleri aranır..
              ...
              //her alt reçete için reçete içeriği bir sonraki seviyeye insert edilir...
              if Tablo.Query1.RecordCount>0 then begin
                Tablo.Query1.First;
                while not Tablo.Query1.Eof do begin
                  //birim kontrolleri ve çarpan hesaplaması..
                ...
                Seviye := 20;//artık daha fazla detay gelmemeye başlıyor..
              end;
              Inc(Seviye);
            end;
...
begin
  ...
  if Tablo.Query4.RecordCount>0 then begin
    ...
    while not Tablo.Query4.Eof do begin
      //Operasyondan ürün ağacındaki ilgili kayda locate olunur.. üretim ağaca göre yapılacak..
      ...
      and TabUretimEmriDetay.Locate('ID',Tablo.Query4.FieldByName('URETIMEMRIDETAYID').AsInteger,[]) then begin
        ...
        showmessage('Operasyon üretim ile eşleşmedi!!');
      Tablo.Query4.Next;
    end;
  end else
    showmessage('Aktarılacak kayıt bulunamadı.');
  AramaYap(nil);
...
begin
  try
    ...
    Application.CreateForm(TBekletmeDlg, BekletDlg);
    BekletDlg.cxProgressBar1.Position := 0;
    BekletDlg.Caption := 'Üretim Operasyonları Oluşturuluyor...';
    BekletDlg.LabelUstTaraf.Caption := 'Güncellemeler Yapılıyor.';
    BekletDlg.Show;
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
    if Veritabani.VeriVarMi(Tablo.FDCnn,'select 1 from URETIMEMRI where URETIMPLANDETAYID=&UPID',['&UPID'],[TabUretimPlanlama.FieldByName('ID').AsInteger]) then
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update SIPARISDETAY set URETIMPLANID=null,URETIMPLANDETAYID=null where URETIMPLANDETAYID=&UPID',['&UPID'],[TabUretimPlanlama.FieldByName('ID').AsInteger]);
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from URETIMPLANLAMADETAY where ID=&UPID',['&UPID'],[TabUretimPlanlama.FieldByName('ID').AsInteger]);
      AramaYap(nil);
    end;
  end;
  TabloYenile(FArama.TabPlanlar,[FArama.cbDepo.EditValue]);
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
    if Veritabani.VeriVarMi(Tablo.FDCnn,'select 1 from URETIMEMRI where URETIMPLANID=&UPID',['&UPID'],[FArama.TabPlanlar.FieldByName('ID').AsInteger]) then
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update SIPARISDETAY set URETIMPLANID=null,URETIMPLANDETAYID=null where URETIMPLANID=&UPID',['&UPID'],[FArama.TabPlanlar.FieldByName('ID').AsInteger]);
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from URETIMPLANLAMA where ID=&UPID',['&UPID'],[FArama.TabPlanlar.FieldByName('ID').AsInteger]);
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from URETIMPLANLAMADETAY where URETIMPLANLAMAID=&UPID',['&UPID'],[FArama.TabPlanlar.FieldByName('ID').AsInteger]);
      TabloYenile(FArama.TabPlanlar,[FArama.cbDepo.EditValue]);
      AramaYap(nil);
    end;
  end;
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
    if Veritabani.VeriVarMi(Tablo.FDCnn,'select 1 from FATURA where URETIMPLANID=&UPID',['&UPID'],[FArama.TabPlanlar.FieldByName('ID').AsInteger]) then
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from URETIMOPERASYON where URETIMPLANID = &UPID',['&UPID'],[FArama.TabPlanlar.FieldByName('ID').AsInteger]);
...
begin
  ...
  if AlinanSip.Count>0 then begin
    ...
    for I := 0 to AlinanSip.Count - 1 do
      AlinanStr := AlinanStr + ',' + AlinanSip[i];
    VarilenSip := Tablo.ListedenCokluSecim('Üretim Planına Dahil Edilecek Verilen Siparişler',MemoSiparisAra.Lines.Text+' and S.TUR=9 and S.GIRISDEPO='+IntToStr(DepoID)
                                          ,[nil,nil,nil,nil,Tablo.RepSubelerOrtakTumSubeler,nil,nil,nil,Tablo.repStokOzellik,Tablo.repStokTipi,nil,nil]
                                          ,['ID','Tarih','Seri','No','Şube','Firma','Kod','Stok','Özellik','Tip','Miktar','Reçete']);
    VarilenStr:= '0';
    if VarilenSip.Count>0 then begin
