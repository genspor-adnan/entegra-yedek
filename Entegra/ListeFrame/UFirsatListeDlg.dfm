object FirsatListeDlg: TFirsatListeDlg
  Left = 0
  Top = 0
  Width = 1155
  Height = 531
  ...
  object SQLMemo: TcxMemo
    ...
    Lines.Strings = (
      ...
      'select '
        ...
        '    P.ID,LOKAL_FIRMA_ID,P.REHBERID,PROJEKODU,BASLAMATARIHI,BITIS' +
        'TARIHI,KONUSU,TURU,P.DURUM,ASAMA,PRJ_SORUMLUSU_ID,PRJ_ASAMA_SORU' +
        'MLUSU_ID,'
      
        #9'ILGILI,LISTEFIYATI,LISTEKUR,P.SATISFIYATI,P.SATISKUR,P.EKLEYEN,' +
        'P.EKLEMETARIHI,P.DEGISTIREN,P.DEGISTIRMETARIHI,'
      
        #9'SONUC,SONUCACIKLAMA,DETAYBOLUMU,TIPI,APLIKASYON,PROJEADI,OLASIL' +
        'IK,'
      
        #9'P.SUBEID, GOOGLEOLAYID,GOOGLEHESAPID,CARIID,SEBEBI,KAYIPFIYATI,' +
        'KAYIPKUR,P.GIRISKAYNAK,RAKIP,'
      #9'ProjeID = P.ID,'
      
        #9'NOTLAR=(Select top 1 GY.YORUM from GOREVYORUM GY where P.ID=GY.' +
        'GOREVID and GY.TUR=70 order by GY.TARIH desc),'
      ''
      #9'ALANRAKIP=(select FIRMA from REHBER RR where P.CARIID=RR.ID),'
      
        #9'FIRMA= CASE WHEN LEN(LTRIM(RTRIM(R1.FIRMA)))-LEN(REPLACE(LTRIM(' +
        'RTRIM(R1.FIRMA)),'#39' '#39','#39#39'))<2 THEN R1.FIRMA ELSE SUBSTRING(R1.FIRM' +
        'A, 0, '
      'CHARINDEX('#39' '#39', R1.FIRMA, CHARINDEX('#39' '#39', R1.FIRMA, 0)+1)) END,'
      ...
      ''
        ...
        '      DOSYAVAR = CASE WHEN  (Select top 1 COUNT(ID) from GOREVYO' +
        'RUM GY where P.ID=GY.GOREVID and GY.TUR=70 and GOREVID=1 )>0 the' +
        'n 1 else 0 end,'
      
        '    SONAKTIVITEKONUSU = (SELECT TOP 1 KONUSU FROM GOREVLER A WHE' +
        'RE PROJEID = P.ID and ACKAPA=1 ORDER BY BITISTARIHI DESC ),'
      
        '    SONAKTIVITETARIHI =  convert(DateTime,(SELECT TOP 1 BITISTAR' +
        'IHI FROM GOREVLER A WHERE PROJEID = P.ID and ACKAPA=1 ORDER BY B' +
        'ITISTARIHI '
      'DESC),103),'
      
        '    SONSATBELGETARIHI =   convert(DateTime,(SELECT TOP 1 TARIH F' +
        'ROM FATBASLIK F WHERE F.PROJEID = P.ID AND'
      
        '       F.TUR  in (10,11,12,14,15,16)  ORDER BY TARIH DESC ),103)' +
        ','
      
        '    SONSATTUTARI = (SELECT TOP 1 FATURA_TUTARI FROM FATBASLIK F ' +
        'WHERE F.PROJEID = P.ID AND'
      '       F.TUR  in (10,11,12,14,15,16)  ORDER BY TARIH DESC ),'
      
        '    SONTEKLIFDURUMU=(select top 1 DURUM from TEKLIF T where T.PR' +
        'OJEID = P.ID order by TARIH desc),'
      
        '    SONTEKLIFTUTARI=(select top 1 TEKLIF_MATRAHI from TEKLIF T w' +
        'here T.PROJEID = P.ID order by TARIH desc),'
      
        '    SONTEKLIFKUR=(select top 1 KUR from TEKLIF T where T.PROJEID' +
        ' = P.ID order by TARIH desc),'
      
        '    SONTEKLIFTARIHI=convert(DateTime,(select top 1 TARIH from TE' +
        'KLIF T where T.PROJEID = P.ID order by TARIH desc),103),'
      'P.*'
      'from PROJELER P'
      #9'inner join REHBER R1 on R1.ID=P.REHBERID'
      #9'left outer join REHBER RP on RP.ID=P.ILGILI'
      
        '    LEFT OUTER JOIN GENINI ProjeTipi ON ProjeTipi.DIL=-1 and Pro' +
        'jeTipi.DEGER = P.TIPI AND ProjeTipi.BOLUM=convert(int,'#39'-2112'#39'+co' +
        'nvert(varchar(10),P.TURU))'
      'WHERE'
      'P.MODUL=1'
      ''
      ''
    ...
    object PageControlSekme: TcxPageControl
      ...
      object TabSheetTeklifler: TcxTabSheet
        ...
        object GridTeklif: TcxGrid
          ...
          object GridTeklifDBTableView1: TcxGridDBTableView
            Navigator.Buttons.CustomButtons = <>
            ScrollbarAnnotations.CustomAnnotations = <>
            DataController.DataModeController.SmartRefresh = True
            DataController.DataSource = DtsTeklifDetay
            DataController.DetailKeyFieldNames = 'TEKLIFID'
            DataController.KeyFieldNames = 'ID'
          ...
          object GridTeklifLevel1: TcxGridLevel
            ...
            object GridTeklifLevel2: TcxGridLevel
              GridView = GridTeklifDBTableView1
            end
          end
        end
    ...
    object TabSheetListe: TcxTabSheet
      ...
      object GridFirsat: TcxGrid
        ...
        object GridFirsatView: TcxGridDBTableView
          ...
          Navigator.Buttons.CustomButtons = <>
          ScrollbarAnnotations.CustomAnnotations = <>
          OnCanFocusRecord = GridFirsatViewCanFocusRecord
          OnSelectionChanged = GridFirsatViewSelectionChanged
          DataController.DataModeController.SmartRefresh = True
          ...
          DataController.Summary.DefaultGroupSummaryItems = <
            ...
            item
              ...
              Column = GridFirsatViewSATISFIYATI
            end>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = '###,###,##0.00'
            ...
            item
              Kind = skCount
              FieldName = 'FIRMA'
              Column = GridFirsatViewFIRMA
            end
            item
              Format = ',0.00;-,0.00'
              Kind = skSum
              Column = GridFirsatViewSONTEKLIFTUTARI
            end>
          DataController.Summary.SummaryGroups = <>
          ...
          Styles.OnGetContentStyle = GridFirsatViewStylesGetContentStyle
  ...
  object FIRSATLAR: TFDQuery
    ...
    SQL.Strings = (
      ...
      'select '
      #9'P.*,'
      #9'RAKIP=(select FIRMA from REHBER RR where P.CARIID=RR.ID),'
  ...
  object TabTeklif: TFDQuery
    ...
    SQL.Strings = (
      ...
      'RP.ADSOYAD as MUS_ILGILIAD,'
        ...
        'where YERID in (select ID from TEKLIFDETAY where TEKLIFID=T.ID))' +
        ' then '#39'Var'#39' else '#39#39' end ,   '
      'TEKLIFGUNSAYISI=DATEDIFF(day,TARIH,GETDATE()),'
      'DURUMGUNSAYISI=DATEDIFF(day,DURUMTARIHI,GETDATE()),'
      
        'TESLIMTARIHI =(Select Min(TESLIMTARIHI) from TEKLIFDETAY Where T' +
        'EKLIFID=T.ID ),'
      
        'SONONAYLAYAN =(Select top 1  R.FIRMA from TEKLIFHAREKET TH  left' +
        ' outer join REHBER R on T.ONAYLAYAN=R.ID '
      '  Where  TH.TEKLIFID=T.ID Order by TH.EKLEMETARIHI desc )'
      'from TEKLIF T'
  ...
  object TabTeklifDetay: TFDQuery
    ...
    SQL.Strings = (
      ...
      'Where'
      'TEKLIFID IN (select ID from TEKLIF WHERE PROJEID=:Prm)'
      'and ALTERNATIFNO=1'
      'ORDER BY PROJEID'
      '')
    Left = 396
