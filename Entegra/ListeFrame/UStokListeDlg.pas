...
begin
  ...
  try
    if Tablo.ListedenBilgiGetir('Reçetesini Kopyalayacağınız Stoku Seçiniz.','select R.ID,R.STOKID,S.KOD,S.STOKADI from STOKLAR S inner join URETIMRECETE R on S.ID=R.STOKID',str,[],'') then begin
...
begin
   ...
   if (ComboDepo.EditValue<>null)and(ComboDepo.EditValue>0)and
       (TGirisKutusuEx.BilgiAlEx(ComboDepo.Text+' deposu için başlama tarihini girin' ,
...
//        Trh := StringReplace( VarToStr(Trh),'/', FormatSettings.DateSeparator,[rfReplaceAll]);
        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,' exec P_StokMaliyetGuncelleORT '+STOKLAR.FieldByName('ID').AsString+','+IntToStr(ComboDepo.EditValue)+','''+FormatDateTime('yyyy-dd-mm 00:00', TDateTime(Tarih))+''' ',[],[]);
...
var
  ...
  Ek: string[10];
begin
  if {Değiştirme / resim ekleme yetkisi var mı DYetkisonuc.Ekle =} True then begin
