...
begin
   if Veritabani.VeriVarMi(Tablo.FDCnn, 'select ANAHTAR from GENINI where BOLUM=&Bolum and ANAHTAR=&Anah and DIL=-1',['&Bolum', '&Anah'],
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, ' insert into GENINI (BOLUM,ANAHTAR,DEGER,DIL,SIRA) values(&BOLUM,&ANAHTAR,&DEGER,&DIL,&SIRA) ',
...
begin
   if Application.MessageBox(PChar(STOKLAR.FieldByName(STstok_adi).AsString+STListeden_cik), PChar(SGenotipOnay), MB_YESNO) = IDYES then
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, ' delete from GENINI where BOLUM='+IntToStr(TMenuItem(Sender).Tag)+' and ANAHTAR = &Kod and DIL=-1 ', ['&Kod'], [STOKLAR.FieldByName('KOD').AsString]);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      EkipmanID := Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'INSERT INTO EKIPMANLAR (URUNID, EKIPMANTUR,'+
...
begin
  ...
  try
    if Tablo.ListedenBilgiGetir('Re�etesini Kopyalayaca��n�z Stok�u Se�iniz.','select R.ID,R.STOKID,S.KOD,S.STOKADI from STOKLAR S inner join URETIMRECETE R on S.ID=R.STOKID',str,[],'') then begin
      ...
      while not Tablo.Query4.Eof do begin
        if (Tablo.Query4.FieldByName('TUR').AsInteger=1)and(Tablo.Query4.FieldByName('URUNID').AsInteger=Tablo.Query3.FieldByName('STOKID').AsInteger) then begin
          ...
          Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update URETIMRECETEDETAY set URETIMRECETEID=&URID, URUNID=&UrunID, BIRIM=&Brm where ID=&URDID',['&URID','&UrunID','&Brm','&URDID'],[Tablo.Query5.Fields[0].AsInteger,STOKLAR.FieldByName('ID').AsInteger,STOKLAR.FieldByName('ANABIRIM').AsInteger,YeniID]);

        end else begin
          YeniID := Tablo.SatirKopyala('URETIMRECETEDETAY',Tablo.Query4.FieldByName('ID').AsInteger);
          Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update URETIMRECETEDETAY set URETIMRECETEID=&URID where ID=&URDID',['&URID','&URDID'],[Tablo.Query5.Fields[0].AsInteger,YeniID]);
        end;
        Tablo.Query4.Next;
      end;
    end;
...
begin
   ...
   if (ComboDepo.EditValue<>null)and(ComboDepo.EditValue>0)and
       (TGirisKutusuEx.BilgiAlEx(ComboDepo.Text+' deposu i�in ba�lama tarihini girin' ,
...
//        Trh := StringReplace( VarToStr(Trh),'/', FormatSettings.DateSeparator,[rfReplaceAll]);
        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'P_StokMaliyetGuncelleORT '+STOKLAR.FieldByName('ID').AsString+','+IntToStr(ComboDepo.EditValue)+','''+FormatDateTime('yyyy-dd-mm 00:00', TDateTime(Tarih))+''' ',[],[]);
...
begin
   ...
   if (ID>0)and(TabYorum.FieldByName('DOKUMANID').AsString<>'') then
       Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update DOKUMAN set KONU='''+STOKLAR.FieldByName('KOD').AsString+' / '+STOKLAR.FieldByName('STOKADI').AsString+''' where ID='+TabYorum.FieldByName('DOKUMANID').AsString,[],[]);
end;

procedure TStokListeDlg.Btnrnlerinstokdurumlarngncelle1Click(Sender: TObject);
begin
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'truncate table STOKDURUM ',[],[]);
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'EXEC SP_Prg_GenelStokDuruMGuncelle 0,'''+FormatDateTime('yyyy-01-01 00:00',Tablo.GenIni.BugunTrh)+''','''+FormatDateTime('yyyy-12-31 23:59',Tablo.GenIni.BugunTrh)+''' ',[],[]);
   Application.ProcessMessages;
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'EXEC SP_Prg_GenelStokIzlemDuruMGuncelle 0,'''+FormatDateTime('yyyy-01-01 00:00',Tablo.GenIni.BugunTrh)+''','''+FormatDateTime('yyyy-12-31 23:59',Tablo.GenIni.BugunTrh)+''' ',[],[]);
   Application.ProcessMessages;
   StokDurumGetir;
   ShowMessage(Guncellendi);
end;
...
begin
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'Delete from STOKDURUM Where STOKID='+STOKLAR.FieldByName('ID').AsString,[],[]);
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'EXEC SP_Prg_GenelStokDuruMGuncelle '+STOKLAR.FieldByName('ID').AsString+','''+FormatDateTime('yyyy-01-01 00:00',Tablo.GenIni.BugunTrh)+''','''+FormatDateTime('yyyy-12-31 23:59',Tablo.GenIni.BugunTrh)+''' ',[],[]);
   StokDurumGetir;
   ShowMessage(Guncellendi);
end;
...
begin
  ...
  sql:=' SELECT S.ID,S.KOD as Kod,S.STOKADI as [Stok Ad�],S.TIPI AS [Tip],S.MARKA AS Marka,'+
  ' StokModel.ANAHTAR AS Model, S.GRUBU AS Grubu,S.OZELLIK AS [�zellik],S.IZLEME AS [�zleme] FROM'+
  ' STOKLAR AS S '+
...
begin
 if stokekranilkacilis then
  ...
  UretimIslemleriMenu.Visible := Tablo.YetkiVarmi(3321,YetkiTur_Gorme); //�retim g�rev framedeki re�ete butonunu g�rme yetkisi..
...
begin
  ...
  begin
    ...
    begin
      if VeriVarmi then
      begin
        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE STOKSEVIYE SET MAKSIMUM=&MAKSIMUM, KRITIK=&KRITIK, MINIMUM=&MINIMUM, '+
      ...
      begin
          Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'INSERT INTO STOKSEVIYE (STOKID, DEPOID, MAKSIMUM, KRITIK, MINIMUM, EKLEYEN, EKLEMETARIHI)'+
...
begin
  if GridStokView.Controller.SelectedRecordCount>0 then begin
    Brkd := -99;
    Brm := -99;
    ctrls := TGirdiDenetimleri.Create.ImageComboBox('Barkod Tipi Se�imi',@Brkd,Tablo.FDCnn,'select ID,AD from BARKODAYARLAR ')
                                     .ImageComboBox('Birim Se�imi',@Brm,Tablo.FDCnn,'select 0,''Anabirim'' union all select 1,''Birim 2''  ');
        ...
        for I := 0 to GridStokView.Controller.SelectedRecordCount-1 do begin
          ...
          if StkID>0 then begin
            ...
            if Brm=0 then
              ...
              BirimTuru := 'BIRIM2';
            VeriTabani.BasitKomutÇalıştır(Tablo.FDCnn,'insert into STOKBARKOD(BARKOD,STOKID,BARKODTIPI,BARKODBIRIMI,VARSAYILAN)'+
...
begin
  Tablo.TablodanSorguAc(5,'insert into URETIMRECETE(KOD,AD,STOKID)values('''+STOKLAR.FieldByName('KOD').AsString+''','''+STOKLAR.FieldByName('STOKADI').AsString+''','+STOKLAR.FieldByName('ID').AsString+') select scope_identity()');
  Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'insert into URETIMRECETEDETAY(URETIMRECETEID,TUR,URUNID,ADET,BIRIM,MIKTAR,ADETHESAP)select &URID,1,ID,1,ANABIRIM,1,1 from STOKLAR where ID=&StokID',['&URID','&StokID'],[Tablo.Query5.Fields[0].AsInteger,STOKLAR.Fields[0].AsInteger]);
  Tablo.ReceteSihirbazBaslat(Tablo.Query5.Fields[0].AsInteger,'D');
  RecetePopupDuzenle;
end;
...
var
  ...
  Ek: string[10];
begin
  if {De�i�tirme / resim ekleme yetkisi var m� DYetkisonuc.Ekle =} True then begin
