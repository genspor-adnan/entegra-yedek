...
begin
  ...
  if Potansiyel then begin
     IKGridViewGIRISTARIHI.Caption:='Ba�vuru Tarihi';
     TabloNo := TabNo_IK_POTANSIYEL;
  end else begin
     IKGridViewGIRISTARIHI.Caption:='Giri� Tarihi';
     TabloNo:=TabNo_IK;
  end;
  Rehber.Close;
...
procedure TIKListeDlg.BtnDuzenlePerizinClick(Sender: TObject);
  procedure ilkSatirinsert(Miktar :Real;Tarih:TdateTime;Aciklama,Donem:string;Iz�nID:Integer);
  var
    Tur:Integer;
  ...
  begin
    ...
    Tur := Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'SELECT IZINTURU FROM PERSONELIZIN WHERE ID=&ID',['&ID'],[Iz�nID],True);
    ...
    Tablo.Query6.SQL.Text := 'UPDATE PERSONELIZIN SET  IZINLIGUNSAYISI=:IZINLIGUNSAYISI, IZINBASLANGIC=:IZINBASLANGIC, ACIKLAMA=:ACIKLAMA,'+
                             ...
                             'DEGISTIRMETARIHI=:DEGISTIRMETARIHI WHERE ID='+IntToStr(Iz�nID);
    izinliGunSayisi := 0;
...
begin
  ...
  begin
    ...
    ctrls := TGirdiDenetimleri.Create.Edit(Tipi+' D�nem',@Donemx).DateTimePicker(BGIzin_tarih_gir,@izinTarihi).Edit(Tipi+'BGGun_miktari_gir',@Hakedilen).Edit('A��klama',@Aciklama);
...
begin                              // PChar(SSilmeSorusu)
   ...
   if Application.MessageBox(PChar(stringgrid1.Cells[0,StringGrid1.Row]+' listeden ��kar�lacak..'), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
...
begin
  ...
  { Vardiya T�r� eklenirken izlenecek yol

  //RehberAyar tablosuna eklenecek bilgiler.
   Etiket = 'Vardiya T�r�'
   Kaynak=Select '' as ANAHTAR,3 as DEGER  union Select 'Sabit',1 union Select 'De�i�ken',2
  }
end;
...
//   Tablo.TablodanSorguAc(1,'select GIRISTARIHI from REHBER where ID='+IntToStr(RehberId));
  ////i�e giri� tarihi bo� ise girilmesi istenir.
   Gun := 0;
   if Tablo.Query1.IsEmpty then
      showmessage(Tablo.AciklamaGetir('REHBER','FIRMA', RehberId)+' '+IKEksik_bilgi)
   else begin
     IseGiris:= Tablo.Query1.Fields[0].AsDateTime;
     IzinTarihi := StrToDate(FormatDateTime('dd'+FormatSettings.DateSeparator+'mm'+FormatSettings.DateSeparator, IseGiris)+IntToStr(BuYil));//i�e giri�ing�n ve ay�n� alal�m
...
begin
  ...
  begin
    ...
    Tablo.Query6.ParamByName('BIRIM').Value := 3; //g�n
    izinliGunSayisi := 0;
    Tablo.Query6.ParamByName('ACIKLAMA').Value := Aciklama;
    if Tur = -1 then begin
     Tablo.Query6.ParamByName('HAK').Value := Miktar;
...
//   Tablo.TablodanSorguAc(1,'select GIRISTARIHI from REHBER where ID='+REHBER.FieldByName('ID').AsString);
   ...
   izinTurBirim := 3;    //Varsay�lan olarak 'G�n' izin t�r� birimi geliyor
   //Miktar := 0;
  ////i�e giri� tarihi bo� ise girilmesi istenir.
  /// Yeniye t�kland���nda Popup button olacak orada izin t�rleri se�ilecek ve e�er y�ll�k izin ise Hak edilen miktar girilcek.
  /// yok e�er di�er izinlerden ise tek sat�r eklenecek.
   ...
   if izinTur=-1 then begin
    ...
    ctrls := TGirdiDenetimleri.Create.edit(Tipi+' D�nemi',@Donemx).DateTimePicker('�zin Tarihini Giriniz',@izinTarihi).edit(Tipi+' G�n Miktar�n� Giriniz',@Hakedilen).Edit('A��klama',@Aciklama);
    if TGirisKutusuEx.BilgiAlEx(Tipi+' �zin Bilgileri (��e Giri�:'+FormatDateTime('dd'+FormatSettings.DateSeparator+'mm'+FormatSettings.DateSeparator+'yyyy', IseGiris)+')', ctrls) = mrOk then begin
       Hakedilen:=StringReplace(VarToStr(Hakedilen),',',FormatSettings.Decimalseparator,[rfReplaceAll]);
       Hakedilen:=StringReplace(VarToStr(Hakedilen),'.',FormatSettings.Decimalseparator,[rfReplaceAll]);
       Donemx := IntToStr(StrToIntDef( VarToStr(Donemx),1));
...
var Grup : Variant;
    ID : integer;
begin
    if TGirisKutusuEx.BilgiAlEx(BGYeni,TGirdiDenetimleri.Create.Edit('Grup Ad�',@Grup)) = mrOk then begin
...
begin
  ...
  Tablo.Query7.SQL.Text :=
    ...
    ' union all ' +
    ' select  STR=''Proje - A�ama Sorumlusu'',SAYI=count(*) from PROJEASAMA where DURUM<>2 and REHBERID = @rehID ' +
    ' union all ' +
    ' select  STR=''�� Listesi - Atayan'',SAYI=count(*) from GOREVLER where ACKAPA=0 and EKLEYEN = @rehID ' +
    ' union all ' +
    ' select  STR=''�� Listesi - Atanan'',SAYI=count(*) from GOREVKULLANICI where ( select distinct ACKAPA from GOREVLER G where G.ID=GOREVKULLANICI.LISTGOREVID)=0 and REHBERID = @rehID and TUR=11 '+
    ' union all ' +
    ' select  STR=''�� Listesi - Bilgilendirilecek'',SAYI=count(*) from GOREVKULLANICI where ( select distinct ACKAPA from GOREVLER G where G.ID=GOREVKULLANICI.LISTGOREVID)=0 and REHBERID = @rehID and TUR=12 '+
    ' union all ' +
    ' select  STR=''Teklif - Haz�rlayan'',SAYI=count(*) from TEKLIF where DURUM not in (3,6,7,8) and HAZIRLAYAN = @rehID ' +
    ' union all ' +
  ...
  Tablo.Query7.First;
  ShowWarning := False;
  WarningStr := '��ten ��kartmakta bulundu�unuz personel daha �nce;';
  while not Tablo.Query7.Eof do begin
    if Tablo.Query7.Fields[1].AsInteger > 0 then begin
      ShowWarning := True;
      WarningStr := WarningStr + #13#10 + Tablo.Query7.Fields[0].AsString + ' olarak ' + Tablo.Query7.Fields[1].AsString +' farkl� yerdeki';
    end;
    Tablo.Query7.Next;
  end;
  if ShowWarning then begin
    WarningStr := WarningStr + #13#10 + 'kay�tlarda kullan�lm��t�r.';
    WarningStr := WarningStr + #13#10 + '�lgili i�leri ba�ka bir personele aktarmak ister misiniz? ';
...
begin
  ...
  if (Sender as TMenuItem).Tag in [1,99] then begin//proje
    if (Sender as TMenuItem).Tag <> 99 then begin
      st:=Tablo.ListedenCokluSecim('Aktar�lacak Proje Bilgileri','select ID=1,ACIKLAMA=''Ekleyen'' union all select ID=2,''Proje Sorumlusu'' union all select ID=3,''A�ama Sorumlusu'' ',[Tablo.cxEditRepository1Label1,Tablo.cxEditRepository1Label1],['ID','A��klama']);
    end;
  ...
  if (Sender as TMenuItem).Tag in [2,99] then begin//�� Listesi
    if (Sender as TMenuItem).Tag <> 99 then begin
      st:=Tablo.ListedenCokluSecim('Aktar�lacak �� Listesi Bilgileri','select ID=1,ACIKLAMA=''Ekleyen'' union all select ID=2,''Atanan'' union all select ID=3,''Bilgilendirilecek'' ',[Tablo.cxEditRepository1Label1,Tablo.cxEditRepository1Label1],['ID','A��klama']);
    end;
      if ((Sender as TMenuItem).Tag = 99)or((st.Count>0)and(TGirisKutusuEx.BilgiAlEx(BGPersonel_secimi, ctrls) = mrOk)) then begin
        if (EskiPer>0)and(YeniPer>0)and(EskiPer<>YeniPer) then begin
          for i := 0 to st.Count-1 do begin
  ...
  if (Sender as TMenuItem).Tag in [3,99] then begin//Teklif
    if (Sender as TMenuItem).Tag <> 99 then begin
      st:=Tablo.ListedenCokluSecim('Aktar�lacak Teklif Bilgileri','select ID=1,ACIKLAMA=''Haz�rlayan'' union all select ID=2,''Onaylayacak'' union all select ID=3,''Onaylayan'' ',[Tablo.cxEditRepository1Label1,Tablo.cxEditRepository1Label1],['ID','A��klama']);
    end;
      if ((Sender as TMenuItem).Tag = 99)or((st.Count>0)and(TGirisKutusuEx.BilgiAlEx(BGPersonel_secimi, ctrls) = mrOk)) then begin
        if (EskiPer>0)and(YeniPer>0)and(EskiPer<>YeniPer) then begin
          for i := 0 to st.Count-1 do begin
  ...
  if (Sender as TMenuItem).Tag in [4,99] then begin//Servis
    if (Sender as TMenuItem).Tag <> 99 then begin
      st:=Tablo.ListedenCokluSecim('Aktar�lacak Servis Bilgileri','select ID=0,ACIKLAMA=''Sorumlu'' union all select ID=1,ACIKLAMA=''Kabul Eden'' union all select ID=2,''Onay Alan'' union all select ID=3,''Onaylayacak'' union all select ID=4,''Onaylayan'' ',[Tablo.cxEditRepository1Label1,Tablo.cxEditRepository1Label1],['ID','A��klama']);
    end;
      if ((Sender as TMenuItem).Tag = 99)or((st.Count>0)and(TGirisKutusuEx.BilgiAlEx(BGPersonel_secimi, ctrls) = mrOk)) then begin
        if (EskiPer>0)and(YeniPer>0)and(EskiPer<>YeniPer) then begin
          for i := 0 to st.Count-1 do begin
...
begin
  if GridKesintiView.DataController.Controller.SelectedRecordCount > 0 then begin
    ...
    repeat
      ...
      .DateTimePicker('Ba�lama Tarihi*',@Tarih,TDateTimeKind.dtkDate,'yyyy-MM-dd')
      .CurrencyEdit('Ayl�k Tutar *',@Tutar,2)
      .ImageComboBox('Kesinti kayna�� *',@KesintiKaynagi,Tablo.FDCnn,'SELECT ''B'' TUR, ''Banka'' ADI UNION ALL SELECT ''K'' TUR, ''Kasa''')
      .ImageComboBox('T�r *',@Sira,Tablo.FDCnn,'SELECT SIRA, ETIKET FROM REHBERAYAR WHERE YERI=6 ')
      .Memo('A��klama',@Aciklama));
      ...
      TabloYenile(TabDemirbasBilgi,[REHBER.Fields[0].AsInteger]);
        ...
        if PmIzinTurleri.Items.Count<1 then begin
            ...
            while not Tablo.Query1.eof do begin
               ...
               if Tablo.Query1.Fields[1].AsInteger=-1 then //Hakedi� alt�na �izgi �izelim
