...
uses
  ...
  cxImageComboBox, cxMemo, cxButtonEdit, cxTimeEdit, cxCurrencyEdit,Variants, System.JSON,
...
begin
  ...
  if Potansiyel then begin
     IKGridViewGIRISTARIHI.Caption:='Başvuru Tarihi';
     TabloNo := TabNo_IK_POTANSIYEL;
  end else begin
     IKGridViewGIRISTARIHI.Caption:='Giriş Tarihi';
     TabloNo:=TabNo_IK;
  end;
  Rehber.Close;
...
begin
   ...
   if TGirisKutusuEx.BilgiAlEx(BGTarih_gir, TGirdiDenetimleri.Create.DateTimePicker(BGTarih_gir+':', @Tarih,dtkDate)) = mrOk then begin
      Trh := TDateTime(Tarih);
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'insert into [PERS_HAREKET] (REHBERID,TARIH,TUR,ACIKLAMA)values(&REHBERID,'''+FormatDateTime('yyyy-mm-dd', Trh)+''' ,&TUR,&ACIKLAMA)'
...
{        Tablo.TablodanSorguAc(1,'select SUBEID=(select FIRMA from REHBER where ID=ROL.SUBEID),'+
        ...
        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'insert into [PERS_HAREKET] (REHBERID,TARIH,TUR,ROLID,ACIKLAMA)values(&REHBERID,'''+FormatDateTime('yyyy-mm-dd', Trh)+''' ,&TUR, &ROLID, &ACIKLAMA)'
...
procedure TIKListeDlg.BtnDuzenlePerizinClick(Sender: TObject);
  ...
  begin
    ...
    Tur := Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'SELECT IZINTURU FROM PERSONELIZIN WHERE ID=&ID',['&ID'],[IzinID],True);
...
begin
  ...
  begin
    ...
    ctrls := TGirdiDenetimleri.Create.Edit(Tipi+' Dönem',@Donemx).DateTimePicker(BGIzin_tarih_gir,@izinTarihi).Edit(Tipi+'BGGun_miktari_gir',@Hakedilen).Edit('Açıklama',@Aciklama);
...
begin
    ...
    for I := 0 to Kullanicilar.Count - 1 do begin
         ...
         GrupListe.Add(ID);
         veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'update [REHBER] set NOTLAR='''+GrupListe.DelimitedText+''' where ID='+TabGrup.Fields[0].AsString,[],[]);
    end
...
begin                              // PChar(SSilmeSorusu)
   ...
   if Application.MessageBox(PChar(stringgrid1.Cells[0,StringGrid1.Row]+' listeden çıkarılacak..'), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
...
begin
  ...
  //RehberAyar tablosuna eklenecek bilgiler.
   Etiket = 'Vardiya Tür?'
   Kaynak=Select '' as ANAHTAR,3 as DEGER  union Select 'Sabit',1 union Select 'Değişken',2
  }
end;
...
begin
   Liste_SP_Cagir(5);   // Son Aranan (KULLANICI_ARAMA tarih desc) -> sunucu-tarafi SP
end;

procedure TIKListeDlg.LabelSikArananlarClick(Sender: TObject);
begin
   Liste_SP_Cagir(3);   // Sik Aranan (KULLANICI_ARAMA SAY desc) -> sunucu-tarafi SP
end;
...
begin
  ...
  begin
    ...
    mResult := TGirisKutusuEx.BilgiAlEx(BGAvans_miktari,TGirdiDenetimleri.Create
    .ImageComboBox('ödeme kaynağı',@OdemeKaynagi,Tablo.FDCnn,'SELECT ''B'' TUR, ''Banka'' ADI UNION ALL SELECT ''K'' TUR, ''Kasa''')
    .CurrencyEdit('Avans tutarı',@Tutar,2));
...
begin
   Liste_SP_Cagir(1);   // Tum kayitlar (order by 1) -> sunucu-tarafi SP
end;
...
begin
   ...
   // Tum/Son/Sik toolbar butonlarini SP listeleme handler'larina bagla
...
//   Tablo.TablodanSorguAc(1,'select GIRISTARIHI from REHBER where ID='+IntToStr(RehberId));
  ////işe giriş tarihi boş ise girilmesi istenir.
   ...
   if Tablo.Query1.IsEmpty then
     ...
     IzinTarihi := StrToDate(FormatDateTime('dd'+FormatSettings.DateSeparator+'mm'+FormatSettings.DateSeparator, IseGiris)+IntToStr(BuYil));//işe girişin gün ve ayını alalım
...
//   Tablo.TablodanSorguAc(1,'select GIRISTARIHI from REHBER where ID='+REHBER.FieldByName('ID').AsString);
   ...
   izinTurBirim := 3;    //Varsayılan olarak 'Gün' izin türü birimi geliyor
   //Miktar := 0;
  ////işe giriş tarihi boş ise girilmesi istenir.
  /// Yeniye tıklandığında Popup button olacak orada izin türleri seçilecek ve eğer yıllık izin ise Hak edilen miktar girilecek.
  /// yok eğer diğer izinlerden ise tek satır eklenecek.
   ...
   if izinTur=-1 then begin
    ...
    ctrls := TGirdiDenetimleri.Create.edit(Tipi+' Dönemi',@Donemx).DateTimePicker('ızin Tarihini Giriniz',@izinTarihi).edit(Tipi+' Gün Miktarını Giriniz',@Hakedilen).Edit('Açıklama',@Aciklama);
    if TGirisKutusuEx.BilgiAlEx(Tipi+' ızin Bilgileri (ışe Giriş:'+FormatDateTime('dd'+FormatSettings.DateSeparator+'mm'+FormatSettings.DateSeparator+'yyyy', IseGiris)+')', ctrls) = mrOk then begin
       Hakedilen:=StringReplace(VarToStr(Hakedilen),',',FormatSettings.Decimalseparator,[rfReplaceAll]);
       Hakedilen:=StringReplace(VarToStr(Hakedilen),'.',FormatSettings.Decimalseparator,[rfReplaceAll]);
       Donemx := IntToStr(StrToIntDef( VarToStr(Donemx),1));
...
begin
  ...
  if Tablo.KodAgacindanSec(KodAgaciLokasyonDlg,sqltext,True,True,False,True,LokID,LokKod,LokAciklama,slist,[nil,nil,nil],['REHBERID','TUR'],[REHBER.FieldByName('ID').AsString,IntToStr(Lokasyon_Genel)]
          ,['Kod','Açıklama','',''],[True,True,False,False],True) then try
    TabEkipmanlar.Edit;
...
begin
  ...
  Tablo.AramaKaydet(MODUL_IK, REHBER.FieldByName('ID').AsInteger);   // Son/Sik Aranan takibi (kart acilinca upsert)
...
begin
   ...
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update REHBER set TEMAS='+IntToStr(Abs(StrToInt(BoolToStr(CheckPDKSTakibi.Checked))))+' where ID='+ REHBER.Fields[0].AsString,[],[]);
end;
...
var Grup : Variant;
    ID : integer;
begin
    if TGirisKutusuEx.BilgiAlEx(BGYeni,TGirdiDenetimleri.Create.Edit('Grup Adı',@Grup)) = mrOk then begin
       ID := veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'insert into [REHBER] (FIRMA, GRUP, DURUM, EKLEYEN) values ('''+vartostr(Grup)+''',336,1,'+Kullanan+')  SELECT SCOPE_IDENTITY()',[],[],True);
...
begin
  ...
  if (Tablo.Query4.RecordCount>0)and(Tablo.Query4.FieldByName('SABLONDUYURUID').Asstring<>'') then begin
      ...
      veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'insert into [DUYURUKULLANICI] ([DUYURUID],[ALICIID],[OKUNDU])  '+
      ...
      veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'insert into DUYURUIMAJ ([DUYURUID],[IMAJID])  '+
...
begin
  ...
  Tablo.Query7.SQL.Text :=
    ...
    ' union all ' +
    ' select  STR=''Proje - Aşama Sorumlusu'',SAYI=count(*) from PROJEASAMA where DURUM<>2 and REHBERID = @rehID ' +
    ' union all ' +
    ' select  STR=''ış Listesi - Atayan'',SAYI=count(*) from GOREVLER where ACKAPA=0 and EKLEYEN = @rehID ' +
    ' union all ' +
    ' select  STR=''ış Listesi - Atanan'',SAYI=count(*) from GOREVKULLANICI where ( select distinct ACKAPA from GOREVLER G where G.ID=GOREVKULLANICI.LISTGOREVID)=0 and REHBERID = @rehID and TUR=11 '+
    ' union all ' +
    ' select  STR=''ış Listesi - Bilgilendirilecek'',SAYI=count(*) from GOREVKULLANICI where ( select distinct ACKAPA from GOREVLER G where G.ID=GOREVKULLANICI.LISTGOREVID)=0 and REHBERID = @rehID and TUR=12 '+
    ' union all ' +
    ' select  STR=''Teklif - Hazırlayan'',SAYI=count(*) from TEKLIF where DURUM not in (3,6,7,8) and HAZIRLAYAN = @rehID ' +
    ' union all ' +
  ...
  Tablo.Query7.First;
  ShowWarning := False;
  WarningStr := 'ışten çıkartmakta bulunduğunuz personel daha önce;';
  while not Tablo.Query7.Eof do begin
    if Tablo.Query7.Fields[1].AsInteger > 0 then begin
      ShowWarning := True;
      WarningStr := WarningStr + #13#10 + Tablo.Query7.Fields[0].AsString + ' olarak ' + Tablo.Query7.Fields[1].AsString +' farklı yerdeki';
    end;
    Tablo.Query7.Next;
  end;
  if ShowWarning then begin
    WarningStr := WarningStr + #13#10 + 'kayıtlarda kullanılmıştır.';
    WarningStr := WarningStr + #13#10 + 'İlgili işleri başka bir personele aktarmak ister misiniz? ';
   ...
   if TGirisKutusuEx.BilgiAlEx(FWCikis, TGirdiDenetimleri.
      ...
      Trh := TDateTime(Tarih);
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update REHBER set DURUM=0 where ID='+REHBER.FieldByName('ID').AsString,[],[]);
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update KULLANICI set DURUM=0 where REHBERID='+REHBER.FieldByName('ID').AsString,[],[]);
...
begin
  ...
  if (Sender as TMenuItem).Tag in [2,99] then begin//?? Listesi
    if (Sender as TMenuItem).Tag <> 99 then begin
      st:=Tablo.ListedenCokluSecim('Aktarılacak ış Listesi Bilgileri','select ID=1,ACIKLAMA=''Ekleyen'' union all select ID=2,''Atanan'' union all select ID=3,''Bilgilendirilecek'' ',[Tablo.cxEditRepository1Label1,Tablo.cxEditRepository1Label1],['ID','Açıklama']);
    end;
      if ((Sender as TMenuItem).Tag = 99)or((st.Count>0)and(TGirisKutusuEx.BilgiAlEx(BGPersonel_secimi, ctrls) = mrOk)) then begin
        if (EskiPer>0)and(YeniPer>0)and(EskiPer<>YeniPer) then begin
          for i := 0 to st.Count-1 do begin
            if st[i]='1' then //ekleyen
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update GOREVLER set EKLEYEN='+VarToStr(YeniPer)+' where ACKAPA = 0 and EKLEYEN='+VarToStr(EskiPer),[],[])
            else if st[i]='2' then //Atanan   tur 11
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update GOREVKULLANICI set REHBERID='+VarToStr(YeniPer)+' where (select ACKAPA from GOREVLER G where G.ID=GOREVKULLANICI.LISTGOREVID) = 0 and TUR=11 and REHBERID='+VarToStr(EskiPer),[],[])
            else if st[i]='3' then //Bilgilendirilecek    tur 12
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update GOREVKULLANICI set REHBERID='+VarToStr(YeniPer)+' where (select ACKAPA from GOREVLER G where G.ID=GOREVKULLANICI.LISTGOREVID) = 0 and TUR=12 and REHBERID='+VarToStr(EskiPer),[],[])
          end;
        end;
      end;
    end;
  ...
  if (Sender as TMenuItem).Tag in [3,99] then begin//Teklif
    if (Sender as TMenuItem).Tag <> 99 then begin
      st:=Tablo.ListedenCokluSecim('Aktarılacak Teklif Bilgileri','select ID=1,ACIKLAMA=''Hazırlayan'' union all select ID=2,''Onaylayacak'' union all select ID=3,''Onaylayan'' ',[Tablo.cxEditRepository1Label1,Tablo.cxEditRepository1Label1],['ID','Açıklama']);
    end;
      if ((Sender as TMenuItem).Tag = 99)or((st.Count>0)and(TGirisKutusuEx.BilgiAlEx(BGPersonel_secimi, ctrls) = mrOk)) then begin
        if (EskiPer>0)and(YeniPer>0)and(EskiPer<>YeniPer) then begin
          for i := 0 to st.Count-1 do begin
            if st[i]='1' then //haz?rlayan
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update TEKLIF set HAZIRLAYAN='+VarToStr(YeniPer)+' where DURUM not in (3,6,7,8) and HAZIRLAYAN='+VarToStr(EskiPer),[],[])
            else if st[i]='2' then //Onaylayacak
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update TEKLIF set ONAYLAYACAK='+VarToStr(YeniPer)+' where DURUM not in (3,6,7,8) and ONAYLAYACAK='+VarToStr(EskiPer),[],[])
            else if st[i]='3' then //onaylayan
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update TEKLIF set ONAYLAYAN='+VarToStr(YeniPer)+' where DURUM not in (3,6,7,8) and ONAYLAYAN='+VarToStr(EskiPer),[],[])
          end;
        end;
      end;
    end;
  ...
  if (Sender as TMenuItem).Tag in [4,99] then begin//Servis
    if (Sender as TMenuItem).Tag <> 99 then begin
      st:=Tablo.ListedenCokluSecim('Aktarılacak Servis Bilgileri','select ID=0,ACIKLAMA=''Sorumlu'' union all select ID=1,ACIKLAMA=''Kabul Eden'' union all select ID=2,''Onay Alan'' union all select ID=3,''Onaylayacak'' union all select ID=4,''Onaylayan'' ',[Tablo.cxEditRepository1Label1,Tablo.cxEditRepository1Label1],['ID','Açıklama']);
    end;
      if ((Sender as TMenuItem).Tag = 99)or((st.Count>0)and(TGirisKutusuEx.BilgiAlEx(BGPersonel_secimi, ctrls) = mrOk)) then begin
        if (EskiPer>0)and(YeniPer>0)and(EskiPer<>YeniPer) then begin
          for i := 0 to st.Count-1 do begin
            if st[i]='0' then //Sorumlu
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update SERVIS set SORUMLU='+VarToStr(YeniPer)+' where ACKAPA = 0 and SORUMLU='+VarToStr(EskiPer),[],[])
            else if st[i]='1' then //Kabul Eden
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update SERVIS set KABUL_EDEN='+VarToStr(YeniPer)+' where ACKAPA = 0 and KABUL_EDEN='+VarToStr(EskiPer),[],[])
            else if st[i]='2' then //Onay alan
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update SERVIS set ONAYALAN='+VarToStr(YeniPer)+' where ACKAPA = 0 and ONAYALAN='+VarToStr(EskiPer),[],[])
            else if st[i]='3' then //onaylayacak
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update SERVIS set ONAYLAYACAK='+VarToStr(YeniPer)+' where ACKAPA = 0 and ONAYLAYACAK='+VarToStr(EskiPer),[],[])
            else if st[i]='4' then //onaylayan
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update SERVIS set ONAYLAYAN='+VarToStr(YeniPer)+' where ACKAPA = 0 and ONAYLAYAN='+VarToStr(EskiPer),[],[])
          end;
        end;
      end;
    end;
...
// IK listesini sunucu-tarafi SP ile getirir (sp_Prog_IK_Liste_Json2 - tek JSON parametre).
//   AMod: 1=Tum (order by 1), 3=Sik (KA.SAY desc), 4=Filtre (JvTimer), 5=Son (KA.tarih desc).
//   IKI PARAM: @Baslik = SELECT ek kolonlari (IK'da bos); @Kosullar = filtreler JSON (guvenli escape).
//   Bos/opsiyonel filtre JSON'a EKLENMEZ (SP absent=NULL=filtre yok); sayilar TJSONNumber.
//   Alan filtreleri yalniz AMod=4'te; personel/aday ayrimini Aday (=Potansiyel) belirler.
var
  ...
  begin
    Result := StrToIntDef(VarToStr(Combo.EditValue), 0);
  end;

  // Aday sayisal combo filtresi: yalniz Filtre+Aday+kutu dolu ise JSON'a eklenir
  procedure ComboEkle(const Ad: string; Combo: TcxImageComboBox);
  begin
    if Filtre and Aday and (Combo.Text <> '') then
      j.AddPair(Ad, TJSONNumber.Create(ComboDeger(Combo)));
  end;
...
begin
  ...
  if SubeVarmi then SubeYetki := Tablo.YetkiliSubeleriGetir(22, YetkiTur_Gorme)
  else SubeYetki := '';

  // Firma/Kod metin filtreleri (yalniz Filtre modu; digerlerinde bos -> JSON'a eklenmez)
  Firma := '';  Kod := '';
  if Filtre then begin
    if Aday then Firma := Trim(FArama.AraFirma2.Text)
    else begin
      Firma := Trim(FArama.AraFirma.Text);
      Kod   := Trim(FArama.AraKod.Text);
    end;
  end;
  ...
  try
    ...
    j.AddPair('Dil', TJSONNumber.Create(Dil));                // eski memo DIL=-1 yerine parametre
    j.AddPair('IlgiliArama', TJSONNumber.Create(0));          // eski @ILGILIARAMA (hep 0)

    // Metin filtreleri (bos ise eklenmez)
    if Firma <> '' then j.AddPair('Firma', Firma);
    if Kod   <> '' then j.AddPair('Kod',   Kod);

    // Aday sayisal filtreleri (yalniz Filtre + Aday + kutu dolu ise)
    ComboEkle('Cinsiyet',  FArama.ComboCinsiyet);
    ComboEkle('Ogrenim',   FArama.ComboOgrenim);
    ComboEkle('Sektor',    FArama.ComboSektor);
    ComboEkle('Departman', FArama.ComboDepartman);
    ComboEkle('Gorev',     FArama.ComboGorev);
    ComboEkle('Dil1',      FArama.ComboDil1);
    ComboEkle('Dil2',      FArama.ComboDil2);
    ComboEkle('Il',        FArama.ComboIl);
    if Filtre and Aday and (FArama.EditUyruk.Tag > 0) then
      j.AddPair('Uyruk', TJSONNumber.Create(FArama.EditUyruk.Tag));

    // Ucret araligi (aday) - alt+ust ikisi de dolu ise
    if Filtre and Aday and (FArama.EditUcret.Text <> '') and (FArama.EditUcret2.Text <> '') then begin
      j.AddPair('UcretAlt', TJSONNumber.Create(StrToCurrDef(FArama.EditUcret.Text,  0)));
      j.AddPair('UcretUst', TJSONNumber.Create(StrToCurrDef(FArama.EditUcret2.Text, 0)));
    end;

    if SubeVarmi and (SubeYetki <> '') then j.AddPair('SubeYetkiList', SubeYetki);
    // TekSube/temsilci yetkisi yalniz Filtre modunda (eski JvTimer davranisi)
    if Filtre then j.AddPair('TekSubeTum', TJSONNumber.Create(ModulYetki_TekSubeTum.IK));
    j.AddPair('KulId', TJSONNumber.Create(StrToIntDef(Kullanan, 0)));  // KULLANICI_ARAMA + temsilci
    j.AddPair('SubeId', TJSONNumber.Create(SubeId));
    j.AddPair('Modul',  TJSONNumber.Create(MODUL_IK));                 // KULLANICI_ARAMA.MODUL (34)

    // @Baslik='' (IK ek-alan yok) + @Kosullar=j; helper j'yi Free eder + TabloYenile yapar.
    Tablo.ListeSPJson(REHBER, 'sp_Prog_IK_Liste_Json2', '', j, LocateID);
...
begin
  if GridKesintiView.DataController.Controller.SelectedRecordCount > 0 then begin
    ...
    repeat
      ...
      .DateTimePicker('Başlama Tarihi*',@Tarih,TDateTimeKind.dtkDate,'yyyy-MM-dd')
      .CurrencyEdit('Aylık Tutar *',@Tutar,2)
      .ImageComboBox('Kesinti kaynağı *',@KesintiKaynagi,Tablo.FDCnn,'SELECT ''B'' TUR, ''Banka'' ADI UNION ALL SELECT ''K'' TUR, ''Kasa''')
      .ImageComboBox('Tür *',@Sira,Tablo.FDCnn,'SELECT SIRA, ETIKET FROM REHBERAYAR WHERE YERI=6 ')
      .Memo('Açıklama',@Aciklama));
    ...
    if mResult = mrOk then begin
      Etiket := Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'SELECT ETIKET FROM REHBERAYAR WHERE YERI=6 AND SIRA=&SIRA',['&SIRA'],[Sira],True);
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE PLANMAAS SET ETIKET=&ETIKET, SIRA=&SIRA, TUR=&TUR, '+
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'DELETE FROM PLANMAAS WHERE ID=&ID',['&ID'],[PlanMaasID]);
      end;
      TabloYenile(TabKesinti,[REHBER.FieldByName('ID').AsInteger]);
    end;
  end;
      ...
      TabloYenile(TabDemirbasBilgi,[REHBER.Fields[0].AsInteger]);
        ...
        if PmIzinTurleri.Items.Count<1 then begin
            ...
            while not Tablo.Query1.eof do begin
               ...
               if Tablo.Query1.Fields[1].AsInteger=-1 then //Hakediş altına çizgi çizelim
