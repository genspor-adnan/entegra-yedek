...
type
  ...
  private
   ...
   function  MTGetActiveBuffer(var Buffer: TRecordBuffer): Boolean;
   procedure MTReadRecord(Buffer:TRecordBuffer;RecNo:Integer);
   procedure MTWriteRecord(Buffer:TRecordBuffer;RecNo:Integer);
   procedure MTAppendRecord(Buffer:TRecordBuffer);
   function  MTFilterRecord(Buffer: TRecordBuffer): Boolean;
...
function THKMemTab.MTGetActiveBuffer(var Buffer: TRecordBuffer): Boolean;   //Checks the State and Results a defined Buffer;
begin
 case State of
  dsBrowse:
...
//Private
procedure THKMemTab.MTReadRecord(Buffer:TRecordBuffer;RecNo:Integer);   //Reads a Rec from Stream in Buffer
begin
 FStream.Position:=MTGetRecStreamPos(RecNo);
 FStream.ReadBuffer(Buffer^, FRecSize);
end;

procedure THKMemTab.MTWriteRecord(Buffer:TRecordBuffer;RecNo:Integer);  //Writes a Rec from Buffer to Stream
begin
 FStream.Position:=MTGetRecStreamPos(RecNo);
 FStream.WriteBuffer(Buffer^, FRecSize);
end;

procedure THKMemTab.MTAppendRecord(Buffer:TRecordBuffer);   //Appends a Rec (from Buffer) to Stream
begin
...
function THKMemTab.GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
...
function THKMemTab.GetFieldData(Field: TField; Buffer: TValueBuffer): Boolean;
...
function THKMemTab.MTFilterRecord(Buffer: TRecordBuffer): Boolean;
