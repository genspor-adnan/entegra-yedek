unit UDokumanWizard;
     // Ýþlem Tipleri 0 tüm 1 Görme  2 deðiþ  3 Revizyon  4 silme 5 Revizyon silme  6 E-Posta 7 Ver (Export)

     //yetki  ve  bildirim   5:herkes  4:þube 3:departman 2:görev 1:kiþi
  ...
  if ComboKATEGORI.EditValue = -1 then begin
     ...
     if TabSozlesme.Active=False then begin
      ...
      if (DETAY.Active)and(TabDokuman.Active) then begin
        // 28/07/2022 AO hata veriyor kaldýrýldý
        ...
        if (i>0) and (Application.MessageBox(PChar(PWHepsiSilinecektirUyari),pchar(Uyari), MB_YESNO + MB_ICONWARNING)=mrNo) then begin
         ...
         // if TabDokuman.State in[dsEdit,dsInsert] then
         //   TabDokuman.Post;
          Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'delete from REHBERBILGI where YERI=&Yeri and YER_ID=&YerID ',['&Yeri','&YerID'],[TabNo_DOKUMAN, TabDokuman.FieldByName('ID').AsInteger]);
          DetayTablosuAc
        end;
      end else
        DetayTablosuAc;
...
begin
  {if cbDokumanTipi.EditValue = 15 then begin//email  vars:46    yeri 1 : firma iletiþim, yeri 4 : ilgili
    AVars := 46;
    ABaslik := 'EPosta Adresi Seçimi';
    IcerikDoldur;
  end else if cbDokumanTipi.EditValue = 20 then begin//fax  vars:43
    AVars := 43;
    ABaslik := 'Faks Numarasý Seçimi';
    IcerikDoldur;
  end else begin
    TabDokuman.Edit;
...
begin
 if (PageControl1.ActivePage = EkAlanlarEkr)and(EkAlanOlustu=False) then begin
     ...
     Durum := TabDokuman.State;   //durum u alalým
     if Durum in [dsEdit, dsInsert] then
        TabDokuman.Post;
     Tablo.AlanOlustur(DokumanWizard, -1, DtsDokuman);// DtsDokuman
     if Durum in [dsEdit, dsInsert] then //eðer durum edit moddaysa tekrar edite getirelim
        TabDokuman.Edit;
...
begin
    ...
    if (Tablo.Query3.FieldByName('Sayi').AsInteger <> 0) or (YetkiID = -99) then begin
        ...
        TabAbone.Append;
        TabAbone.FieldByName('TUR').AsInteger:=Tur; //Tür 5:herkes 4:þube 3:departman 2:görev 1:kiþi
...
begin
   Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, 'delete from DOKUMANILGILI where DOKUMANID='+TabDokuman.Fields[0].AsString+ ' and DOKUMANILGILIID='+ TabIlgili.Fields[0].AsString,[],[]);
   Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, 'delete from DOKUMANILGILI where DOKUMANID='+ TabIlgili.Fields[0].AsString + ' and DOKUMANILGILIID='+TabDokuman.Fields[0].AsString,[],[]);
   TabIlgili.Close;
   TabIlgili.Open;
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay),MB_YESNO) = IDYES then begin
    for I := GridRevizeView.Controller.SelectedRecordCount -1  downto 0 do begin
      ...
      Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, 'delete from IMAJ where ID= &ID',['&ID'],[ID]);
      TabRevize.Close;
      TabRevize.Open;
...
//              ,[nil,nil,nil,Tablo.RepCariGrup,Tablo.RepCaribolum,Tablo.RepCariSinif],['Id','Kullanýcý','Rol','Grup','Kategori','Sýnýf']);
...
begin
  ...
  VeriTabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'update DUYURUKULLANICI set OKUNMATARIHI='+s+' where DUYURUID in (select ID from DUYURU where YER='+IntToStr(Tabno_DOKUMAN)+' and YER_ID='+TabDokuman.FieldByName('ID').AsString+')',[],[]);
...
begin
  ...
  end else if (Shift = [ssAlt,ssCtrl]) and (Key = Ord('D'))and(PageControl1.ActivePage = EkAlanlarEkr) then begin   //Bileþen Düzenle
    ctrl := FindVCLWindow(Mouse.CursorPos);
    if Assigned(ctrl) then begin
      OutputDebugString(PChar(ctrl.Name));
      ctrlPos := ctrl.ScreenToClient(Mouse.CursorPos);
...
begin
  ...
  DYetkisonuc:=Tablo.DokumanYetkiKontrol(321,DokumanID);   //YEtki Kontrolu Yapýlýyor.
  ...
  DokumanYetkiID := DokumanID; //Dokuman yetki bilgileri atanýyor.
  DokumanYetkiTur := 321;
   ...
   case IslemOp of
     ...
     'D','K':begin
              if DYetkisonuc.Degistir = true then begin
                    ...
                    //IMAJ tablosundan bilgileri alalým
...
begin
  ...
  if TabImaj.FieldByName('ICDIS').AsString = 'True' then //eðer dosyada tutuluyorsa
     Tablo.TablodanSorguAc(3,' DECLARE @SONUC varbinary(MAX) exec sp_Imaj_Okuma '+TABLO.Query1.FieldByName('ID').AsString+' ,@SONUC OUTPUT select BELGE=@SONUC, BELGEADI='''+ExtractFileExt(Ad)+'''' )
      ...
      Tablo.TablodanSorguAc(3,'select ID,ICDIS,BELGE,BELGEADI from IMAJ where ID='+TABLO.Query1.FieldByName('ID').AsString); //eðer doküman tabloda BELGE alanýnda ise
...
begin
   ...
   if TabImaj.FieldByName('ICDIS').AsString = 'True' then //eðer dosyada tutuluyorsa
      ...
      Tablo.TablodanSorguAc(5,'select ID,ICDIS,BELGE,BELGEADI from IMAJ where ID='+TabRevize.Fields[0].AsString); //eðer doküman tabloda BELGE alanýnda ise
...
begin
//   Eðer yeni kayýt eklenirken iptal edildiyse kayýr edilmiþ tüm bilgiler silinir
   if IslemOp='E' then
...
begin
    ...
    begin
       ...
       if tablo.Query8.FieldByName('ADET').AsInteger > 0 then begin
             ...
             if EkleDetay then
                Ekle(DETAY, TabNo_DOKUMAN, TabDokuman.FieldByName('ID').AsInteger,'Deðiþ');
             ...
             //sürüm veya tarih deðiþirse imaj tablosundaki bu bilgiler de update olmalý
...
begin
   //önce kýsayol oluþturulacak dosyayý bulalým
     ...
     if Tablo.ListedenBilgiGetir('Doküman Listesi',SqlText,st,[]) then begin
       Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, 'insert into DOKUMANILGILI(DOKUMANID, DOKUMANILGILIID,SUBEID) values('+TabDokuman.Fields[0].AsString+','+ st.Strings[2]+','+IntToStr(SubeId)+')', [],[]);
       dokilgi:=TabDokuman.Fields[0].AsString;
       dok:=st.Strings[2];
       Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn, 'insert into DOKUMANILGILI(DOKUMANID, DOKUMANILGILIID,SUBEID) values('+dok+','+ dokilgi+','+IntToStr(SubeId)+')', [],[]);
       TabIlgili.Close;
       TabIlgili.Open;
...
begin
 ...
 while not TabAltKlasor.Eof do begin
   ...
   Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'Delete FROM DOKUMANYETKI WHERE YerID ='+inttostr(Altid),[],[]);
   ...
   //alt klasorlerde bulunan dokumanlarýn yetkilerini atiyoruz
   ...
   Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'Delete FROM DOKUMANYETKI WHERE YerID ='+inttostr(AltDokuman),[],[]);
   ...
   // klasor içinde  bulunan dokumanlarýn yetkilerini atiyoruz
...
begin
  ...
  //Herkes iþaretlendiginde sutundaki tüm kullanýcýlar iþaretlensin
  if (tabyetki.FieldByName('REHBERID').AsInteger = 0)  and (TabYetki.FieldByName('YERID').AsInteger = DokumanYetkiID) and(Tabyetki.FieldByName(HAlan).AsBoolean = False )
   then
    begin
     Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'update DOKUMANYETKI SET '+HAlan+' =1 where YERID ='+InttoStr(DokumanYetkiID),[],[] );
     TabloYenile(TabYetki, [DokumanYetkiTur, DokumanYetkiID]);
    end
  {  else
    //Herkes iþaretli iken bir kullanýcýnýn iþareti kalkarsa herkesin iþareti kalksin
   ...
   then
    begin
     Veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'update DOKUMANYETKI SET '+HAlan+' = 0 where REHID=0 AND YERID ='+InttoStr(DokumanYetkiID),[],[] );
     TabYetki.Post;
      TabYetki.Close;
      TabYetki.Open;
    end }
     ...
     if  (TabYetki.FieldByName('YERID').AsInteger = DokumanYetkiID) then
          TabYetki.Post;
  ///Alt Klasor Kontrolu Yapýlýyor. Eðer Var ise Kullanýcýya Yetkiler Alt klasorlere uygulansýn mý soruyor
...
{ if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay),MB_YESNO) = IDYES then begin
   case tabyetki.FieldByName('TUR').AsInteger of
    ...
    //5: veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'update DOKUMANYETKI SET GOR = 0 , EKLE = 0 , SIL = 0, DEGISTIR = 0 WHERE REHBERID = 0 AND YERID = '+InttoStr(DokumanYetkiID), [],[])
    else
      veritabani.BasitKomutÇalýþtýr(Tablo.FDCnn,'delete from DOKUMANYETKI WHERE ID = '+tabyetki.FieldByName('ID').AsString, [],[]);
   end;}
