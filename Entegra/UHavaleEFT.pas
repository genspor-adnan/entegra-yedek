...
(*
  Deðiþiklikler
  2010-05-17
  1 - ilk açýlýþta en üstteki bankanýn ve bu bankaya gönderilecek havale/eftlerin tümünün seçili gelmesi saðlandý.
  2 - banka seçimi deðiþtirildiðinde yeni  bankaya gönderilecek havale/eftlerin tümünün seçili gelmesi saðlandý.
  3 - Talimat önizleme sayfasýnýn tam ekran gelmesi saðlandý.
  4 - Seçili havale olmamasý durumuna hata mesajý eklendi "secimyokhata"
  *)
interface
...
begin
  Result := ANumber;
  while Length(Result) < ADigit do   //hesapno 123 gibiyse 8 karakter olana kadar önüne 0 konmalý
    Result := '0' + Result;
end;
...
begin
  ...
  if DosyaTuru = '.pdf' then begin
     HavaleEFTEkrani.JvInstallLabel1.Lines[IMZA_ADIM_DOSYATURU] := 'PDF dosya onay için hazýrlanýyor...' ;
     fs.loadfromfile('PDFDosya\'+DosyaAdi);
     tmpdosyaadi := 'PDFDosyaOK\'
  End else if DosyaTuru = '.txt' then begin
     HavaleEFTEkrani.JvInstallLabel1.Lines[IMZA_ADIM_DOSYATURU] := 'Text dosya onay için hazýrlanýyor...' ;
  ...
  /// / web servis bizden imzalanacak olan datayý TByteDynArray tipinde istiyor. Onun için fs içindeki datayý buna çeviririz
  ...
  // Artýk burada E-Ýmza firmasýna dosyayý gönderebiliriz..
  ...
  begin
    // Gelen parmakizini ekranda gösterelim, Ayný parmakizi cep telefonunda da görünecek
    //HavaleEFTEkrani.LabelParmakIzi.Caption := '   Parmak Ýzi : ' + HResult.Parmakizi;
    ...
    case SR.ResultCode of
      ...
      2:  mes := 'Ýþlem zamanaþýmýna uðradý';
    ...
    HavaleEFTEkrani.JvInstallLabel1.Lines[IMZA_ADIM_IMZALAYAN] := SR.SignerName + ' Tarafýndan Ýmzalandý.';
    // showmessage(SR.Message_); //Doðrulama logu (hata olduðunda EGA ya bildireceðiz
    //ShowMessage(SR.SignerName); // Ýmzalanmýþ data
    // SR.FSignedData;    //gelen Data  base 64   (  xxx.P7B olarak kaydedersem içini görmüþ olurum)
    // datayý çevirip kaydedelim
  ...
  fs2.Position := 0;
  // Ýmzalý dosya olarak kaydedelim
...
begin
  ...
  // sað tarafta banka iþaretlendiðinde içerikteki tüm havalelerin seçili gelmesi saðlandý.
...
begin
  ...
  // ilk bankanýn seçili gelmesi ve o banka hareketlerinin sað tarafta gözükmesi..
  TabGonderen.first;
...
begin
  //týklý olmayan satýrlar preview da gözükmesin.
...
begin
  if MessageDlg(Eminmisin, mtInformation, [mbYes, mbNo], 0) = mrYes then
     //Application.MessageBox('MB_OKCANCEL+MB_ICONSTOP','baþlýk',MB_OKCANCEL+MB_ICONINFORMATION);
     Close
  else
    ModalResult:= mrNone;
end;
...
begin
 { // Ýþaretlilerden Text oluþturalým
  case BankaKodu of
   ...
   32:   Result := TEB_Dosya_Olustur; // T.EKONOMÝ BANKASI A.S.
   46:   Result := Ak_Dosya_Olustur ;// AKBANK T.A.S.
   59:   Result := TEB_Dosya_Olustur; // ÞEKERBANK T.A.S.
   62:   Result := Garanti_Dosya_Olustur; // T.GARANTÝ BANKASI A.S.
   64:   Result := TEB_Dosya_Olustur; // T.IÞ BANKASI A.S.
...
Begin
//SFTP toolunun gönderdiði hata kodu
//EScSFTPError.ErrorCode ile gelen hata kodunun açýklamasý
  Case ErrorCode of
     0:	  Result :=	'SSH_FX_OK '	;
     1:	  Result :=	'SSH_FX_EOF '	;
...
begin
  ...
  //ftp ye gönderilmeyecekse bu sayfayý geçmesi gerekiyor..
  ...
  case i of
    0:Begin
        ...
        try
          IdFTP1.Connect;
        Except
          raise exception.Create('Baðlantýda Hata');
        end;
...
begin
  if TabBankaAyar.FieldByName('TALIMAT_GONDER').AsInteger > 0 then Begin
     if TabBankaAyar.FieldByName('TALIMAT_IMZALA').AsBoolean = false then Begin //doðru dosyayý bulalým..
     ...
     End;
     FTPyeGonder(FTPDosya,FTPDosyaYeri,PDFDosyaAdi); //dosya ve dosyaadý ile
  End;
  ...
  if TabBankaAyar.FieldByName('TEXT_GONDER').AsInteger > 0then Begin
     ...
     End;
     FTPyeGonder(FTPDosya,FTPDosyaYeri,TextDosyaAdi); //dosya ve dosyaadý ile
  End;
...
begin
  ...
  if TabBankaAyar.FieldByName('TALIMAT_OLUSTUR').AsBoolean=True then begin
      ...
      // Baský önizlee ekranýmýz zaten mevcut. Bu önizlemeyi PDF olarak kaydederiz,
      ...
      //raporu almak için hazýrlarýz,
      //frxReport1.PrepareReport;
      frxReport1.Export(frxPDFExport1);
      // ve göstermek için tekrar hazýrlarýz.(aksi taktirde patlýyor..)
      frxReport1.PrepareReport;
