...
procedure TITSEzcaDepoDlg.BekletmeSonlandır;
begin
...
try
HTTPReqResp1.URL:= 'http://212.174.130.240/DepoMalAlim/DepoMalAlimReceiverService';//'http://212.174.130.240:8080/DepoDogrulama/DepoDogrulamaReceiverService';  // bunu wsdl den aldık
HTTPReqResp1.UseUTF8InHeader:=true;
HTTPReqResp1.SoapAction:= 'DepoMalAlimReceiverService';
HTTPReqResp1.UserName:='genotip';  //kullanıcı adı ve şifre
HTTPReqResp1.Password:='genotip001';

HTTPReqResp1.Execute(Memo1.Text,Stream);  // burada oluşturduğumuz xml i post ediyoruz. cevap Stream içinde dönecek
...
begin
  ...
  if ARecord.Values[AColumn1.Index] = 'Doğru Bildirim.' then
...
// toplamKayit:= GridDc.RecordCount; // tümünü seçmek için
  ToplamKayit:= GridDc.FilteredRecordCount; // filtre kullanılıyorsa filtrelenmiş olanlar arasında tümünü seçmek için
...
UrunSeri := '%'+TxtUrunSeriNo.Text+'%';
    ...
    begin
    if BarBildirim.ActiveGroupIndex = 0 then     //Mal Alim Hatasız Gonderilen Kayıtlar
      begin
      TabMalAlim.Close;
    ...
    if BarBildirim.ActiveGroup.SelectedLinkIndex = 2  then     //Mal iade Malalim hatasızlar ve satışı olmayanlar gelecek.
    begin
    if BarBildirim.ActiveGroupIndex = 1 then   //Mal Alım iade için ürünün alımı yapılmış ve satılmamaış olması gerekir.
      begin
      TabMalIade.Close;
    ...
    if BarBildirim.ActiveGroup.SelectedLinkIndex = 3  then    //Satış için
    begin
    if BarBildirim.ActiveGroupIndex = 0 then    //Satış yapılmışlar
      begin
      TabSatis.Close;
      TabSatis.sql.clear;
...
var
  ...
  Baslik    : string;
begin
  BekletmeBaslat('Alım Gönderim İşlemi','Lütfen Bekleyiniz...',100);
  BekletmeIlerle(10);
