...
begin
   ...
   Sql1 := 'SELECT' + #13#10 +
              ...
              'CALFARK= CASE WHEN  charindex(''*'',dbo.fn_SaatOlarak('+DbTarihFark('mi','PP.GIRIS','PP.CIKIS')+'))=0 THEN' + #13#10 +
              'dbo.fn_CALFARK(dbo.fn_SaatOlarak('+DbTarihFark('mi','PV.GIRIS','PV.CIKIS')+'),dbo.fn_SaatOlarak('+DbTarihFark('mi','PP.GIRIS','PP.CIKIS')+')) ELSE ''00:00'' END,' + #13#10 +
  ...
  Sql1 := Sql1 + ' AND (PP.GIRIS between '+DbConv('''' + FormatDateTime('mm-dd-yyyy', TarihBas.Date) + ' 00:00:00''','DATETIME',102)+' ' +
       ' and '+DbConv('''' + FormatDateTime('mm-dd-yyyy', TarihBit.Date)+ ' 23:59:59''','DATETIME',102)+'))';
