...
begin
  ...
  if (StokZorunluSecimVar)and(stokhizmetaracagirantur in [9,19,100]) and (PageControl1.ActivePage.Name='SheetStok') then begin//Bu ?r?n yerine se?ilmesi gereken ba?ka ?r?nler var m?
    if EsdegerUrunlerListelendi=False then begin
      ...
      if (Tablo.Query0.Fields[0].AsString<>'')and(Tablo.Query0.Fields[0].AsString<>TabStokListe.FieldByName('ID').AsString) then begin
        ...
        LabelSonEklenen.Caption := 'Eşdeğer ürünler Listesi';
        FocusDuzenle;
  ...
  if PageControl1.ActivePage <> SheetDagitim then begin
     if (PageControl1.ActivePage.Name='SheetHizmet')and(cxDBTreeList1cxDBTreeListColumnTur.Value = 'Başlık') then
        exit;
...
begin
  ...
  if NewPage <> SheetDagitim then begin
      ...
      case NewPage.PageIndex of
        0:cxDBTreeList1cxDBTreeListColumnAd.Caption.Text := 'Stok Adı';
        1:cxDBTreeList1cxDBTreeListColumnAd.Caption.Text := 'Hizmet Adı';
        2:cxDBTreeList1cxDBTreeListColumnAd.Caption.Text := 'Paket Adı';
      end;
  end;
end;
...
begin
  ...
  Str := 'Bu ürün daha önce '+FExtToStr(Adet)+' '+BirimAd+' eklenmiş, üzerine eklensin mi?';
...
{  RootKod := TabStokListe.FieldByName('KOD').AsString;
  ...
  while not TabStokListe.Eof do begin
    if (TabStokListe.FieldByName('ROOTKOD').AsString=RootKod) and (GirisCikis = FWCikis) then begin
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'update STOKID set CIKISTURU='+TabGiris.FieldByName('TUR').AsString+' ,CIKFATBASID=&FatBasID, CIKFATURAID=&FaturaID where PAKETID=&PaketID and LOTNO=&LotNo'
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'update ITS_PAKET set BELGENO=&BelgeNo,FATBASID=&BelgeBasID,BELGETURU=&BelgeTuru where ID=&ITSPaketID'
    ...
    if GirisCikis =FWCikis then
    begin
    Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'update KAREKOD set MALSATILANGLN=(select top 1 BILGI from REHBERBILGI RB inner join REHBERAYAR RA on RB.ETIKET=RA.ETIKET and RA.VARSAYILAN=81 and RB.YER_ID=&RehberID and RB.YERI=2) '
    ...
    Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'update ITS_PTS_GELEN_URUN set STOKID = '''+TabDetayGiris.FieldByName('URUNID').AsString+''',GIRISTURU='+TabDetayGiris.FieldByName('TUR').AsString+' ,GIRFATBASID=&FatBasID, GIRFATURAID=&FaturaID where PTS_GELEN_PAKET_ID=&PaketID and LOTNUMARASI=&LotNo'
                                    ,['&FatBasID','&FaturaID','&PaketID','&LotNo']
                                    ,[TabGiris.FieldByName('ID').AsInteger,TabDetayGiris.FieldByName('ID').AsInteger,PaketID,TabStokListe.FieldByName('KOD').AsString]);
    ...
    Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'update FATURA set IZLEME = 3 where FATBASID=&FatBasID and ID=&FaturaID '
                                    ,['&FatBasID','&FaturaID']
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'update ITS_PTS_GELEN_PAKET set FATBASID=&BelgeBasID,BELGETURU=&BelgeTuru where ID=&ITSPaketID'
    ...
    if GirisCikis = FWGiris then
    begin
    Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'update KAREKOD set MALALINANGLN=(select top 1 BILGI from REHBERBILGI RB inner join REHBERAYAR RA on RB.ETIKET=RA.ETIKET and RA.VARSAYILAN=81 and RB.YER_ID=&RehberID and RB.YERI=2) '
