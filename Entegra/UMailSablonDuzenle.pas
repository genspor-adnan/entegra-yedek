...
uses
  ...
  FireDAC.Comp.Client, SynEdit, Vcl.Menus, Vcl.StdCtrls, cxButtons,
...
begin
  ...
  Content := Utf8Encode(SynEdit1.Lines.Text);
  Len := Length(Content);
  if Len > 0 then
...
begin
   SynEdit1.Lines.Text := Utf8Encode(TabSablon.FieldByName('ICERIK').AsString);
end;

procedure TMailSablonDuzenleDlg.TabSablonBeforePost(DataSet: TDataSet);
begin
  if TabSablon.RecordCount>0 then
     TabSablon.FieldByName('ICERIK').AsString := string(Utf8Encode(SynEdit1.Lines.Text));
end;
