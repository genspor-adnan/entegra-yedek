...
begin
   ...
   if GelirMi=1 then
      Caption := 'Gelir Seçimi Ekraný'
   else if GelirMi=0 then
      Caption := 'Masraf Seçimi Ekraný'
   else
      Caption := 'Masraf/Gelir Seçimi Ekraný';
...
begin
  if cxDBTreeList1.SelectionCount = 1 then begin
     if (not BaslikSecilebilir)and(cxDBTreeList1.Selections[0].HasChildren) then
        raise Exception.Create('Baþlýk deðil Detay seçmelisiniz!')
     else
        ModalResult := mrOk;
      {else begin
        if Gelirmi=16 then
           raise Exception.Create('Detay deðil Baþlýk seçmelisiniz!')
        else
           ModalResult := mrOk;}
  end
  else
