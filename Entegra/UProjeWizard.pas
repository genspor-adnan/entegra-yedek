...
begin
  ULog.OturumYakala(FOturumID);   // LAZY: asama detay silme -> yakala
...
begin
  ULog.OturumYakala(FOturumID);   // LAZY: asama detay post -> yakala
...
begin
  ULog.OturumYakala(FOturumID);   // LAZY: butce detay post -> yakala
...
begin
  ULog.OturumYakala(FOturumID);   // LAZY: proje ilk degisikligi -> yakala
      //Admin değil ise  RolId = -1 ise yönetici demektir..
...
begin
  ULog.OturumYakala(FOturumID);   // LAZY: proje post -> yakala
  ProjeBoslukKontrolu;
...
begin
  ULog.OturumYakala(FOturumID);   // LAZY: butce ekleme -> yakala
...
begin
  ULog.OturumYakala(FOturumID);   // LAZY: butce silme -> yakala
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
     Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from PROJEBUTCE where ID='+TabProjeButce.FieldByName('ID').AsString,[],[]);
     TabloYenile(TabProjeButce,[TabProjeler.Fields[0].AsInteger]);
  end;
end;
...
begin
  if (ComboPRJ_TURU.EditValue=null) or (ComboPRJ_TURU.EditValue=0) then begin
    ...
    Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from GENINI where BOLUM like ''-2132%'' and len(BOLUM)>5 and cast(BOLUM as varchar(30)) not in (select ''-2132''+cast(DEGER as varchar(30)) from GENINI where BOLUM=-2132)',[],[]);
    Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from GENINI where BOLUM=0 and DEGER like ''-2132%'' and len(DEGER)>5 and cast(BOLUM as varchar(30)) not in (select ''-2132''+cast(DEGER as varchar(30)) from GENINI where BOLUM=-2132)',[],[]);
    if not Veritabani.VeriVarMi(Tablo.FDCnn,'select * from GENINI where DIL='+IntToStr(Dil)+' AND  BOLUM=0 and DEGER='+IntToStr(comboPRJ_TIPI.Tag),[],[]) then begin
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,
        'insert into GENINI(BOLUM,ANAHTAR,DEGER,DIL,SIRA) select 0,ANAHTAR,cast(BOLUM as varchar(10))+cast(DEGER as varchar(10)),DIL,0 from GENINI where BOLUM='+IntToStr(Ops_Proje_Turu)+' and DEGER='+VarToStr(ComboPRJ_TURU.EditValue),[],[]);
    end;
    Tablo.LabelClickCombobox(Sender);
  end;
...
begin
   ...
   if ULog.OturumYakalandiMi(FOturumID) or ((TabProjeler.State in [dsEdit, dsInsert]) and TabProjeler.Modified) then
