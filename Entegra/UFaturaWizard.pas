...
//     FreeAndNil(FATURASAYI);
  ...
  try
    ...
    showmessage('Ek alanlar oluşturulurken bir hata ile karşılaşıldı.');
  end;
end;
...
begin
   ...
   case TabFatbaslik.FieldByName('TIPI').AsInteger of
     1,5: begin //Sat��  veya ithal/ihrağ
  ...
  begin
    ...
    if (IslemOp <> 'K')and(
       ...
       ((IslemOp = 'D')and((KilitKontrolEt(2,Tur,TabFatbaslik.FieldByName('FATURATARIH').AsDateTime,2))or (IadeKontrolEt) ))or
        ...
        Aman_Kilitle;
    //e��er say�m sonucu giri� ve ��k�� fiİleri oluşmu�sa bunlar değiştirilemez
    ...
    case IslemOp of
      'E':
        if TabFaturaIDsi < 1 then // e�er ID 0 veya -1 ise ekleme yapİls�n, normal Id varsa d�n���m var demektir. �rne�in irsaliyeden-->faturaya
...
begin
  ...
  if ((Tur=15)and(EFaturaKullanimda>0))or((Tur=14)and(EIrsaliyeKullanimda)) then begin
     //YaziciYaz.Visible:=EFaturaKullanimda<>11; //e-arşiv varsa görünmeyecek
  ...
  //      ComboSENARYO.Properties.Items.Assign(RepItem.Items);
     ...
     if (UTSKullanimda)and(TabFatbaslik.FieldByName('EFATURADURUM').AsInteger in [1, 2, 21, 22]) then begin//efatura ise tıbbi cihaz da ekleyelim
        ...
        NewItem.Description := 'İlaç_TıbbiCihaz';
        NewItem.Value := 8;
    end;
...
//           cbStokDepo2.DataBinding.DataField := 'GIRISDEPO';
  ...
  if TabFatura.active then begin
     if (IslemOp = 'E')and(not TabFatura.IsEmpty) then begin
        ...
        if tur in [0, 3, 8, 10, 11, 12, 109] then
           i:=SubeId //geliş belgelerinde başlık firma görünmeli
        else
           i:=RehberId;
  ...
  //İrsaliyeden satış faturasına dönüşüm yapıldığında faturanoyu sıfırlaması için edit moduna geçirilir. before postta 0 atılır..
  ...
  //else if (IslemOp ='K')and(Tur in [15,16])then //Kopyalandıysa, efaturaya geçmiş mi diye bakalım
  ...
  if (Tur =12)and(IslemOp in ['E','K']) then //gelen fi�se ve ilk ekran ise
   ...
   if (IslemOp ='E')and((ComboFatTipi.EditValue=4)or(ComboFatTipi.EditValue=7)or(ComboFatTipi.EditValue=8)) then  //serbest meslek makbuzu ise bağtan stopaj� soral�m
...
begin
  if TabFatura.FieldByName('TUR').AsInteger=0 then begin//hizmet ise
    ...
    if Tablo.KodAgacindanSec(KodAgaciHizmetDlg,sqltext,False,False,False,True,HizID,HizKod,HizAciklama,slist,[nil,nil,Tablo.repStokAnaBirim,nil],[],[],['Kod','Açıklama','Birim','KDV'],[True,True,True,True],True) Then begin
       Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update FATURA set URUNID='+IntToStr(HizID)+'where ID='+TabFatura.FieldByName('ID').AsString,[],[]);
       TabloYenile(TabFatura, [TabFatbaslik.FieldByName('ID').AsInteger]);
    end;
...
begin
   if TabFatbaslik.FieldByName('TIPI').AsInteger=1 then
      ...
      MenuTipiIade.Caption := 'Tipini Alış/Satış Yap'
end;
...
begin
  ...
  if (Tur in [14, 15] )and(cbStokDepo.EditValue<>1) then begin //ilk satır eklendiğinde, konsinye Çıkış deposu ise sadece dönüşümden Çıkış yapılmalıdır
...
//  if Modul.Stok then begin
    ...
    //   AraDlg.cbStokDepo.Enabled := True // daha �nce depo se�imi yapİlmam��, yapİlabilir
...
begin
   {efat }
{ 01/11/2022 AO satıcı ve özelkod alanlarının değişebilmesi için burası iptal edildi
  if not(KilitKaldirildi)and(FATBASLIK.FieldByName('EFATURADURUM').AsInteger in [2, 52]) then begin  //iİlem gören fat veya giden e-irs ise iİlem yapİlamaz
...
begin
  ...
  if (TabFatbaslik.FieldByName('TIPI').AsInteger=2)and(TabFatbaslik.FieldByName('FATURATARIH').AsDateTime <= IadeOlanUrununSatisTarihi) then begin
     Showmessage('İade tarihi, satış tarihinden daha önce olamaz.');
     Abort;
  end;
...
{efat } if not(KilitKaldirildi)and(TabFatbaslik.FieldByName('EFATURADURUM').AsInteger in [2,52]) then begin
 ...
 {09/03/2025 AO Silme kontrolü dış silme ile ortak olsun diye utabloda yapılmaktadır
  if Veritabani.VeriVarMi(Tablo.FDCnn,'SELECT * FROM FATURA WHERE YERID ='+FATURA.FieldByName('ID').AsString, [], []) then begin
        Tablo.UyariGoster(Uyari, DonusumYapilmis);
        Abort;
  end;
 ...
 // SQLStr[23] := '	     (asd.Barkod like ''%<ara>%'')) ';
  ...
  if Tablo.ListedenBilgiGetir(HizmetUrunSec,SQLStr,st,[nil,nil,nil,nil,nil,nil,nil,Tablo.repStokAnaBirim,nil,nil,nil,nil,nil,nil,nil],'FaturaWizardHizmetUrunSec') then begin
    ...
    if FatSatirID>0 then begin
      ...
      //adeti 0 giriyoruz altta gerÇek adet girilecek amağ izlem bilgisini almaktür
      ...
      //if FATURA.FieldByName('IZLEME').AsInteger > 0  then
         //izleme var izlem satırlarını kopyalayalım.
         //YeniFatSatirID := Tablo.SatirKopyala('STOKIZLEME', YeniFatSatirID, 'SATIRID');
         //Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE STOKIZLE SET ADET=-1*ADET where IZLEMID='+,[],[]);
...
{         IzlemId := StrToIntDef(Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'insert into STOKIZLEME ([STOKID],[BELGETUR],[BASLIKID],[SATIRID],'+
         ...
         //daha �nce ��k�� konsinye ile anadepodan -> kons ��k�� deposuna atm��t�k
         Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE STOKIZLEMEDEPO SET ADET=-1*ADET where IZLEMID='+,[],[]); }
         // bu firmaya verilmiş konsinyelerin listeye gelmesi ve kullanıcının bunlardan seçmesi lazım
         //StokIzlemBilgisi;
...
begin
  ...
  if Tur in [14,15] then begin //çıkan irs veya fatura
     ...
     if (EFaturaKullanimda>0)or(EIrsaliyeKullanimda) then begin
         ...
         if EFat = -1 then //sorgulama yapİld�ysa
...
//AO 05.10.2025 �TS kullan�mda ve bildirimi olan Ürün varsa senaryo yu otomatik ilağ_t�bbicihaz yapacağz..
  if (UTSKullanimda) and (TabFatbaslik.FieldByName('TUR').AsInteger=15)and (TabFatbaslik.FieldByName('EFATURADURUM').AsInteger in [1,21]) then begin
     if Veritabani.VeriVarMi(Tablo.FDCnn,'select  F.ID, F.IZLEME, S.BILDIRIM from FATURA F inner join STOKLAR S on S.ID = F.URUNID '+
        ...
        if TabFatbaslik.FieldByName('SENARYO').AsInteger<>8 then begin  //ilağ t�bbicihaz de�ilse
           Tablo.TablodanSorguAc(1,'SELECT TOP 1 BILGI FROM REHBERBILGI RB (nolock) INNER JOIN REHBERAYAR RA (nolock) ON RA.YERI=2 and RA.SIRA=RB.SIRA '+
  ...
  //Satışta ekipmanlar varsa; müşteri cari kaydına eklenir //silmede de çıkarılır
  if TabFatbaslik.FieldByName('TUR').AsInteger = 15 then
     EkipmanKaydet;
  ...
  if (TabFatbaslik.FieldByName('TUR').AsInteger in[11,15]) and (not TabFatura.IsEmpty) then begin
    ...
    while not TabFatura.Eof do begin
      if (TabFatura.FieldByName('YERI').AsInteger = TabNo_DONUSUM_ALIS_IRS_FAT) or (TabFatura.FieldByName('YERI').AsInteger = TabNo_DONUSUM_SATIS_IRS_FAT) then
        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update FATURA set STOKDURUMDEGIS=STOKDURUMDEGIS where ID='+TabFatura.FieldByName('YERID').AsString,[],[]);
      TabFatura.next;
    end
...
begin
   if not Active then exit;//ekran hen�z ağİlmad�ysa ��ks�n
