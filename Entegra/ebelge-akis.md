...
- `additionalItemIdentifications` = `[{schemeID, itemIdentification}]` → **fatura/arşiv VE e-İrsaliye:** yalnızca **Senaryo=8** (ilaç/tıbbi cihaz) VE `TibbiCihazKimlik<>''`. (2026-07 düzeltme: e-İrsaliye eskiden "gerçek veri varsa" gönderiyordu ama SP ilaç-DIŞI ürünlerde de `AdditionalItemIdentification` döndürdüğünden ilaç olmayan irsaliyede de yazılıyordu → GİB'e gereksiz gidiyordu. Artık her iki yol da (yerel UBL + izibiz JSON) `Senaryo=8` ile gate.) schemeID tıbbi cihaz için `"TIBBICIHAZ"` (ilaç `"ILAC"`); değer `fn_Efatura_AdditionalItemIdentification` (`{(UNO)..(LNO)..(URT)..}` her adet). `TibbiCihazKimlikParcala` ile parçalanır.
...
- **Yerel UBL (`UBLXMLUret`) e-İrsaliye `DespatchLine/Item`**: fatura ile paritede `BrandName`, `ModelName`, `StandardItemIdentification`(GTIN=BARKOD), `AdditionalItemIdentification`(TIBBICIHAZ, **yalnız Senaryo=8**), `AdditionalItemProperty`(LOTNO) UBL şema sırasında yazılır — İTS/ilaç okuması **bu dış Item'dan** yapılır. İç `GoodsItem/InvoiceLine/Item` legacy'dir ve artık yalnız `Name` + `ItemInstance/SerialID` içerir (2026-07: `BrandName`/`ModelName` iç Item'dan KALDIRILDI — dış Item ile mükerrer geliyordu). Sürücü `DriverPerson`: `FirstName`+`FamilyName` DAİMA yazılır (GİB zorunlu, `SoforAdSoyadAyir` ile ad boşluktan ayrılır); kimlik no `NationalityID`'de.
