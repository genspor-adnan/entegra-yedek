...
// ---------------------------------------------------------------- verme ----
export function UtsVermeModali({ onKapat, onTamam }: {
  onKapat(): void;
  ...
  const [kurumNo, setKurumNo] = useState('');
  ...
  return (
    <Modal baslik="ÜTS Verme Bildirimi" dar onKapat={onKapat}
      alt={<>
      ...
      <div className="alan-izgara tek-sutun" style={{ padding: 10 }}>
        <UrunAlanlari uno={uno} setUno={setUno} seriNo={seriNo} setSeriNo={setSeriNo}
                      ...
                      kilitli={gonderiyor} />
        {/* Cari seçimi KUN'u kartından doldurur (225); elle de yazılabilir. */}
        <GenLookup
          kaynak="cari"
          etiket="Alan Cari (ÜTS kurum no'yu doldurur)"
          alanlar={[{ ad: 'unvan', baslik: 'Unvan', genis: true },
                    { ad: 'vkno', baslik: 'VKN' },
                    { ad: 'utsKurumNo', baslik: 'ÜTS Kurum No' }]}
          deger={cariUnvan || undefined}
          saltOkunur={gonderiyor}
          onSec={x => {
            setCariUnvan(String(x?.unvan ?? ''));
            const kun = String(x?.utsKurumNo ?? '').trim();
            setKurumNo(kun);
            setHata(kun ? '' :
              'Bu carinin ÜTS Kurum No’su boş — cari kartına girin ya da aşağıya elle yazın.');
          }}
        />
        <label className="alan">
          <span className="etiket zorunlu-isaret">Alan Kurum ÜTS No (KUN)</span>
          <input value={kurumNo} maxLength={30} disabled={gonderiyor}
                 onChange={e => setKurumNo(e.target.value)} />
        </label>
        <label className="alan">
          <span className="etiket zorunlu-isaret">Belge No (BNO)</span>
          <input value={belgeNo} maxLength={50} disabled={gonderiyor}
                 onChange={e => setBelgeNo(e.target.value)} />
        </label>
        <label className="alan">
          <span className="etiket">Gerçek İşlem Tarihi (GIT)</span>
