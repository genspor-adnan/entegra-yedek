...
{
    private const string SayfaAdresi = "https://www.titck.gov.tr/dinamikmodul/85";
    ...
    {
        var (adres, tarih) = await EnGuncelDosyaAsync(iptal);
    ...
    /// yüklenirdi.
    /// </summary>
    private async Task<(string Adres, string Tarih)> EnGuncelDosyaAsync(CancellationToken iptal)
    {
        var istemci = _http.CreateClient("katalog");
