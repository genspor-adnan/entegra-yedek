...
insert into public.yetki (kod, ad, grup, tur, sira) values
    ('fiyat_listesi',      'Fiyat listesi',  'stok', 0, 24),
    ...
    --   calistirabilmemeli.
    ('fiyat_listesi.uret', 'Fiyat listesini uret', 'stok', 1, 131)
...
select r.id, y.id, 1, 1, 1, 1
 ...
 where r.kod = 'yonetici' and y.kod in ('fiyat_listesi', 'fiyat_listesi.uret')
...
select r.id, y.id, 1
  from public.rol r cross join public.yetki y
 where r.kod = 'salt_okur' and y.kod = 'fiyat_listesi'
on conflict (rol_id, yetki_id) do nothing;

do $$
declare v integer;
begin
    select count(*) into v from public.yetki where kod like 'fiyat_listesi%';
    raise notice '203 tamam: % yetki (fiyat_listesi, fiyat_listesi.uret).', v;
end $$;
