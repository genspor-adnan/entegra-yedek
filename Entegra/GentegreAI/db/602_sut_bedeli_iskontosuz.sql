-- =====================================================================
--  602_sut_bedeli_iskontosuz.sql
--  SUT BEDELİ HİÇBİR ŞEKİLDE İSKONTOLANMAZ.
--
--  Kullanıcı: "SGK'da birim fiyat ve katkı değişmez. İskonto uygulanabilir
--  ama o da sadece katkıya uygulanır. SUT fiyatı hiçbir şekilde değişmez."
--
--  595 bunu ZATEN amaçlıyordu - kendi yorumu şöyle diyor:
--    "TTB/SUT'ta ise tarife ve SUT bedeli KURUMUN ödediğidir - hastaneyle
--     hasta arasındaki indirim kurumun ödemesini azaltamaz; indirim hastanın
--     cebinden çıkan katkıya işler."
--
--  Ama kod bu kuralı yalnız EKRANDAN KATKI GİRİLDİĞİNDE uyguluyordu:
--
--      v_sgk_liste := round(v_ham * (case when coalesce(v_katki_giren, 0) > 0
--                                         then p_miktar else v_carpan end), 2);
--
--  `v_katki_giren` yalnız ekrandan gelen katkıdır (p_katki_birim/p_katki_toplam);
--  katkı LİSTEDEN geliyorsa 0 kalıyor ve çarpan `v_carpan` (miktar x iskonto)
--  oluyordu. Sonuç: SGK'nın ödediği resmi bedel hastanenin indirimiyle
--  düşüyordu. Ölçülen (muayene 520030, SUT 85,73):
--
--      iskonto  0 -> SGK 85,73
--      iskonto 10 -> SGK 77,16      <-- SGK 8,57 TL eksik ödenmiş sayılıyor
--      iskonto 50 -> SGK 42,87
--
--  SUT bedeli SGK'nın mevzuatla belirlenmiş ödemesidir; hastanenin hastaya
--  yaptığı indirim onu değiştiremez. Çarpan artık koşulsuz MİKTARDIR.
--  İskonto yerinde duruyor - katkı dalında (v_ek) aynen işlemeye devam eder,
--  yani indirim hastanın ödediği kısma yazılır. Değişen TEK ŞEY bu satır.
-- =====================================================================
...
begin
    ...
    if p_sgk_liste_elle is not null then
        ...
        --   değiştiremez. Eskiden çarpan yalnız EKRANDAN katkı girildiğinde
        --   miktara sabitleniyordu; katkı listeden gelince iskonto SUT'a da
        --   işliyor ve SGK payı eksik hesaplanıyordu.
...
comment on function public.fn_dagilim_coz is
  'Satirin kova dagilimi. SUT bedeli ISKONTOSUZDUR (602): iskonto yalniz hasta '
  'ek katkisina isler, SGK''nin odedigi bedeli degistirmez.';
