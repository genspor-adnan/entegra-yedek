-- ============================================================================
--  Gentegre AI — Dip toplam: KDV matrahi 2. iskontoyu da icersin
--  095_diptoplam_kdv_iskonto2.sql   (024'un duzeltilmis hali)
--
--  HATA: KDV satiri matrahi "(birim_fiyat*adet)*(100-iskonto)/100" ile
--  hesapliyordu - IKINCI iskonto (belge_satir.iskonto2) matraha girmiyordu.
--  Ara toplam ve iskonto satirlari iskonto2'yi zaten iceriyordu, dolayisiyla
--  iki kademeli iskontolu her belgede KDV (ve genel toplam) FAZLA cikiyordu.
--  Ornek: 13 x 65,00 %10+%5 -> matrah 722,48 ama KDV 76,05 (72,25 olmaliydi).
--
--  DUZELTME: KDV matrahi artik satirin kendi TUTAR kolonundan okunur
--  (belge_satir.tutar / doviz_tutari); bu deger BelgeHesap.SatirTutari ile
--  yazilir ve iki iskontoyu da icerir - tek formul ilkesi.
--
--  Mevcut belgelerin kayitli toplamlari DEGISTIRILMEZ (kesilmis belge yeniden
--  hesaplanmaz); fonksiyon bundan sonraki hesaplamalarda dogru calisir.
-- ============================================================================
        ...
        SELECT
            ...
            CASE
                        WHEN COALESCE(NULLIF(FB.belge_dovizi,''),'TL')  = 'TL' THEN
                            ...
                            ELSE CAST((kdv*(F.tutar/100.0)) AS numeric(18,6)) END)::numeric,2)
                        ELSE
                            ROUND((SUM(CASE WHEN kdv_durum ='Dahil' THEN F.doviz_tutari-((F.doviz_tutari)*(100.0/(100.0+kdv)))
                                    ...
                                    ELSE CAST((kdv*((case when F.doviz_cinsi=FB.rapor_dovizi then F.doviz_tutari else F.tutar/NULLIF(FB.doviz_kuru,0) end)/100.0)) AS numeric(18,6))  END)::numeric),2)* FB.doviz_kuru
                    END AS deger,
            ROUND((SUM(CASE WHEN kdv_durum ='Dahil' THEN F.doviz_tutari-(F.doviz_tutari*(100.0/(100.0+kdv)))
                                    when otv_miktar>0.0 then (((case when F.doviz_cinsi=FB.rapor_dovizi then F.doviz_tutari else F.tutar/NULLIF(FB.doviz_kuru,0) end)+
                                    ((CASE WHEN F.otv_yuzde =0 THEN adet*otv_miktar ELSE (otv_miktar*(((case when F.doviz_cinsi=FB.rapor_dovizi then F.doviz_birim_fiyat else F.birim_fiyat/NULLIF(FB.doviz_kuru,0) end)*F.adet)/100.0))  END)))*kdv/100.0)
                                    ELSE CAST((kdv*((case when F.doviz_cinsi=FB.rapor_dovizi then F.doviz_tutari else F.tutar/NULLIF(FB.doviz_kuru,0) end)/100.0)) AS numeric(18,6))  END)
                                    )::numeric
                                    ,2) AS doviztutari,
            FB.kur AS kur,
