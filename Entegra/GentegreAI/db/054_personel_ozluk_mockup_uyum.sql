...
alter table public.taraf_personel add column if not exists sgk_sicil_no character varying(30) not null default '';
alter table public.taraf_personel add column if not exists meslek_kodu character varying(60) not null default '';
alter table public.taraf_personel add column if not exists yonetici_taraf_id integer references public.taraf(id);
