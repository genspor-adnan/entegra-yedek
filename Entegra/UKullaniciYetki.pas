...
begin
  Tablo.Query3.Close;
  Tablo.Query3.SQL.Text := 'declare @RolID int, @ModulID nvarchar(20) ';
  Tablo.Query3.SQL.Add('set @RolID='+TabRol.FieldByName('ID').AsString+' ');
  Tablo.Query3.SQL.Add('set @ModulID='''+TabModul.FieldByName('MODULID').AsString+''' ');
  Tablo.Query3.SQL.Add('delete from YETKI where ROLID = @RolID and MODULID like @ModulID+''%'' ');
  Tablo.Query3.SQL.Add('insert into YETKI(ROLID,MODULID,HAK,TUR) ');
  Tablo.Query3.SQL.Add('select @RolID,MODULID,1,1 from MODUL M where MODULID like @ModulID+''%'' and M.TUR in(1,2,3,4,6,7,8,9,16,17,18,19) ');
  Tablo.Query3.SQL.Add('union all ');
  Tablo.Query3.SQL.Add('select @RolID,MODULID,1,2 from MODUL M where MODULID like @ModulID+''%'' and M.TUR in(2,4,7,9,17,19) ');
  Tablo.Query3.SQL.Add('union all ');
  Tablo.Query3.SQL.Add('select @RolID,MODULID,1,3 from MODUL M where MODULID like @ModulID+''%'' and M.TUR in(3,4,8,9,18,19) ');
  Tablo.Query3.SQL.Add('union all ');
  Tablo.Query3.SQL.Add('select @RolID,convert(int,(convert(nvarchar(4),M.MODULID)+convert(nvarchar(10),D.ID))),1,1  ');
  Tablo.Query3.SQL.Add('from MODUL M inner join DOKUMLER D on M.DOKUMTUR=D.MODUL  ');
  Tablo.Query3.SQL.Add('where LEN(M.MODULID)=4 and M.MODULID like ''__99'' and MODULID like @ModulID+''%'' ');
  Tablo.Query3.SQL.Add('union all ');
  Tablo.Query3.SQL.Add('select @RolID,MODULID=convert(bigint,(convert(varchar(4),M.MODULID)+convert(varchar(10),-R.ID))),1,1 ');
  Tablo.Query3.SQL.Add('from MODUL M inner join REHBER R on R.ID<0 and R.DURUM> 0 ');
  Tablo.Query3.SQL.Add('where LEN(M.MODULID)=4 and M.MODULID like ''__98'' and MODULID like @ModulID+''%'' ');
  Tablo.Query3.SQL.Add('union all ');
  Tablo.Query3.SQL.Add('select @RolID,MODULID=convert(bigint,(convert(varchar(4),M.MODULID)+convert(varchar(10),''0''))),1,1 ');
  Tablo.Query3.SQL.Add('from MODUL M ');
  Tablo.Query3.SQL.Add('where LEN(M.MODULID)=4 and M.MODULID like ''__98'' and MODULID like @ModulID+''%'' ');
  Tablo.Query3.SQL.Add('union all ');
  Tablo.Query3.SQL.Add('select @RolID,convert(int,(convert(nvarchar(4),M.MODULID)+convert(nvarchar(10),D.ID))),1,1    ');
  Tablo.Query3.SQL.Add('from MODUL M inner join DEPOLAR D on D.DURUM >0 --M.DOKUMTUR=D.MODUL  ');
  Tablo.Query3.SQL.Add('where LEN(M.MODULID)=4 and MODULID like @ModulID+''%'' ');
