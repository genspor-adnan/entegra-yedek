...
begin
   if EkleDetay then
      Ekle(TabDetay,TabNo_SERVIS, ServisID, 'Değiş');
end;
...
begin
  if (TabDetay.Active)  then begin
    ...
    if (i>0) and (Application.MessageBox(PChar(PWHepsiSilinecektirUyari),pchar(Uyari), MB_YESNO + MB_ICONWARNING)=mrNo) then begin
      ...
      TabServis.Post;
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from REHBERBILGI where YERI=&Yeri and YER_ID=&YerID ',['&Yeri','&YerID'],[TabNo_SERVIS,ServisID]);
      DetayEkrPage(Self);
    end;
  end;
end;
...
//        Tablo.LogIslemleri(DatasetinTabNosunuBul(TabServisUygulama),(TabServisUygulama.FieldByName('ID').AsInteger),5,TabServisUygulama);
     Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, ' delete from SERVISBILGI where ID=&id ',['&id'],[TabGenel.FieldByName('ID').AsInteger]);
     TabloYenile(TabGenel,[ServisID])
  end;
end;
...
begin
  ...
  TabloGirisDlg.Caption := 'Geçmiş çözümler';
...
begin
  Tablo.GridYorumCellDblClick(Sender,ACellViewInfo,AButton,AShift,AHandled, TabNo_SERVISHAREKET);
...
begin
  ...
  if (Shift = [ssAlt,ssCtrl]) and (Key = Ord('T')) then  begin   //Yeni Bile?en Ekle
    ...
    if TGirisKutusuEx.BilgiAlEx('Tarih', TGirdiDenetimleri.Create.DateTimePicker('Tarih:', @Tarih, dtkDate)) = mrOk then begin
         Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE GOREVYORUM SET EKLEMETARIHI='''+Formatdatetime('yyyy-mm-dd hh:nn', Tarih)+''' WHERE ID='+TabYorum.Fields[0].AsString, [], []);
...
begin
   ...
   if TGirisKutusuEx.BilgiAlEx(TabGenel.FieldByName('AD').AsString , TGirdiDenetimleri.Create.Memo('Açıklama' , @Bilgi)) = mrOk then begin
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, ' update SERVISBILGI set ACIKLAMA=&czm where ID=&id ',['&czm','&id'],
...
begin
   ...
   Bilgi := vartostr( (Sender as TcxButtonEdit).EditingValue );
   if TGirisKutusuEx.BilgiAlEx(TabGenel.FieldByName('AD').AsString , TGirdiDenetimleri.Create.Memo('çözüm' , @Bilgi)) = mrOk then begin
      TabGenel.Edit;
...
begin
  ...
  if State = dsDragLeave then begin
    ...
    if TreeHitTest.HitAtBackground then begin
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'update SERVISBILGI set USTID = null where ID='+TabGenel.FieldByName('ID').AsString,[],[]);
      TabloYenile(TabGenel,[ServisID]);
      Accept := False;
    end;
  end;
...
begin
  ...
  if Tip=1 then begin//i? personel
    ...
    DetayStr := DetayStr + '<b>  Müşteri : </b> '+Tablo.AciklamaGetir('REHBER','FIRMA',TabServis.FieldByName('REHBERID').AsInteger)+'<br/>';
    if StrToIntDef(TabServis.FieldByName('MUS_ILGILI').AsString,0) > 0 then
      DetayStr := DetayStr + '<b>  ılgili : </b> '+Tablo.AciklamaGetir('REHBERPERSONEL','ADSOYAD',TabServis.FieldByName('MUS_ILGILI').AsInteger)+'<br/>';

  end;
  DetayStr := DetayStr + '<b>  Konusu : </b> '+tabServis.FieldByName('KONUSU').AsString+'<br/>';
  DetayStr := DetayStr + '<b>  Durum : </b> '+VarToStr(Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'select ANAHTAR from GENINI where BOLUM=-3007 and DEGER='+TabHareketler.FieldByName('DURUM').AsString+' and DIL=-1',[],[],True))+'<br/>';
  ...
  if not TabGenel.IsEmpty then begin
    ...
    while not TabGenel.Eof do begin
      ...
      if TabGenel.FieldByName('ACIKLAMA').AsString <> '' then
        DetayStr := DetayStr + '<b> Açıklama : </b><i>'+TabGenel.FieldByName('ACIKLAMA').AsString+'</i>'+'<br/>';
      if TabGenel.FieldByName('COZUM').AsString <> '' then
        DetayStr := DetayStr + '<b> çözüm    : </b><i>'+TabGenel.FieldByName('COZUM').AsString+'</i>'+'<br/>';
      TabGenel.Next;
    end;
  end;
...
begin
  ...
  if islemOp='E' then begin
    Tablo.TablodanSorguAc(6,'select '+DbUst(1)+'DEGER from GENINI where BOLUM=-3007 order by DEGER '+DbSinir(1));
    if not Tablo.Query6.IsEmpty then begin
      TabloYenile(TabHareketler,[ServisID]);
      if TabHareketler.IsEmpty then begin
        Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'insert into SERVISHAREKET(SERVISID,BASLAMA,DURUM,EKLEYEN,ACILIS,PERSONEL)'+
...
begin
   SipariOlutur1.Visible := ((not TabServisBelge.IsEmpty) and (TabServisBelge.FieldByName('TUR').AsInteger = 80)and((TabServisBelge.FieldByName('HEDEF').AsString='Verilen Sipariş ')or(TabServisBelge.FieldByName('HEDEF').AsString='')));
   VerSipariineDntr1.Visible := ((not TabServisBelge.IsEmpty) and (TabServisBelge.FieldByName('TUR').AsInteger = 80)and((TabServisBelge.FieldByName('HEDEF').AsString='Alınan Sipariş')or(TabServisBelge.FieldByName('HEDEF').AsString='')));
