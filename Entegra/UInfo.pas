...
begin
  ...
  TabLog.SQL.Text :=
    ...
    ' KOD = MAX(COALESCE(CASE WHEN L.USTTABLOID IN (315,316,318,319,144) THEN NULL ELSE NULLIF(LRk.KOD,'''') END, NULLIF(LRc.KOD,''''), NULLIF(LRs.KOD,''''), NULLIF(LRuf.KOD,''''))),' +
    ' AD  = MAX(COALESCE(CASE WHEN L.USTTABLOID IN (315,316,318,319,144) THEN NULL ELSE NULLIF(LRk.AD,'''')  END, NULLIF(LRc.AD,''''),  NULLIF(LRs.AD,''''),  NULLIF(LRuf.AD,''''))),' +
    ' ADET = COUNT(*)' +
    ' FROM ISLEMLOG L' +
