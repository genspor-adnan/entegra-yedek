...
begin
  ...
  TabLog.SQL.Text :=
    ...
    ' KOD = MAX(CASE WHEN L.TABLOID=L.USTTABLOID THEN COALESCE(CASE WHEN L.USTTABLOID IN (315,316,318,319,144) THEN NULL ELSE NULLIF(LRk.KOD,'''') COLLATE DATABASE_DEFAULT END, NULLIF(LRc.KOD,'''') COLLATE DATABASE_DEFAULT, NULLIF(LRs.KOD,'''') COLLATE DATABASE_DEFAULT, NULLIF(LRuf.KOD,'''') COLLATE DATABASE_DEFAULT) END),' +
    ' AD  = MAX(CASE WHEN L.TABLOID=L.USTTABLOID THEN COALESCE(CASE WHEN L.USTTABLOID IN (315,316,318,319,144) THEN NULL ELSE NULLIF(LRk.AD,'''') COLLATE DATABASE_DEFAULT END, NULLIF(LRc.AD,'''') COLLATE DATABASE_DEFAULT, NULLIF(LRs.AD,'''') COLLATE DATABASE_DEFAULT, NULLIF(LRuf.AD,'''') COLLATE DATABASE_DEFAULT) END),' +
    ' ADET = COUNT(*)' +
    ' FROM ISLEMLOG L' +
