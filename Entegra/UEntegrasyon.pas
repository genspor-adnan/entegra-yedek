...
begin
   ...
   if CNNAktarilacak.Connected then begin
     LabelBaglanti.Caption:='Bağlantı Başarılı.';
     cxGroupBox3.Enabled:=True;
   end else begin
     LabelBaglanti.Caption:='Bağlantı Başarısız!';
...
begin
  if cxImageComboBox1.Text = 'Genotıpdan Rehber Al' then  begin
  ...
  end else  if cxImageComboBox1.Text = 'Genotıpa Rehber Ver' then  begin
    TabAktarilacak.first;
    Tablo.Query1.Close;
    Tablo.Query1.Connection:=CNNAktarilacak;
    Tablo.Query1.SQL.Text:='SELECT KOD FROM REHBER';
...
begin
   ...
   bilgi:='.';
   ctrls := TGirdiDenetimleri.Create.Edit('Aktarım Yapılacak Server Adını Giriniz.',@bilgi);
   if TGirisKutusuEx.BilgiAlEx('Server Adı veya IP Adresi:',ctrls) = mrOK then begin
     if Trim(bilgi) = '' then begin
        MessageDlg(('Server Adı Boş Olamaz.'),mtError,[mbOK],0);
        Exit;
     End else begin
        ServerName := bilgi;
        bilgi:='GEN2005';
        ctrls := TGirdiDenetimleri.Create.Edit('Aktarım Yapılacak Veritabanı Adını Giriniz.',@bilgi);
       if TGirisKutusuEx.BilgiAlEx('Veritabanı Adı:',ctrls) = mrOK then begin
         if Trim(bilgi) = '' then begin
            MessageDlg(('Veritabanı Adı Boş Olamaz.'),mtError,[mbOK],0);
            Exit;
         End else begin
            DBName := bilgi;
            bilgi:='SA';
           ctrls := TGirdiDenetimleri.Create.Edit('Kullanıcı Adını Giriniz.',@bilgi);
           if TGirisKutusuEx.BilgiAlEx('Kullanıcı Adı:',ctrls) = mrOK then begin
             if Trim(bilgi) = '' then begin
             ...
             End else begin
               UserName := bilgi;
               bilgi:='GENOTIP';
               ctrls := TGirdiDenetimleri.Create.Edit('Aktarım Yapılacak Server Şifresini Giriniz.',@bilgi);
               if TGirisKutusuEx.BilgiAlEx('Server Şifresi('+UserName+'):',ctrls) = mrOK then begin
                  PassWord := bilgi;
