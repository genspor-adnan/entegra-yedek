...
-- ROW_NUMBER() ile kesin ve mutlak tekilleştirme yapıyoruz. 
-- Her ID icin sadece En Guncel Islem Tarihli (DEGISTIRMETARIHI DESC) 1 SATIR gelecek.
SELECT 
    ID, KOD, FIRMA, ADSOYAD, FATBASLIK, GRUP, TEMAS, SEKTOR, 
    KATEGORI, SINIF, DURUM, OZELKOD, TEMSILCIAD, DEGISTIRMETARIHI
FROM (
    SELECT 
        R.ID, 
        R.KOD, 
        ...
        RowNum = ROW_NUMBER() OVER(PARTITION BY R.ID ORDER BY K.DEGISTIRMETARIHI DESC)
