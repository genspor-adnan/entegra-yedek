...
var
  ...
  SeriTakipislemturu : string;     // G : giriþ, C: Çýkýþ, D: Düzelt , S:Sil  , GD: GiriþDüzel, CD : Çýkýþ Düzelt
...
function YeterliSeriNoSecildi:Boolean;
 begin
      ...
      if lblKayitSayisi.Caption<>lblGerekliSayi.Caption then
       begin
         HataliSeriNoMemoyaEkle('','Seçilen Seri Numarasý Sayýsý gereken sayýdan farklý, lütfen kontrol ediniz');
         Result:=False;
       end;
 end;  //Eklendi
     ...
     for i := 0 to memoSeriNolar.Lines.Count - 1 do
        begin
           if  (memoSeriNolar.Lines.IndexOf(memoSeriNolar.Lines[i])>=0) and (memoSeriNolar.Lines.IndexOf(memoSeriNolar.Lines[i])<>i) then
              begin
                Result:=True;
                HataliSeriNoMemoyaEkle(memoSeriNolar.Lines[i],'Seri Numarasý Listesinde tekrar eden seri numaralarý var, Lütfen kontrol ediniz');
//                abort;
              end;
        end;
   end;  //eklendi
  ...
  for i := 0 to memoSeriNolar.Lines.Count - 1 do
   begin
      ...
      if Tablo.Query5.RecordCount>=1 then
       begin
         Result:=True;
         HataliSeriNoMemoyaEkle(memoSeriNolar.Lines[i],'Ayný ürün için listedeki seri numaralarý daha önce kullanýlmýþtýr. Lütfen kontrol ediniz');
       end;
   end;
end;   //Eklendi
  ...
  if SeriTakipislemturu= 'G' then
   begin // Yeni Seri No Giriþi
      if memoSeriNolar.Lines.Count <> SeriTakipSeriNosayisi then
       begin
         HataliSeriNoMemoyaEkle('','Listedeki Seri Numarasý sayýsý gereken sayýdan farklý, lütfen kontrol ediniz');
         abort;
       end;
...
begin
  if Key = 38 then
   ...
   begin
      ...
      if SeriTakipislemturu = 'C' then // çýkýþý yapýlmamýþ seri numaralarý
       tabSeriNoListesi.SQL.Add(' AND ISNULL(CIKISTURU,0) = 0 AND ISNULL(CIKFATURAID,0) = 0')
      else if (SeriTakipislemturu = 'CD') and ( SeriTakipCagiranTur in [14, 15, 16] ) then // çýkýlan seri nolar üzerinde düzeltme yapýlacaksa
       tabSeriNoListesi.SQL.Add(' AND ISNULL(CIKISTURU,0) > 0 AND ISNULL(CIKFATURAID,0) = '+inttostr(SeriTakipCagiranSatirId))
      else if (SeriTakipislemturu = 'GD') and ( SeriTakipCagiranTur in [7, 10, 11, 12] ) then // Girilen seri nolar üzerinde düzeltme yapýlacaksa
  ...
  lblGerekliSayi.Caption:= IntToStr(SeriTakipSeriNosayisi);
{
  if (SeriTakipislemturu = 'GD') or (SeriTakipislemturu = 'CD') then
   begin
     btnSerinoKaydet.Caption:='Seçilen Numaralarý Çýkar';
     editSeriNoKeyUp(Self,k,[]);
   end
  else
   btnSerinoKaydet.Caption:='Seri Numaralarýný Kaydet';
}
