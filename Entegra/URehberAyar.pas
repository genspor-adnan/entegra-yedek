...
begin
  Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from REHBERAYAR where YERI=&Yeri and BOLUM=&Bolum AND SUBEID='+IntToStr(SubeId)+' ',['&Yeri','&Bolum'],[nereye,bolum]);
  Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'insert into REHBERAYAR(YERI,SIRA,ETIKET,GIRIS,KAYNAK,VARSAYILAN,ZORUNLU,BOLUM,SUBEID) '
...
begin
   i:=Yer;
   if i=4 then //Kurum ve pers iletişim aynı olacak
      i:=1;
...
begin
  ...
  if GridAyarViewGIRIS.EditValue=10 then
    GridAyarViewKAYNAK.EditValue:='9\(999\)999 99 99';//'0\(000\)000 00 00' --Eksik no girildiğinde yada boş olduğunda hata vermesi engellendi.
end;
...
begin
   //Varsayılan kısmı dolu ise ve data girilmişse silinemez ve SIRA ve Varsayılan kısmı değiştirilemez
...
begin
  ...
  if (TabAyar.FieldByName('GIRIS').AsInteger in [4,6,8])and(TabAyar.FieldbyName('KAYNAK').AsString = '') then
    Mesaj := Mesaj + 'KAYNAK ';
  if Mesaj <> '' then begin
    Mesaj := Mesaj + 'Alanları boş bırakılamaz.';
    ShowMessage(Mesaj);
    Abort;
  end;
end;
