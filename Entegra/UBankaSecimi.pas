          ...
          if (VarToStr(BankaKodu)<>'')and(VarToStr(BankaAdi)<>'') then begin
              ...
              if not Tablo.Query1.IsEmpty then begin
                 ...
                 if MResult = mrYes then
                    Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE BANKALAR SET BankaADI=&BankaADI '+
...
begin
   ...
   if not Tablo.Query1.IsEmpty then
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'delete from BANKALAR where BANKAKODU= ' + TabBankalar.FieldByName('BANKAKODU').AsString,[],[]);
      TabloYenile(TabBankalar,[]);
   end;
end;
...
//   kur:= ADOQuery2.FieldByName('DEGER').AsString;
    ...
    with nd.ChildNodes.FindNode('banka').ChildNodes do begin
      ...
      try
         ...
         Memo1.Lines.Add(rssFeedData.BKOD+','+Trim(rssFeedData.NAME)+' aktarılamadı..');
      end;
    end;
    ...
    while nd.ChildNodes.FindSibling(nd, I) <> nil do begin
      ...
      try
         ...
         Memo1.Lines.Add(rssFeedData.BKOD+','+rssFeedData.SKOD+','+Trim(rssFeedData.NAME)+','+rssFeedData.CITY+' aktarılamadı..');
      end;
      Inc(I)
    end;
  end;
...
begin
  ...
  while Donus do begin
    if not TabBankalar.FieldByName('BANKAKODU').IsNullOrEmpty then begin
      Denetimler  := TGirdiDenetimleri.Create.Edit(BGSube_Kod,@SubeKodu).Edit(BGSube_Ad,@SubeAdi).ComboBox('ıl *',@Il,Tablo.ComboboxInit('SELECT ILADI FROM ILLER where ILNO<100 ').Items,TComboBoxStyle.csDropDownList);
        ...
        if (SubeAdi <> '') and (SubeKodu <> '') and (Il <> '') then begin
          ...
          if KayitKontrol then begin
            ...
            if MResult = mrYes then begin
              Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE BANKASUBELER SET SUBEADI=&SUBEADI '+
            ...
            Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'INSERT INTO BANKASUBELER(SUBEADI,SUBEKODU,BANKAKODU,ILNO) '+
