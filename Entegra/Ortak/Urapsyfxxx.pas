...
uses
  ...
  Printers, ExtCtrls, ComCtrls, DB, UFDCompatHelpers, quickrpt, JPEG, EanQr, EanSpecs, {DBarcode,}
  {DBarQrp,}QRExport,  Chart, UPreview,Barcode,
   frxClass, ConverterQR2FR, Qrctrls,QRPDFFilt,QRBarCode;
//  UQRDBAngledLine; {TeeProcs, TeEngine, Chart, DBChart}
const
  BandSayisi = 30;
type
  TRapSyf = class(TForm)
  ...
  public
    ...
    function DokumYap(mPrevMi1: smallint; mEkranAdi: string;BuRaporuKullan: TQuickRep = nil): integer;
...
{  Mail antet ekleme olayının Başlangıcı................ }


procedure ResimBirlestir(t: TCanvas; x, y: Integer; s: TBitmap; TrCol: TColor);
var
...
procedure RaporuJpegOlarakKaydet(QuickRep: TQuickRep; Klasor,DosyaAdi: string);
var
  JPG: TJPEGImage;
  BMP: TImage;
  mf: TMetafile;
...
//  bmp.Height := 0;
  ...
  begin
    ...
    BMP.Picture.SaveToFile(DosyaAdi + '-' + inttostr(i) + '.bmp');
    RapSyf.imgRapor.Picture.LoadFromFile(DosyaAdi + '-' + inttostr(i) + '.bmp');

  // birleştirme operasyonu

  // Üst Logo katmak için...
    if mailantetust <> '' then
    begin
      RapSyf.imgRapor.Picture.LoadFromFile(DosyaAdi + '-' + inttostr(i) + '.bmp');
      Rapsyf.imgLogo.picture.LoadFromFile(mailantetust);

      rap := Tbitmap.Create;
      rap.width := RapSyf.imgRapor.width;
      rap.height := RapSyf.imgRapor.height;
      rap.assign(RapSyf.imgLogo.picture);
      ResimBirlestir(rapsyf.imgRapor.Canvas, 1, mailantetustyer, rap, clr);
      RapSyf.imgRapor.Picture.SaveToFile(DosyaAdi + '-' + inttostr(i) + '.bmp');
    end;

  // Alt Logo eklemek için...
    if mailantetalt <> '' then
    begin
      RapSyf.imgRapor.Picture.LoadFromFile(DosyaAdi + '-' + inttostr(i) + '.bmp');
      Rapsyf.imgLogo.picture.LoadFromFile(mailantetalt);

      rapalt := Tbitmap.Create;
      rapalt.width := RapSyf.imgRapor.width;
      rapalt.height := RapSyf.imgRapor.height;
      rapalt.assign(RapSyf.imgLogo.picture);
      ResimBirlestir(rapsyf.imgRapor.Canvas, 1, mailantetaltyer, rapalt, clr);
      RapSyf.imgRapor.Picture.SaveToFile(DosyaAdi + '-' + inttostr(i) + '.bmp');
    end;

    JPG.Assign(RapSyf.imgRapor.Picture.Bitmap);

 // orj  -- JPG.Assign(bmp.picture.bitmap);

    if not DirectoryExists('.\'+Klasor) then Createdir('.\'+Klasor);
...
{  Mail antet ekleme olayının sonuuu................ }
...
//sayıtoyazı
  TabKosBic.Close;
  TabKosBic.CommandText := 'Select * from AYARLAR where RAPORADI=''' + Raporadi + ''' and ALANTURU=''SABİT'' and ISNULL(TABLO,'''')<>''''';
  TabKosBic.Open;
...
//barcode lar...
  ...
  begin
    BulTable := TabloBul(TabKosBic.FieldByName('TABLO').AsString);
    TQrEan(FindComponent('B' + TabKosBic.fieldbyname('SIRANO').AsString)).BarCode := TDataSet(BulTable).fieldbyname(TabKosBic.fieldbyname('ALANADI').AsString).AsString;
    TabKosBic.Next;
  end;
...
begin
  ...
  if TabloAd = 'SORGU' then
    ...
    for k := 0 to RapTabloGost.ComponentCount - 1 do
      if (RapTabloGost.Components[k] is TDataSet) then
        if UpStr(RapTabloGost.Components[k].Name) = (UpStr(TabloAd)) then begin
...
function TRapSyf.DokumYap(mPrevMi1: smallint; mEkranAdi: string;BuRaporuKullan: TQuickRep = nil): integer;
var
  Renk: string[30];
...
{          Else If Tur='ZARF-9 9.84X24.13CM' Then
  ...
  var
    prnName : string;
  begin
    printeradi := GenRegIni.RegReadString('Yazıcılar', RapTabAYAR.FieldByName('RAPORADI').AsString + '_Yzc', '', RgstryLC);
    SayfaAdi := GenRegIni.RegReadString('Yazıcılar', RapTabAYAR.FieldByName('RAPORADI').AsString + '_Syf', '', RgstryLC);
    if SayfaAdi <> '' then
    begin
    ...
    else if RapTabAYAR.FieldByName('TABLO').AsString = 'BOŞLUK' then
    begin
    ...
    begin
      ...
      if (mPrevmi = 1) and (Uppercase(RapTabAYAR.FieldByName('OZELLIK').AsString) = 'EKRAN') then
      begin //Print Dialog çağrılıyor mu?
  ...
  begin
    ...
    begin
      if RapTabAYAR.FieldByName('TABLO').AsString = 'GRUPBAŞI' then
        ...
        Tur := RbGroupFooter
      else if RapTabAYAR.FieldByName('TABLO').AsString = 'SAYFABAŞI' then
      begin
        Tur := RbPageHeader;
      ...
      else if RapTabAYAR.FieldByName('TABLO').AsString = 'RAPORBAŞI' then
      begin
        Tur := RbTitle;
        RapSyf.RaporSyf.Options := RapSyf.RaporSyf.Options - [FirstPageHeader];
      end
...
//          Picture.Bitmap.LoadFromFile(yol);     := GlobalImage.Picture.Bitmap;
        ...
        if (RapTabAYAR.FieldByName('OZELLIK').AsString <> 'AutoSize') then begin
          if RapTabAYAR.FieldByName('EN').AsString <> '' then
            Size.Width := Round(RapTabAYAR.FieldByName('EN').AsFloat * 10)
          else
...
//               Dt:=qrsColumnNo
  ...
  procedure LogoAyarlar;
    function AlanBilgiGetir : String;
    var
      BulTable: TComponent;
      DataSet : TADODataSet;
    begin
       BulTable := TabloBul(RapTabAYAR.FieldByName('TABLO').AsString);
       Dataset := TADODataSet(BulTable);
       Result := Dataset.FieldByName(RapTabAYAR.FieldByName('ALANADI').AsString).AsString
    end;
  var
    QCntrlType: TQRNewComponentClass;
    QCntrl: TQRImage;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          if RapTabAYAR.FieldByName('TABLO').AsString <> '' then
             s := AlanBilgiGetir
          else
             s := RapTabAYAR.FieldByName('ALANADI').AsString;
   ...
   begin
      ...
      if Bnd <> nil then begin
         ...
         if BulTable <> nil then begin
            ...
            //Burayı iptal ettik before printte text ine değer atıyoruz
...
//            else if st = 'UPCE' then TQrBarcode(QCntrl).BarTip := bcUPCE0;
        ...
        with TQrBarcode(QCntrl) do begin
          Name := 'B' + RapTabAYAR.FieldByName('SIRANO').AsString;
          //Burda bilgiyi alıp texte atalım
          ...
          ShowText := pos('BAŞLIK',RapTabAYAR.fieldbyname('OZELLIK').AsString)>0;

          i := pos('Modül:', RapTabAYAR.FieldByName('OZELLIK').AsString);
          if i > 0 then //Ratio:3 veya 2 Çizgi kalınlıklarını belirtir..
...
//    MyRect, MyOther: TRect;
  ...
  begin
    ...
    begin
      ...
      begin
        QCntrlType := TQrEan;
        QCntrl := TqrEan(Bnd.AddPrintable(QCntrlType));
{
...
//            ShowMessage(findcomponent('Dts'+RapTabAYAR.FieldByName('TABLO').AsString).Name);
   //         QCntrl := TQRCustomLabel(Bnd.AddPrintable(QCntrlType));
        ...
        st := Uppercase(Trim(st));
        if st = 'CODABAR' then
          TQREan(QCntrl).TypBarCode := bcCodabar
        else if st = 'CODE128A' then
          TQREan(QCntrl).TypBarCode := bcCode128
//            else if st = 'CODE128B' then TDBEan(QCntrl).TypBarCode := bsCode128B
//            else if st = 'CODE128C' then TDBEan(QCntrl).TypBarCode := bsCode128C
        else if st = 'CODE25INTERLEAVED' then
          TQREan(QCntrl).TypBarCode := bc25Interleaved
        else if st = 'CODE39' then
          TQREan(QCntrl).TypBarCode := bcCode39Full
        else if st = 'EAN13' then
          TQREan(QCntrl).TypBarCode := bcEan13
        else if st = 'EAN8' then
          TQREan(QCntrl).TypBarCode := bcEAN8
        else if st = 'UPCA' then
          TQREan(QCntrl).TypBarCode := bcUPCA
        else if st = 'UPCE' then
          TQREan(QCntrl).TypBarCode := bcUPCE0;
      end;
      with TQREan(QCntrl) do
      begin
        Name := 'B' + RapTabAYAR.FieldByName('SIRANO').AsString;
        left := Round(RapTabAYAR.FieldByName('SOL').AsFloat * 10);
        top := Round(RapTabAYAR.FieldByName('UST').AsFloat * 10);
  ...
  begin
   ...
   {TODO -oDeveloper -cURapSyf : Tablo döküme ihtiyaç duyuluyor ?! }
   //FastRaporDlg.frxReport1.DataSets.Add(TabloDokum.frxSQLKomut);
   conv := TConverterQr2Fr.Create;
   conv.Source := RapSyf.RaporSyf;
   conv.Target := FastRaporDlg.frxReport1;
   conv.Convert;

   FastRaporDlg.RaporKaydet(FastRaporDlg.frxReport1);
//   frxReport1.SaveToFile('c:\converted_fromQR.fr3');
//   frxReport1.LoadFromFile('c:\converted_fromQR.fr3');
...
//   AExportFilter : TQRRTFExportFilter;
  PDFExporter : TQRPDFDocumentFilter;
begin
//  KopyaSay := 1;
  mPrevmi := mPrevmi1;
  Dokumyap := 0;
...
//    RaporSyf.PrinterSettings.PrintMetaFile := true;
    RapSyf.RaporSyf.Parent := Self;
    RapSyf.RaporSyf.Font.Charset := TURKISH_CHARSET;
    OnIzForm := tpreview.create(nil);
...
//      RapSyf.RaporSyf := nil;
    ...
    begin
      if (RapTabAYAR.FieldByName('ALANTURU').AsString = 'SAYFA') or
        ...
        SabitAyarlar
      else if RapTabAYAR.FieldByName('ALANTURU').AsString = 'DİKEYSABİT' then
        DikeySabitAyarlar
...
//   RapSyf.RaporSyf.PrinterSettings.ApplySettings;
    ...
    case mPrevmi of
      0:
        begin
          Screen.Cursor := crDefault;
//            RapSyf.RaporSyf.QRPrinter.Save('c:\' + Raporadi + '.rpt')
...
{$IFNDEF PROJECT_INTEGRATION}
      //3: RaporuJpegOlarakKaydet(RapSyf.RaporSyf,'MailRoot', '.\MailRoot\' + TABLO.TabLabSonucDOSYANO.AsString + '_' + TABLO.TabLabSonucGELISNO.AsString + '_' + TABLO.TabLabSonucKARTNO.AsString + '_' + EkranAdi);
...
//           RapSyf.RaporSyf.QRPrinter.Save('\\gelisimweb\Inetpub\wwwroot\LISNET\rapor\'+Dosyano+'-'+GelisNo+'-'+Kartno+'-'+mEkranAdi+'.qrp');
          ...
          Sleep(500);
          Tablo.Query5.Close;
          Tablo.Query5.SQL.Text := 'select * FROM LABYAZRAPOR WHERE DOSYANO=''' + Dosyano + ''' AND GELISNO=' + GelisNo +
            ' AND KARTNO=' + Kartno + ' AND AD=''' + Dosyano + '-' + GelisNo + '-' + Kartno + '-' + mEkranAdi + '.qrp' + '''';
          Tablo.Query5.Open;

          if Tablo.Query5.RecordCount = 0 then
          begin
            Tablo.Query6.Close;
            Tablo.Query6.SQL.Text := 'Insert Into LABYAZRAPOR (LABYAZ_ID,DOSYANO, GELISNO, KARTNO, AD)values(' + Tablo.TabAraSQL.fieldbyname('ID').AsString + ',''' + Dosyano + ''',' + GelisNo +
              ',' + Kartno + ',''' + Dosyano + '-' + GelisNo + '-' + Kartno + '-' + mEkranAdi + '.qrp' + ''')';
            Tablo.Query6.ExecSQL;
          end;

        end; {5}
      6: // Hastayı FaxKuyruk tablosuna kaydeder...
        begin
          Tablo.Query6.Close;
          Tablo.Query6.SQL.Text := 'Insert Into FAXKUYRUK (DOSYANO, GELISNO, KARTNO)values(''' + Dosyano + ''',' + GelisNo +
            ',' + Kartno + ')';
          Tablo.Query6.ExecSQL;
        end; {6}
     { 7: // Raporu oluşturur ve FaxRapor tablosuna kaydeder...
        begin
          RapSyf.RaporSyf.Prepare;

          PDFExporter := TQRPDFDocumentFilter.Create(
            '.\FaxRoot\' + TABLO.TabLabSonucDOSYANO.AsString + '_' +
            TABLO.TabLabSonucGELISNO.AsString + '_' +
            TABLO.TabLabSonucKARTNO.AsString + '_' + EkranAdi + '.pdf');
          try
            if ((mailantetust <> '') or (mailantetalt <> '')) then
              RapSyf.RaporSyf.OnStartPage := ReportStartPage;
            RapSyf.RaporSyf.ExportToFilter(PDFExporter);
          finally
            PDFExporter.Free;
          end;
//           RapSyf.RaporSyf.QRPrinter.Save('\\gelisimweb\Inetpub\wwwroot\LISNET\rapor\'+Dosyano+'-'+GelisNo+'-'+Kartno+'-'+mEkranAdi+'.qrp');
          //RapSyf.RaporSyf.QRPrinter.Save(InetPubYolu + Dosyano + '-' + GelisNo + '-' + Kartno + '-' + mEkranAdi + '.qrp'); \\gelisimweb\Inetpub\wwwroot\LISNET\rapor\
          //RaporuJpegOlarakKaydet(RapSyf.RaporSyf, 'FaxRoot','.\FaxRoot\' + TABLO.TabLabSonucDOSYANO.AsString + '_' + TABLO.TabLabSonucGELISNO.AsString + '_' + TABLO.TabLabSonucKARTNO.AsString + '_' + EkranAdi);
//          Sleep(500);

          Tablo.Query6.Close;
          Tablo.Query6.SQL.Text := 'Insert Into FAXRAPOR (DOSYANO, GELISNO, KARTNO, RAPORADI, KURUM) values(''' + Dosyano + ''',' + GelisNo +
             ',' + Kartno + ',''' + Dosyano + '_' + GelisNo + '_' + Kartno + '_' + mEkranAdi + '.pdf'', ''' + Tablo.TabGelisler_KURUM.AsString + ''')';
          Tablo.Query6.ExecSQL;
        end; 7}
{$ENDIF}
...
begin
  ...
  Query.Connection := Tablo.FDCnn;
end;



procedure TRapSyf.ReportStartPage(Sender: TCustomQuickRep);
var
  ust : TQrImage;
  alt : TQRImage;
begin
  //
  TQRPDFDocumentFilter(Sender.ExportFilter).SetMargins(-50,0);
  ust := TQRImage.Create(Sender);
  try
    ust.Top := 0;
    ust.Width := Sender.Width;
    ust.Picture.LoadFromFile(mailantetust);
    ust.Height := ust.Picture.Graphic.Height;
    ust.Left := 0;
    ust.Center := True;
    Sender.ExportFilter.AcceptGraphic(10,-10,ust);
  finally
    ust.Free;
  end;
  alt := TQRImage.Create(Sender);
  try
    alt.Picture.LoadFromFile(mailantetalt);
    alt.Width := Sender.Width;
    alt.Height := alt.Picture.Graphic.Height;
    alt.Top := 0;
    alt.Center := True;
    alt.BringToFront;
    Sender.ExportFilter.AcceptGraphic(10,1050,alt);
  finally
    alt.Free;
  end;
end;

procedure TRapSyf.FormDestroy(Sender: TObject);
begin                       //
  if (Assigned(MyDataSets)) then
    MyDataSets.Free;
