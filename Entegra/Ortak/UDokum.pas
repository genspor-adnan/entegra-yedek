...
begin
  kom := '(';
  if (EQUAL = 'Başlayan') or (EQUAL = 'İçinde geçen') then
  ...
  if EQUAL = 'İçinde geçen' then kom := kom + '%';
...
{      Yils :='';
  ...
  kom := kom + VALUE;
  if (EQUAL = 'Başlayan') or (EQUAL = 'İçinde geçen') then kom := kom + '%';
  if (FieldTipi = FtString) or (FieldTipi = FtMemo) or (FieldTipi = FtDate) then kom := kom + '''';
  kom := kom + ')';
  komut.Add(kom);
  komutsatiri := TRUE;
...
begin
   if not BoslukKontrol(TabKosul.AsString['KOD_ADI'], 'Kod Adı') then Abort;
   if not BoslukKontrol(TabKosul.AsString['ACIKLAMA'], 'Açıklama') then Abort;
   if not BoslukKontrol(TabKosul.AsString['ICERIKTURU'], 'İçerik Türü') then Abort;
end;
...
//      index1 := Table1.IndexDefs.Items[0].Fields;
    ...
    begin //İndex Alan Sayısını belirle 'kod;sırano'
      inc(IndexAlanSay);
...
begin
  ...
  FList.Assign(TabDokum.FieldByName('FIELDLIST'));
   {Field'lerden Table Adları belirlenir}
  TabKosul.First;
...
//          if simge <> '.' then Delete(TabAd,1,Rev_Pos(simge, TabAd));
  ...
  if TableAdlari.Count = 0 then {boşsa}
    TableAdlari.Add('STOK');

  if TableAdlari.Count > 1 then {Tablolar arasındaki bağı kur}
...
begin
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from DOKUMLER  where MODUL>''-'' and  STANDART=0 AND DOKUMLER.RAPORADI in ('+
                 ' select D2.RAPORADI from DOKUMLER D2 where   D2.STANDART=1)',[],[]);
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from [AYARLARYENI] where DOKUMID not in (select ID from DOKUMLER) ',[],[]);
   Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'delete from [KOSULLAR] where DOKUMID not in (select ID from DOKUMLER) ',[],[]);
end;
...
begin
   // Analist Değişkenleri Klasörünü ekle
...
begin
  ...
  if not Tablo.Yetkivarmi(110103,YetkiTur_Gorme) then begin//Excel Aktarımı
...
begin
   ...
   if Tablo.Query3.RecordCount>0 then begin
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE AYARLARYENI SET DOKUMID='+IntToStr(DID)+', EKLEYEN='+Kullanan+',EKLEMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''',DEGISTIREN='+Kullanan+',DEGISTIRMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''' WHERE ID = &ID', ['&ID'],[IntToStr(AID)]);
   end;
   ...
   while not Tablo.Query3.eof do begin
     ...
     Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'UPDATE KOSULLAR SET DOKUMID='+IntToStr(DID)+', EKLEYEN='+Kullanan+',EKLEMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''',DEGISTIREN='+Kullanan+',DEGISTIRMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''' WHERE ID = &ID', ['&ID'],[IntToStr(AID)]);
     Tablo.Query3.next;
   end;
...
//      delete(ad, 1, 3);
  ...
  begin
    ...
    begin
      ...
      if ( UpperCase(ComboIcerik) = 'SAAT') or (ComboIcerik = 'ŞİMDİ') or (ComboIcerik = 'Şimdi') or (ComboIcerik = 'şimdi')  then
  ...
  try
    ...
    begin
      ...
      case ATabKosul.FieldByName('ICERIKTURU').AsInteger of
        ...
        2 : SpinCreate('Spin_' + ATabKosul.FieldByName('ID').AsString, ATabKosul.FieldByName('DEGER').AsString, 220, i * 5);  //tamsayı
...
begin
  FFrameBilgi := AValue;
  AValue.Etiketler.AsBoolean['DökümEkranı'] := True;
end;
...
begin                       {
  if not qryListe.Active then
    raise Exception.Create('Önce sonuçları listeleyiniz');
...
begin
   ...
   if OpenDialog1.Execute then begin
      ...
      if TabDokum2.RecordCount >0 then
         if MessageDlg('Dökümlerde aynı adla rapor bulundu. Üzerine kaydedilsin mi?',
...
var
  ...
  j : Integer;
begin
  SaveDialog1.Title := 'Kaydedilecek Dosya Adı';
  if SaveDialog1.Execute then
  begin
    AssignFile(F, SaveDialog1.FileName);
    if FileExists(SaveDialog1.FileName) then
      if MessageDlg(OpenDialog1.FileName + ' adlı dosya zaten var. Üzerine yazılsın mı?',
    ...
    try
      ...
      on EInOutError do
        MessageDlg('Dosya oluşturmada hata var!!!', mtError, [mbOk], 0);
    end;
...
begin
  str := '';
  MesajStrAl('', 'Excel Dosya Adını Dizinli Giriniz :', 'E', nil, str, '', 'E', nil, str);
  if str = '' then exit;
...
begin
 ...
 { Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'Delete from KULHAR '+
...
begin
  ...
  if SaveDialog1.Execute then
  begin
    with Veritabani.SorguBaslat(Tablo.FDCnn, StringReplace(TabKomut.SQL.Text, 'İşlem Sayı', DokumAdi, [rfReplaceAll]),[],[]) do
    try
      Open;
      AssignFile(F, SaveDialog1.FileName);
      try
...
begin
  ...
  while not Tablo.Query5.eof do begin
    ...
    if Tablo.Query3.Fields[0].AsInteger > 1 then begin //daha önce fatura kesilmiş mi bakalımmmm
      Tablo.Query4.Close;
      ...
      if Tablo.Query4.RecordCount > 0 then
        StrPCopy(ss, 'Hasta:' + Tablo.Query4.FieldByName('AD').AsString + ' ' +
          ...
          ' Fat.Tutarı:' + Format('%-10m', [Tablo.Query4.FieldByName('FATURA_TUTARI').AsCurrency]) +
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
      ...
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn,'DELETE FROM DOKUMLER WHERE ID = &ID', ['&ID'],[TabDokum.Fields[0].AsInteger]);
...
begin
  ...
  begin
    ...
    // Eğer sütun bir veritabanı alanına bağlıysa ve veri tipi kontrol edilebilir durumdaysa
    if Assigned(AItem.DataBinding.Field) then begin
      // Veri tipini kontrol et
      case AItem.DataBinding.Field.DataType of
        ftFloat, ftCurrency, ftBCD, ftFMTBcd:
        begin
          // Sütunun özelliklerini para birimi düzenleme özelliklerine dönüştür
          AItem.PropertiesClassName := 'TcxCurrencyEditProperties';

          // Gerekirse biçimlendirme ayarlarını yap
          // TcxCurrencyEditProperties(AItem.Properties).DisplayFormat := 'c'; // Windows ayarlarını kullanır

          // İsteğe bağlı olarak, farklı bir format belirleyebilirsiniz
...
{AO 06.06.2025 bazı raporlarda  hata verdiği için çıkardım
  for I := 0 to qryListe.fieldcount-1 do
