...
begin
   if Application.MessageBox(PChar(TabKuyruk.FieldByName('ADSOYAD').AsString+' adlı hasta kuyruktan silinecektir. Onaylıyor musunuz?'), 'Silme Onayı', MB_OKCANCEL)<> IDOK then exit;
   Tablo.Query1.Close;
...
begin
   ...
   Tablo.Query1.Open;
   if Tablo.Query1.RecordCount < 1 then raise exception.Create('Kuyruk Boş...');
...
{   for i := 5 to 13 do begin
   ...
   TabloDokum.MenuIslemleri(KurukDegisMenu, KuyrukSecClick, 'Ekle', 'Kayıt Kabul', '',-1);
...
begin
   ...
   else if (key in ['A'..'Z'])or(key in ['a'..'z'])or(key in ['ı','İ','ğ','Ğ','ş','Ş','ü','Ü','ç','Ç','ö','Ö']) then begin
...
//   C:\Psign.exe [1][UNBOLD]ADNAN ODABAŞI[BEEP][STOP]
end;
