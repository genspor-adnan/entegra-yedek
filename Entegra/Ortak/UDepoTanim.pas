...
begin
   ...
   if Tablo.Query1.RecordCount>=1 then
    begin
       Application.MessageBox('Bu depoda hareket görmüş ürünler vardır. Silinemez. Lütfen depo yu pasif yapınız.','H A T A', MB_OK+ MB_ICONERROR);
       Abort;
    end;
...
begin
  if Pos('Cannot insert the value NULL into column ''DEPOKODU''',E.Message) >0 then
    ShowMessage('Depo Kodu Boş olamaz.')
  else if Pos('Violation of UNIQUE KEY constraint ''IX_DEPOLAR_DEPOKOD''',E.Message)>0 then
    ShowMessage('Bu depo kodu daha önce başka bir depo için kullanılmış. Lütfen başka bir depo kodu tanımlayınız.')
  else if Pos('Violation of UNIQUE KEY constraint ''IX_DEPOLAR_DEPOADI''',E.Message)>0 then
    ShowMessage('Bu depo adı daha önce başka bir depo için kullanılmış. Lütfen başka bir depo adı tanımlayınız.');
...
begin
  if TabDepolar.FieldByName('DEPOKODU').AsString='' then
    begin
       Application.MessageBox('Depo Kodu Boş Olamaz','U Y A R I', MB_OK+ MB_ICONWARNING);
       abort;
    end;
  if TabDepolar.FieldByName('DEPOADI').AsString='' then
    begin
       Application.MessageBox('Depo Adı Boş Olamaz','U Y A R I', MB_OK+ MB_ICONWARNING);
       abort;
