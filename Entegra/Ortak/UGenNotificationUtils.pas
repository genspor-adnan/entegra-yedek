...
begin
  ...
  begin
    ...
    begin
      //Hata Oluştu demektir bunları bi alalım
      //Dönen Hata kodları
      //WP:00  Hatalı Kullanıcı Adı yada şifresi
      //WP:01  Geçersiz IP Adresi
      //WX:02  Kontörlü Hesap Değil
      //WX:03  Kontör Hatası
      //WP:90   XML Hatası
      Donen.HataKodu := copy(DonusBilgisi.Strings[0], 4, 2);
      if Donen.HataKodu = '00' then
        Donen.Mesaj := 'Hatalı Kullanıcı Adı yada şifresi'
      else if Donen.HataKodu = '01' then
        Donen.Mesaj := 'Geçersiz IP Adresi'
      else if Donen.HataKodu = '02' then
        Donen.Mesaj := 'Kontörlü Hesap Değil'
      else if Donen.HataKodu = '03' then
        Donen.Mesaj := 'Kontör Hatası'
      else if Donen.HataKodu = '90' then
        Donen.Mesaj := 'XML Hatası';
      Donen.KontorMiktari := '0';
    end
    else
    begin
      // Hata Yoksa Kontör miktarını ve mesajını oluşturalım
      Donen.HataKodu := '-1';
      Donen.KontorMiktari := DonusBilgisi.Strings[0];
      Donen.Mesaj := 'Kontör Son kullanım tarihi :' + copy(DonusBilgisi.Strings[1], 7, 2) + '/' + copy(DonusBilgisi.Strings[1], 5, 2) + '/' + copy(DonusBilgisi.Strings[1], 1, 4);
    end;
  ...
  begin
    ...
    //10 Kullanıcı Kodu / şifresi hatalı
    //90 Sistem hatası
  ...
  3.2.1 öRNEK KREDİ CEVAP
  CREDIT-BalRequest sonrasında sunucudan gelebilecek cevap aşağıdadır. Res değeri mevcut kredinizi verir.
  ...
  <CREDIT-Response>
    ...
    try
      ...
      //Gönderilecek xml oluşturuluyor..
      XMLStructe.Text := //'<?xml version="1.0" encoding="ISO-8859-9"?>' +
        '<CREDIT-BalRequest>' +
      ...
      //Başlık Kısmı standart bilgilerin hazırlanması unicede olarak İsmail ACET 2010-09-16
    ...
    begin
      ...
      until ANode = nil;
      // Hata Yoksa mesaj referansını  ve mesajını oluşturalım
      Donen.HataKodu := '-1';
      Donen.Mesaj :=  '';
...
var
  ...
  procedure SMSDurumGuncelle(durum:Integer;durumaciklama:string);
   begin
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'UPDATE SMSLER SET DURUM = '+inttostr(durum)+' , '+
...
begin
   ...
   try
    //Başlık Kısmı standart bilgilerin hazırlanması
    XMLStructe.Text := //'<?xml version="1.0" encoding="ISO-8859-9"?>' +
      '<SMS-StaRequest>' +
      '<CLIENT user="' + SMSHesapBilgileri.SMSKullaniciAdi + '" pwd="' + SMSHesapBilgileri.SMSSifre + '"/>' +
    ...
    XMLStructe.Text := XMLStructe.Text + '</SMS-StaRequest>';
      ...
      case StrToInt(Kontrol) of
       ...
       400 : SMSDurumGuncelle(9,'Başarılı');
       401 : SMSDurumGuncelle(9,'Alıcıya gönderilecek');
       402 : SMSDurumGuncelle(-1,'Alıcıya ulaşmadı');
       403 : SMSDurumGuncelle(-1,'Alıcıya ulaşmadı(Zaman Aşımı)');
       404 : SMSDurumGuncelle(-1,'Alıcıya ulaşmadı(Bilinmeyen Hata)');
       405 : SMSDurumGuncelle(-1,'Alıcıya ulaşmadı(Sistem Hatası)');
       406 : SMSDurumGuncelle(-1,'Alıcıya ulaşmadı(SMS Kapalı)');
       410 : SMSDurumGuncelle(-1,'Mesaj SMSC de bulunmadı');
       420 : SMSDurumGuncelle(-1,'Mesaj sunucuda bulunamadı');
       -20 : SMSDurumGuncelle(-1,'Bu mesaj size ait değil');
      -201 : SMSDurumGuncelle(-1,'Silindi(Geçersiz Cep No)');
      -202 : SMSDurumGuncelle(-1,'Silindi(Mesaj Tekrarı)');
      ...
      -210 : SMSDurumGuncelle(-1,'Silindi(Zaman Aşımı)');
      -220 : SMSDurumGuncelle(-1,'Silindi(Yetersiz kredi zaman aşımı)');
      -222 : SMSDurumGuncelle(-1,'Silindi(Abone mesajın bloke edilmesi tercih ettiğinden)');
      -250 : SMSDurumGuncelle(-1,'Silindi(Müşteri tarafından)');
...
var
 ...
 var
  ...
  begin
    ...
    // ShowMessage('Gönderim için mesajlar oluşturuluyor .');  //Döngü ile dizideki mesaj ve numaraları alalım
    ...
    begin
      ...
      begin
       XMLStructure.Text := XMLStructure.Text + '<INSERT to="' + Mesajlar[i].Numara + '" text="' + 'Tel no hatalı' + '" ' + ZamanEk + '/>'

      end;
    end;
   // ShowMessage('Gönderim için mesajlar oluşturuluyor Bitti.') ;
    ...
    try
      ...
      //Başlık Kısmı standart bilgilerin hazırlanması unicode olarak
      HTTPReq.setRequestHeader('Content-Type: text/xml','<?xml version="1.0" encoding="windows-1254"?>');
   ...
   // ShowMessage('Mesaj karşıya gönderildi.') ;  //ShowMessage(DonusBilgisi.Text);
    {
               Hata Numarası
    ...
    begin
      ...
      begin
        ...
        donen.Mesaj := 'İstek Hatası. İstek değerlendirilemedi. (Hatalı istek formatı)';
      end
      else if pos('ERR1040', DonusBilgisi.Text) > 0 then
      begin

        Donen.HataKodu := 'ERR1040';
        donen.Mesaj := 'İstek Hatası. İstek tarihçe kayıt numarası alınamadı.';
      end
      else if pos('ERR1030', DonusBilgisi.Text) > 0 then
      begin

        Donen.HataKodu := 'ERR1030';
        donen.Mesaj := 'İstek Hatası. İstek tarihçe kaydı tutalamadı.';
      end
      else if pos('ERR1020', DonusBilgisi.Text) > 0 then
      begin

        Donen.HataKodu := 'ERR1020';
        donen.Mesaj := 'İstek Hatası. Veritabanına bağlanamadı.';
      end
      else if pos('ERR1010', DonusBilgisi.Text) > 0 then
      begin
        Donen.HataKodu := 'ERR1010';
        donen.Mesaj := 'İstek Hatası. Veritabanı bağlantı işlemi yapılamadı';
      end
      ...
      begin
        ...
        Donen.Mesaj := 'Gönderim Başarısız'
    end
    else
    begin
...
begin
  ...
  AVeriSeti.AsString['AlıcıListesi'] := Dize.Birlestir('|', rcp);
end;
...
{  if EpostaHesapBilgisi.HesapId = -1 then
   begin
     ...
     Result.SonucMesaji:='Geçerli Bir Eposta Hesabı Seçiniz';
   end
  else}
  if AliciListesi.Count=0 then
   begin
     Result.GonderimBasarili:=False;
     Result.SonucMesaji:='Alıcı listesinde en az bir alıcı olmalıdır.';
   end
 else
  begin
      try
