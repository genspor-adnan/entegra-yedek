  ...
  try
    ...
    begin
      ...
      if pos('unknown', E.Message) > 0 then
         EpostaSonucMsg:='Hatalı Mail adresi'
      else
        EpostaSonucMsg:='Başarısız';
    end;
  end;
...
begin
  if rgZamanlama.ItemIndex = 1 then //Zamanlı SMS gönderilmesi
  begin
...
// **************  HATALI VEYA EKSİK ALANLAR KONTROL EDİLİYOR ****************
  ...
  if trim(mMesaj.Text) = '' then
    HataMesaji.Add('Mesaj içeriği boş.');
  ...
  HataMesaji.Free;
// **************  HATALI VEYA EKSİK ALANLAR KONTROL EDİLİYOR **************** SONU
// **************  HATA YOK İSE GÖNDERİM İŞLEMİ BAŞLIYOR ****************
...
// **************  HATA YOK İSE GÖNDERİM İŞLEMİ BAŞLIYOR **************** SONU
  ...
  begin
    ...
    begin
      ...
      try
        ...
        if Durum='Başarılı' then Statu:=9
        else if Durum='Hatalı' then Statu:=-1
        else if Durum='Zaman Aşımı' then Statu:=-1
