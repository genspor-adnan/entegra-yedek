...
begin
   A := ''; B:= '';
   MesajStrAl('','Muayene Katsayısını KDV Dahil Giriniz :', 'E', nil,A,
                 'Tetkik Katsayısını KDV Dahil Giriniz :', 'E', nil,B);
   if (A = '')OR(B = '') then exit;
...
begin
   ...
   Table1.Close;
   Table1.Open;
   ShowMessage('Silme İşlemi Sona Erdi....');
end;
...
begin
   YeniFiyatAdi := '';
   MesajStrAl('','Yeni Fiyat Adını Giriniz :', 'E', nil,YeniFiyatAdi, '', 'E', nil,YeniFiyatAdi);
   ...
   Table1.cLOSE;
   Table1.Open;
   ShowMessage(YeniFiyatAdi+' Fiyatı Oluşturuldu..');
end;
...
begin
   ...
   YeniFiyatAdi := '0';
   if MesajStrAl('','Katsayı/Fiyatın '+ArtAzal+' Miktarı :', 'E', nil,YeniFiyatAdi, 'Başlayan Kod? (Hepsi için boş geçiniz)', 'E', nil,Kod) then
...
begin
   ...
   YeniFiyatAdi := '20';
   if MesajStrAl('','Katsayı/Fiyatın '+ArtAzal+' Oranı % :', 'E', nil,YeniFiyatAdi, '', 'E', nil,YeniFiyatAdi) then
      YeniOran := StrToFloat(YeniFiyatAdi)
   else
      YeniOran := 0;
   ...
   Kod:=''; islemadi:='';
   if not MesajStrAl('','Başlayan Kod? (Hepsi için boş geçiniz)', 'E', nil,Kod, 'İşlem adının içinde geçen (Hepsi için boş geçiniz)', 'E', nil, islemadi) then exit;
...
begin
   ...
   YeniFiyatAdi := ''; Kod:='';
   MesajStrAl('','Yeni Çarpanı Giriniz :', 'E', nil,YeniFiyatAdi,  'Başlayan Kod? (Hepsi için boş geçiniz)', 'E', nil,Kod);
   if YeniFiyatAdi = '' then exit;
...
{   if FiyatAdi = 'TTB' then begin
      ...
      if Tablo.Query1.RecordCount > 0 then
         if MessageDlg('SEC alanı boş kayıtlara rastlandı. Devam edilsin mi?', mtConfirmation, [mbYes,mbNo], 0) <> mrYES then exit;
   end;  }
...
begin
   ...
   ShowMessage('Çevirme İşlemi Tamamlandı...');
{   DEG('000000', '025000');
   DEG('025000', '050000');
   DEG('050000', '075000');
   DEG('075000', '100000');
