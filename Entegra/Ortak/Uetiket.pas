...
{ Radyoloji Vers. 8.1.3.8 7/9/2008 de Adnan tarafından yapıldı..
...
begin
  ...
  if (En <> 0) and (TABLO <> 'BOŞLUK') then
    RapTablo.Ayarlar.FieldByName('EN').AsFloat := En;
  if (Boy <> 0) and (TABLO <> 'BOŞLUK') then
...
begin
  if ListBox1.Count<1 then
     raise Exception.Create('Tanımlı etiket bulunamadı. Yeni tuşuyla tanımlayın!');

  if StrToInt(YanSay.Text) > 20 then begin
    ShowMessage('Yanyana etiket sayısı 20 yi aşıyor!!!');
    ModalResult := mrNone;
  end
  else begin
...
//     DegerAtama(3,'SAYFA', 'BOŞLUK', '',0,0,0,0,0,'',0);
    DegerAtama(3, 'SAYFA', 'BOŞLUK', '', 0, SSolBosluk, SUstBosluk, SSagBosluk, 0, '', 0);
    DegerAtama(5, 'SAYFA', 'KOLON', '', YanYanaSay, 0, 0, 0, 0, '', 0);
//     if SUstBosluk > 0 then
//        DegerAtama(10,'BAND', 'SAYFABAŞI', '',0,0,0,0,SUstBosluk,'',0);
...
begin
  Ad := '';
  if not MesajStrAl('', 'Etiket Adını Giriniz :', 'E', nil, Ad, '', 'E', nil, Ad) then exit;
...
begin
  ...
  //Soldakini değiştirelim
//  GenotipIni.DeleteKey('EtiketListesi', EskiAd);
//  GenotipIni.WriteString('EtiketListesi', Ad,'');
//  //Sağdakini değiştirelim
  Tablo.Query1.Close;
...
begin
  ...
  if Application.MessageBox('Bu etiket silinecektir. Onaylıyor musunuz?', 'O N A Y', MB_YESNO) <> IDYES then exit;
  Tablo.Query1.Close;
  Tablo.Query1.SQL.Text := 'Delete From Ayarlar Where RAPORADI = ''Etkt_'+ListBox1.Items[ListBox1.Itemindex]+''' ';    ///Etiket_''';
  Tablo.Query1.ExecSQL;
