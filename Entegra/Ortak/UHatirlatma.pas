...
Function THatirlatmaDlg.Gun(I1: Integer; s2: string): Integer;
  begin
    ...
    else if pos('Yıl', s2) > 0 then Gun := I1 * 365
    else Gun := I1;
  end;
}
...
begin
  ...
  else if pos('Yıl', per) > 0 then Gun := IncYear(AnTarih, xsay)
...
begin
  if YeniEklenenCagri then begin
    ...
    Tablo.LogIslemleri('HATIRLATMA', 'Değiş', TabCagri, true, false, false, false);
  ...
  if (not YeniEklenenCagri) then
    if Application.MessageBox(PChar(TabCagri.FieldByName('ISLEMADI').AsString + ' için çağrı tarihleri yeniden düzenlensin mi?'),
...
begin
  ...
  begin
    Application.MessageBox('İşlem seçiniz.', 'U Y A R I', MB_ICONWARNING);
    sbIslemSec.Click;
    abort;
  end;
  ...
  if TabCagri.state = dsInsert then
    ...
    begin
      Application.MessageBox('İlk geliş zamanı belirleyiniz (Geçmiş tarihe hatırlatma tanımlanamaz).', 'U Y A R I', MB_ICONWARNING);
  ...
  begin
    ...
    begin
      Application.MessageBox('Geliş periyodu belirleyiniz.', 'U Y A R I', MB_ICONWARNING);
    ...
    if rbseans.Checked then begin
      ...
      begin
        Application.MessageBox('Seans sayısı veya sonlanma zamanı belirleyiniz.', 'U Y A R I', MB_ICONWARNING);
...
begin
  if Application.MessageBox(PChar(TabCagri.FieldByName('ISLEMADI').AsString + ' için tanımlanmış olan çağrı bilgisi silinecektir. Emin misiniz?'),
    'O N A Y', mb_YesNo) <> IDYes then abort;
...
begin
  ...
  begin
    i := Application.MessageBox(PChar('Değişiklikler kaydedilsin mi ?'), 'O N A Y', MB_YESNOCANCEL);
    case i of
      IDYes:
        begin
