...
var index1, TabAd, sat : String[100];
    ...
    begin
      ...
      IndexAlanAl := index1;
      IndexAlanSay := 0;
      while pos(';', index1) > 0 do begin //İndex Alan Sayısını belirle 'kod;sırano'
         inc(IndexAlanSay);
...
begin //  ANDOR1 TABLE1, FIELD1, EQUAL1, VALUE1
   ...
   if TabloDokum.TabKosul.RecordCount > 0 then begin {koşul var}
      komut.add(' where ');
      TabloDokum.TabKosul.First;
      while not TabloDokum.TabKosul.eof do begin
...
//      komut.Add(')');
   ...
   if (TabloDokum.TabKosul.RecordCount = 0{koşul yoksa})and(TableBagList.Count > 0{birden fazla table varsa})then
       komut.add(' where ')
   else if (TabloDokum.TabKosul.RecordCount > 0{koşul var})and(TableBagList.Count > 0{birden fazla table varsa})then
       komut.add('and');
...
begin
   if DtsDokumler.State in [dsEdit, dsInsert] then
        if MessageDlg('Döküm Ekranı Değişti !!! '+#13#10+'Yapılan Değişiklikler Kaydedilsin mi?',
...
//      raise hata.create('Şu anda rapor ayarları var. Yeniden oluşturmak istiyorsanız rapor ayarlarını silin.');
   ...
   GlobBandNo:=0;
   Sayac:=1;
   DegerAta('SAYFA', 'YAZICI', '(yazıcı ismi)',0,0,0);
   DegerAta('SAYFA', 'BOYUT', 'A4',0,0,0);

   // Yedi tane band ekleniyor
   Sayac:=10;
   DegerAta('BAND', 'SAYFABAŞI', '',0,0,3);
   ...
   GlobBandNo:=10;
   SolYer:=0;
   DegerAta('SABİT', '', 'Dr. Ad Soyad',0,0,0);
   For k:=0 To FList.Count-1 Do Begin
      DegerAta('SABİT', '', Noktasiz(FList.Strings[k]),SolYer,2,0);
      SolYer:=SolYer+3
   End;
   GlobBandNo:=11;
   Sayac:=50;
...
begin
   If not TabloDokum.Query1.Active Then
      raise Hata.Create('Önce sonuçları listeleyiniz');
...

{
   ...
   try
      ...
      showmessage('Bu adla kayıtlı rapora rastlandı!');
      exit;
   end;
