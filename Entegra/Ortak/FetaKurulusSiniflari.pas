...
{*                                                       *}
{*                                                       *}
{* (c) Telif Hakký 2009 Feta Bilgisayar                  *}
{********************************************************}
...
type
  ...
  public
    type
      Dizin = class
      public
        class procedure ÞuAnkiDizindekiDosyalar(AUzanti : string;AListe: TStrings);
  ...
  public
    ...
    class function BunlardanBiriVarMý(ADeðer: Integer;
      ADeðerler: array of Integer): Boolean;
  end;
  ...
  TSaatDeðeriniDeðiþtir = (sddDeðiþiklikYok,sddGünBaþýOlarak,sddGünSonuOlarak);
  ...
  public
    ...
    class function TarihSaatStringOlarak(ATarih: TDateTime;ATarihSaatBiçimi: TTarihSaatBiçimi = tsbNormal;
      ASaatDeðeriDeðiþsin: TSaatDeðeriniDeðiþtir = sddDeðiþiklikYok): string;
  ...
  public
    ...
    class function SqlTarihAralýðý(ABaþlangýç, ABitiþ: TDateTime; AKolonlar: array of string;ABetweenKullan: Boolean = True): string;
    class procedure TabloSatýrlarýnýKopyala(ABaðlantý: TFDConnection; ATabloAdý, ADeðiþecekAlan, AÞimdikiDeðeri, AÝstenenDeðer : string);
    class function BasitKomutÇalýþtýr(cnn: TFDConnection;ASQL: string; AParamAdlarý: array of string;AParamDeðerleri: array of Variant; ASonuçDönecek : Boolean = False;AUseDataSource : TDataSource = nil): Variant;
    class function BaðlantýDizesiDeðerDeðiþtir(AKaynakDize: string; AAnahtar: string;ADeðer: string): string;
    class function BoþDataSet(ABaðlantý: TFDConnection;ATabloAdi : string) : TADOQuery;
    class function IntegerListGetir(cnn: TFDConnection;ASql: string; AParamAdlarý: array of string;AParamDeðerleri: array of variant): TList<Integer>;
  end;
  ...
  { Doðrudan çaðrýlabilir iþlev ve yordamlar }
...
class function TarihSaat.KüçükTarihSaatStringOlarak(ATarih: TDateTime;
  ...
  ASaatDeðeriDeðiþsin: TSaatDeðeriniDeðiþtir): string;
var
  fmt : string;
begin
  ...
  case ASaatDeðeriDeðiþsin of
    sddDeðiþiklikYok: Result := FormatDateTime(fmt + ' hh:nn', ATarih);
    sddGünBaþýOlarak: Result := FormatDateTime(fmt + ' 00:00', ATarih);
...
begin
  ...
  case ASaatDeðeriDeðiþsin of
    sddDeðiþiklikYok: Result := FormatDateTime(fmt + ' hh:nn:ss', ATarih);
    sddGünBaþýOlarak: Result := FormatDateTime(fmt + ' 00:00:00', ATarih);
    sddGünSonuOlarak: Result := FormatDateTime(fmt + ' 23:59', ATarih);
  end;
end;
...
class function Veritabani.BasitKomutÇalýþtýr(cnn: TFDConnection;
  ASQL: string; AParamAdlarý: array of string;
  AParamDeðerleri: array of Variant;ASonuçDönecek : Boolean;
...
class function Veritabani.BaðlantýDizesiDeðerDeðiþtir(AKaynakDize,
  AAnahtar, ADeðer: string): string;
var
  cs : TStringList;
begin
...
class function Veritabani.BoþDataSet(ABaðlantý: TFDConnection;ATabloAdi: string): TADOQuery;
begin
  Result := SorguBaslat(ABaðlantý,'SELECT TOP 0 * FROM ' + ATabloAdi,[],[]);
end;

class function Veritabani.IntegerListGetir(cnn: TFDConnection; ASql: string;
  AParamAdlarý: array of string;
  AParamDeðerleri: array of variant): TList<Integer>;
begin
  Result := TList<Integer>.Create;
  with SorguBaslat(cnn, ASql, AParamAdlarý, AParamDeðerleri) do
    try
      Open;
...
class function Veritabani.SqlTarihAralýðý(ABaþlangýç, ABitiþ: TDateTime;
  AKolonlar: array of string; ABetweenKullan: Boolean): string;
begin
  if Length(AKolonlar) > 1 then
    Result := Format('%s >= %s AND %s <= %s',[
      AKolonlar[0],
      TarihSaat.TarihSaatStringOlarak(ABaþlangýç,tsbSQL,sddGünBaþýOlarak),
      AKolonlar[1],
      TarihSaat.TarihSaatStringOlarak(ABitiþ,tsbSQL,sddGünSonuOlarak)
    ])
...
class procedure Veritabani.TabloSatýrlarýnýKopyala(ABaðlantý: TFDConnection;ATabloAdý,
  ADeðiþecekAlan, AÞimdikiDeðeri, AÝstenenDeðer: string);
var
  fld : string;
  lst : TStringList;
  _out : string;
...
begin
  ...
  with FetaKurulusSiniflari.Veritabani.SorguBaslat(ABaðlantý,'SELECT c.name,t.name as tip FROM '+
    'syscolumns c inner join systypes t on t.xtype = c.xtype WHERE '+
    'c.id = OBJECT_ID(&1) ORDER BY c.colorder',['&1'],[ATabloAdý]) do
  try
    Open;
    _out := '';
    Locate('name',ADeðiþecekAlan,[]);
    stringAlan := Dize.BunlardanBiriVarMi(Fields[1].AsString,
      ['varchar','nvarchar','char','nchar']);
    { INSERT için alanlarý topluyoruz }
    Fields[0].CopyAllRowsToStringList(lst);
    fld := lst.Join(',');
    _out := Format('INSERT INTO %s (%s)',[ATabloAdý,fld]);
    { SELECT için ayarlamalar yapýyoruz }
    if stringAlan then
      deðer := '''' + AÝstenenDeðer + ''''
    else
      deðer := AÝstenenDeðer;
    deðer := deðer + ' AS ' + ADeðiþecekAlan;
    lst.Change(ADeðiþecekAlan,deðer);
    if stringAlan then
      deðer := '''' + AÞimdikiDeðeri + ''''
    else
      deðer := AÞimdikiDeðeri;
    _out := _out + Format('SELECT %s FROM %s WHERE %s = %s',
      [lst.Join(','),ATabloAdý,ADeðiþecekAlan,deðer]);
    ABaðlantý.ExecSQL(_out);
  finally
    Free;
    lst.Free;
  end;
...
class function SayiYonetimi.BunlardanBiriVarMý(ADeðer: Integer;
  ADeðerler: array of Integer): Boolean;
var
  deðer : Integer;
begin
  Result := False;
  for deðer in ADeðerler do
    if ADeðer = deðer then begin
      Result := True;
      Exit;
    end;
end;
  ...
  begin
    ...
    if F > 0 then begin
      ...
      if Copy(AKodlanmisDize,F + Length(AStartTag),1) = '=' then begin
        {Burada belirtilen kod _write kullanarak çýktýya yansýtýlmalý gerektiðini bildiriyor}
        Result.WriteDirective := True;
        { Deðer çýkarma iþlevlerinin düzgün çalýþmasý için = sembolünü geçmesini saðlýyoruz }
        AStartTag := AStartTag + '=';
        //AStartPos := AStartPos + 1;
      end else if Copy(AKodlanmisDize,F + Length(AStartTag),1) = '*' then begin
        {Burada belirtilen kod bir baþlýk bilgisi olduðunu function,var gibi
          global olarak kullanýlan ifade içerdiðini bildiriyor}
      ...
      { Bu etiketten önce bir girdi var ise bunlarý _write ile yazýyoruz }
      Result.BeginEndBlock := Result.BeginEndBlock + _Write(Copy(AKodlanmisDize,AStartPos,(F) - AStartPos));
      { Etiketler arasýndaki kod kýsýmýný yazýyoruz }
      ...
      { Son pozisyon güncellemesini yapýyor F2 deðiþkeni  --> %> % iþaretini gösteriyor. }
...
begin
  ...
  try
    repeat
      {Kod bloðu var mý bakýyoruz }
      ti := DoCodeBlock(CurPos,'<%','%>');
      { Bu çaðýrmadan birþey iþlenmiþ mi? }
      if ti.Processed then begin
        {Bu kod baþlýk bloðu mu ?}
        if ti.HeaderBlock then begin
          Header := Header + ti.HeaderString;
          BeginEndBlock := BeginEndBlock + ti.BeginEndBlock;
        end
        { Bu kod sadece _write kullanarak direk yazýlmasý gereken biþey mi ? Örn. bir deðiþken }
        else BeginEndBlock := BeginEndBlock + ti.BeginEndBlock;
        { Hep ayný þeyleri bulmamasý için Pozisyonu deðiþtiriyoruz }
    ...
    if (BeginEndBlock = '') then begin
      ...
      { O zaman tamamýný _write ile çevreliyoruz }
...
class procedure DosyaSistemi.Dizin.ÞuAnkiDizindekiDosyalar(AUzanti: string;
  AListe: TStrings);
var
  SearchRec: TSearchRec;
  ListeSonu: Boolean;
