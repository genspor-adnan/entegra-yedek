...
uses Windows, SysUtils, Classes, Graphics, Forms, Controls, StdCtrls, Buttons,
  ExtCtrls, UFDCompatHelpers, Registry, Dialogs, DBCtrls,
...
type                                    
  ...
  public
    ...
    procedure ReadSection(Bolum: string; Liste: TStrings;AOnbellekKullanma: Boolean = True;clearList: Boolean = True);
...
procedure TIni.ReadSection(Bolum: string; Liste: TStrings;AOnbellekKullanma: Boolean = True;clearList: Boolean = True);
var
  i : Integer;
  anahtarlar: TStringList;
begin
...
var
  ...
  tmpTable: TADOQuery;
begin
  with _query_exec(IniSql.Connection,'DELETE FROM ' + Dosya +
    ' WHERE (BOLUM=%BOLUM%) AND (ANAHTAR=%ANAHTAR%)',
    ['%BOLUM%','%ANAHTAR%'],[Bolum,Anahtar]) do
  try
    i := 0;
    ...
    if i = 0 then Connection.RollbackTrans;
    Free;
  end;
  if i = 0 then Exit;
  AnahtarSil(Bolum,Anahtar);
