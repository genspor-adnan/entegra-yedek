...
const
	...
	AlphaNum3 : String = '_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789éöóêåíãøùçõúôûâàïğîëäæıÿ÷ñìèòüáşÉÖÓÊÅÍÃØÙÇÕÚÔÛÂÀÏĞÎËÄÆİß×ÑÌÈÒÜÁŞ¸¨';
	...
	Printable1251: String = ' !%&()-+=*[]{}~`;:"?/\<>.,_|#@abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789éöóêåíãøùçõúôûâàïğîëäæıÿ÷ñìèòüáşÉÖÓÊÅÍÃØÙÇÕÚÔÛÂÀÏĞÎËÄÆİß×ÑÌÈÒÜÁŞ¸¨';
...
function SkipBlanksLen( var ptr : PChar; var len : Integer ) : PChar; // Ïğîïóñêàåì ïğîáåëû è ñèìâîëû òàáóëÿöèè
function RemoveTrailingBlanksLen( var ptr : PChar; var len : Integer ) : PChar; // Óäàëÿåì êîíöåâûå ïğîáåëû è òàáóëÿöèè - óìåíüøàåì len íà êîëè÷åñòâî êîíöåâûõ ïğáåëîâ è òàáóëÿöèé
function FindLexLen( var ptr : PChar; var len : Integer ) : PChar; // Ïğîïóñêàåì óïğàâëÿşùèå ñèìâîëû è ïğîáåëû ( âñå ñèìâîëû ìåíüøå $21 )
function NextLine( var ptr : PChar; var len : Integer ) : PChar; // Ïàğà ptr/len óñòàíàâëèâàåòñÿ çà ïåğâûé CR è/èëè LF
...
// Èçâëåêàåì ñòğîêó äî ïåğâîãî ñèìâîëà, íå âõîäÿùåãî â AllowedChars, ıòîò ñèìâîë áóäåò âûäàí â ATerm.
// Åñëè âñå ñèìâîëû èç AllowedChars, òî ATerm áóäåò #0
// Åñëè AllowedChars='' òî áóäåò èñïîëüçîâàòüñÿ AlphaNum2
// Ïàğà ptr/len áóäåò óêàçûâàòü íà ñèìâîë-òåğìèíàòîğ
function GetTerm( var ptr : PChar; var len : Integer; var ATerm : Char; AllowedChars : String = '' ) : String;

// Åñëè ïåğâûé çíà÷èìûé ñèìâîë - àïîñòğîô èëè êàâû÷êà, òî áóäåò âîçâğàùåíà ğàñêâî÷åííàÿ ñòğîêà,
// èíà÷å èçâëåêàåò ñòğîêó äî òåğìèíàòîğà, à ïğîáåëû/òàáóëÿöèè ñ îáîèõ ñòîğîí áóäóò óäàëåíû.
// Ïàğà ptr/len áóäåò óêàçûâàòü íà ñèìâîë-òåğìèíàòîğ.
// Åñëè ATerminators='' òî áóäåò èñïîëüçîâàòüñÿ DefaultStringTerminators
function GetStringTerm( var ptr : PChar; var len : Integer; ATerminators : String = '' ) : String;

// İêâèâàëåíòíî âûçîâó GetStringTerm(ptr,len,'');
function GetString( var ptr : PChar; var len : Integer ) : String;

// Åñëè ïåğâûé çíà÷èìûé ñèìâîë - àïîñòğîô èëè êàâû÷êà, òî áóäåò âîçâğàùåíà ğàñêâî÷åííàÿ ñòğîêà,
// èíà÷å èçâëåêàåò ñòğîêó äî ïåğâîãî íåäîïóñòèìîãî ñèìîëà.
// Ïàğà ptr/len áóäåò óêàçûâàòü íà ïåğâûé ñèìâîë, íå âõîäÿùèé â ÷èñëî äîïóñòèìûõ
// Åñëè AllowedChars='' òî áóäåò èñïîëüçîâàòüñÿ AlphaNum2
...
//-------------------------------------------------------------
// !!! buf óêàçûâàåò íà îáëàñòü, äëèííîé íå ìåíåå len*3+2 !!!
...
{	test	eax,eax
  jz		@@Ret
  test	ecx,ecx  âñåãäà ecx<>0 <- ıòî óæå ïğîâåğåíî
  jnz		@@Start
	xor		eax,eax
	jmp		@@Ret
@@Start:}
...
asm
  ...
  JC		@@RetNIL
  CMP		AL,''''
  JE		@@1	{ íà÷èíàåòñÿ ñ àïîñòğîôà }
  CMP		AL,'"'
  JNE		@@UNQ	{ íà÷èíàåòñÿ íå ñ àïîñòğîôà è íå ñ êàâû÷êè }
{ íîğìàëüíàÿ îòêâî÷åííàÿ ñòğî÷êà }
@@1:  MOV		AH,AL	{ ñîõğàíÿåì ïåğâûé ñèìâîë }
@@2:	LODSB
	CMP		AL,' '
  JC		@@Unmatch
  CMP		AL,AH
...
@@Store:
	STOSB
	LOOP	@@2
@@Overflow:	{ Îøèáêà - ïåğåïîëíåíèå áóôåğà }
	MOV		EAX,-1
  JMP		@@Abort

@@Unmatch:	{ Îøèáêà - íåò çàâåğøàşùåãî àïîñòğîôà èëè êàâû÷êè }
	MOV		EAX,-3
  JMP		@@Abort
...
@@ESC:
  ...
  JC		@@Store
@@ErrEsc:
	MOV		EAX,-2	{ Îøèáêà - íåâåğíûé ñèìâîë ïîñëå # }
@@Abort:
	XOR		ESI,ESI	{ nil }
	JMP		@@Exit
...
@@Quoted:
			mov		dh,al	{ ñîõğàíÿåì ğàçäåëèòåëü - àïîñòğîô èëè êàâû÷êó }
			loop	@@Dequote
			jmp		@@Unmatch
...
// Êàâû÷êà èëè àïîñòğîô ïğè ñêàíèğîâàíèè îòêâî÷åííîé ñòğî÷êè
@@Quote:
			dec		ecx
			jz		@@Exit
			lodsb
			cmp		al,dh		// ïğîâåğÿåì íà äâîéíóş êàâû÷êó/àïîñòğîô
			jne		@@Unquoted
@@Store:
			stosb
@@Iterate:
...
begin
	...
	begin
  	case -len of
  	2: Msg := 'StrUnquote: Íåâåğíûé ñèìâîë ïîñëå $';
  	3: Msg := 'StrUnquote: Íåïàğíûé àïîñòğîô èëè êàâû÷êà';
  	else
    	Msg := 'StrUnquote: Íåèçâåñòíàÿ îøèáêà '+IntToStr(len);
  	end;
...
begin
  ...
  if Len<=0 then
    ...
    begin
	    ptr := nil;
	  	case -Len of
  	  1: Msg := 'UnQuote: Ïåğåïîëíåíèå áóôåğà';
    	2: Msg := 'UnQuote: Íåâåğíûé ñèìâîë ïîñëå #';
	    3: Msg := 'UnQuote: Íåïàğíûé àïîñòğîô èëè êàâû÷êà';
  	  else
    		Msg := 'Unquote: Íåèçâåñòíàÿ îøèáêà '+IntToStr(Len);
	    end;
  	  raise EStrConvError.Create( Msg );
    end;
	 end
...
@@ErrEsc:
	XOR		EAX,EAX
  DEC		EAX  { Îøèáêà - íåâåğíûé ñèìâîë ïîñëå # }
	JMP		@@Exit
...
begin
  ...
  if Len<=0 then
    ...
    if Len<0 then
    begin
	  	if Len =-1 then Msg := 'DeEscBuf: Íåâåğíûé ñèìâîë ïîñëå #'
  	  else Msg := 'DeEscBuf: Íåèçâåñòíàÿ îøèáêà '+IntToStr(Len);
  	  raise EStrConvError.Create( Msg );
		end;
	 end
...
//-------------------------------------------------------------
// âûâîä â hex êàê åñòü + âåäóùèå 0x
function c2x( Dest, Src : Pointer; len : Integer ) : Integer; assembler;
asm
	push	esi
	push	edi
...
{---------------------------------------------------------------------------}
// âûâîä â hex â îáğàòíîì ïîğÿäêå
function i2x( Dest, Src : Pointer; len : Integer ) : Integer; assembler;
asm
	push	esi
	push	edi
...
//-------------------------------------------------------------
// Âûâîä äàìïà
// ğàçìåğ áóôåğà = (len*3)+((len-1) shr 2)-1
function DumpToBuf( Dest, Src : Pointer; len : Integer ) : Integer; assembler;
asm
	push	esi
	push	edi
...
begin
  if src='' then Result := ''
    ...
    begin
        ...
        begin
    	 	  case -Len of
          1: Msg := 'MacroExpand: Íåâåğíûé ïàğàìåòğ';
          2: Msg := 'MacroExpand: Ïåğåïîëíåíèå áóôåğà';
         	3: Msg := 'MacroExpand: Íåâåğíûé íîìåğ ïàğàìåòğà';
    		  else
		    	  Msg := 'MacroExpand: Íåèçâåñòíàÿ îøèáêà '+IntToStr(Len);
      		end;
		      raise EStrConvError.Create( Msg );
    	  end
      else Result := Copy( buf, 1, len );
...
//-------------------------------------------------------------
// Ïğîïóñêàåì ïğîáåëû è ñèìâîëû òàáóëÿöèè
...
//-------------------------------------------------------------
// Óäàëÿåì êîíöåâûå ïğîáåëû è ñèìâîëû òàáóëÿöèè
...
//-------------------------------------------------------------
// Ïğîïóñêàåì óïğàâëÿşùèå ñèìâîëû è ïğîáåëû ( âñå ñèìâîëû ìåíüøå $21 )
...
// Âíèìàíèå!!! Èçìåíÿåò çíà÷åíèå óêàçàííîé ñòğîêè!!!
...
// Åñëè ïåğâûé çíà÷èìûé ñèìâîë - àïîñòğîô èëè êàâû÷êà, òî áóäåò âîçâğàùåíà ğàñêâî÷åííàÿ ñòğîêà,
// èíà÷å èçâëåêàåò ñòğîêó äî òåğìèíàòîğà, à ïğîáåëû/òàáóëÿöèè ñ îáîèõ ñòîğîí áóäóò óäàëåíû.
// Ïàğà ptr/len áóäåò óêàçûâàòü íà ñèìâîë-òåğìèíèòîğ
function GetStringTerm( var ptr : PChar; var len : Integer; ATerminators : String ) : String;
var	i : Integer;
		pValue : PChar;
begin
...
//-------------------------------------------------------------
// Åñëè ïåğâûé çíà÷èìûé ñèìâîë - àïîñòğîô èëè êàâû÷êà, òî áóäåò âîçâğàùåíà ğàñêâî÷åííàÿ ñòğîêà,
// èíà÷å èçâëåêàåò ñòğîêó äî òåğìèíàòîğà ( ñèìâîë èç DefaultStringTerminators ).
// Ïàğà ptr/len áóäåò óêàçûâàòü íà ñèìâîë-òåğìèíèòîğ
function GetString( var ptr : PChar; var len : Integer ) : String;
begin
...
//-------------------------------------------------------------
// Èùåì òåğìèíàòîğ, Result = ñòğîêà äî òåğìèíàòîğà èëè èñõîäíàÿ ñòğîêà
// Ñòğîêè íå ğàñêâî÷èâàşòñÿ.
function GetSubStr( var ptr : PChar; var len : Integer; const ATerminators : String ) : String;
type	PWord = ^Word;
var	i : Integer;
begin
...
//-------------------------------------------------------------
// Èçâëåêàåò ñòğî÷êó - äî î÷åğåäíîãî CRLF.  ptr,len áóäóò óêàçàçûâàòü ÇÀ CRLF
function GetLine( var ptr : PChar; var len : Integer ) : String;
type	PWord = ^Word;
var	i : Integer;
begin
...
//=============================================================
// èùåì ñèìâîë (ñ ïîíèìàíèåì êàâû÷åê)
function QMemScan( ptr : PChar; len : Integer; c : Char ) : PChar;
asm
	test	eax,eax
  jz		@@Ret
...
const
		female : Array[1..2] of String = ('îäíà','äâå');
		upto20 : Array[1..20] of String =
			('îäèí','äâà','òğè','÷åòûğå','ïÿòü','øåñòü','ñåìü','âîñåìü','äåâÿòü','äåñÿòü',
			'îäèííàäöàòü','äâåíàäöàòü','òğèíàäöàòü','÷åòûğíàäöàòü','ïÿòíàäöàòü','øåñòíàäöàòü','ñåìíàäöàòü','âîñåìíàäöàòü','äåâÿòíàäöàòü','äâàäöàòü');
		decades: Array [2..9] of String = ('äâàäöàòü','òğèäöàòü','ñîğîê','ïÿòüäåñÿò','øåñòüäåñÿò','ñåìüäåñÿò','âîñåìüäåñÿò','äåâÿíîñòî');
		hundreds: Array [1..9] of String = ('ñòî','äâåñòè','òğèñòà','÷åòûğåñòà','ïÿòüñîò','øåñòüñîò','ñåìüñîò','âîñåìüñîò','äåâÿòñîò');
var n, m, i : LongInt;
		s : String;
...
begin
	n := Trunc(ANum);
	if n=0 then Result := 'Íîëü ğóáëåé'
	else
		begin
			Result := '';
			SpellNum(Result,['ğóáëü','ğóáëÿ','ğóáëåé'],ugMale);
			if n<>0 then
			begin
				SpellNum(Result,['òûñÿ÷à','òûñÿ÷è','òûñÿ÷'],ugFemale);
				if n<>0 then
				begin
					SpellNum(Result,['ìèëëèîí','ìèëëèîíà','ìèëëèîíîâ'],ugMale);
					if n<>0 then SpellNum(Result,['ìèëëèàğä','ìèëëèàğäà','ìèëëèàğäîâ'],ugMale);
				end;
			end;
		end;
...
//-------------------------------------------------------------
function SpellDate( const ADate : TDateTime ) : String;
const aMonth : array [1..12] of String = ('ÿíâàğÿ','ôåâğàëÿ','ìàğòà','àïğåëÿ','ìàÿ','èşíÿ','èşëÿ','àâãóñòà','ñåíòÿáğÿ','îêòÿáğÿ','íîÿáğÿ','äåêàáğÿ');
