...
var
   ...
   ZipFile,filename:TFileStream;
    ...
    begin
          ...
          Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'EXEC MASTER..xp_cmdshell '''+WRar+''' ',[],[]);
...
//    Yol := YedekDizin+ '\'+GENYEDEK;  //   WinrarExeDizin
   ...
   while i<5 do begin
    ...
    inc(i);
     ...
     try
      Veritabani.BasitKomutÇalıştır(Tablo.FDCnn, 'BACKUP DATABASE [' + trim(copy(ServerAdi,pos('/', ServerAdi)+1,50)) + '] TO DISK =''' + Yol+ GENYEDEK+'.BAK' + ''' WITH RETAINDAYS=1', [], []);
    Except
      Hatalar := '1';
      BekletmeDlg.Destroy;
      Exit;
