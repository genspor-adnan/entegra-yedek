...
//  LabelIstekNo.Caption:= IntToStr( ReherIni.ReadInteger('GenelOpsiyon','MailİstekNo',1));
//
...
begin
  if ComboSube.Text='' then
   ...
   if ComboGonderen.Text='' then
    ...
    try
     {
       gönderim işlemi için gerekli kod buraya
      }
     ...
     webservis.MesajKaydet( '' , Istekno, '', ComboGonderen.Text, ComboMesajTipi.Text, EditKonu.Text, MemoSorunlar.Lines.Text, ComboOncelik.Text, ComboSube.Text , ComboKime.Text );
      ...
      // gönderimden sonra istek numarasının arttırılması
  //   ReherIni.Writeinteger('GenelOpsiyon','MailİstekNo',strtoint(LabelIstekNo.Caption)+1);
