...
uses Windows, SysUtils, Classes, Graphics, Forms, Controls, StdCtrls, Buttons,
  ExtCtrls, UFDCompatHelpers, DBTables, Registry, Dialogs, Mask, DBCtrls, ToolWin, ComCtrls;
...
begin
  ...
  ListeKULHAR := TStringList.Create();
  yukle();
...
begin
  ...
  //  ReadString := Deger;
  Result := Deger;
  for i := 0 to ListeINI.Count - 1 do
  begin
    IniSatir := TIniSatir(ListeINI.Objects[i]);
    if (IniSatir.BOLUM = Bolum) and (IniSatir.Anahtar = Anahtar) then
    begin
      Result := IniSatir.DEGER
    end;
  end;
end;
...
begin {
  ...
  begin
    IniSatir := TIniSatir(ListeINI.Objects[i]);
    if (IniSatir.BOLUM = Bolum) then
    begin
      Liste.Add(IniSatir.ANAHTAR + '=' + IniSatir.DEGER);
    end;
  end;
end;
...
begin {
  ...
  begin
    IniSatir := TIniSatir(ListeINI.Objects[i]);
    if Liste.IndexOf(IniSatir.BOLUM) = -1 then
    begin
      Liste.Add(IniSatir.BOLUM);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    while not Eof do
    begin
      IniSatir := TIniSatir.Create();
      IniSatir.BOLUM := fieldbyname('BOLUM').AsString;
      IniSatir.SIRANO := fieldbyname('SIRANO').AsString;
      IniSatir.ANAHTAR := fieldbyname('ANAHTAR').AsString;
      IniSatir.DEGER := fieldbyname('DEGER').AsString;
      ListeINI.Add(
        IniSatir.BOLUM + '-' +
        IniSatir.SIRANO + '-' +
        IniSatir.ANAHTAR + '-' +
        IniSatir.DEGER);
      ListeINI.Objects[RecNo - 1] := IniSatir;
      Next;
    end;
