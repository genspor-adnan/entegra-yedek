...
type
  ...
  { ! Bu yardýmcýlarý kullanabilmek için uses kýsmýnda bu unit'i referans edin. !}


  TListExtension = class helper for TList
  public
    /// <summary>
    ///  Listenin içerisinde var mý yok mu kontrol eder
    ///  <param name="AItem">Kontrol edilecek pointer tipinde deðiþken</param>
    /// </summary>
  ...
  public
    /// <summary>
    ///   TabloDokum modülünde bulunan Menü iþlemlerinin TMenuItem e aktarýlmýþ hali
    ///   Fazladan AImageIndex parametresi eklenmiþtir.
    /// </summary>
    ...
    /// <summary>
    ///   Caption özelliðinin içeriðini & iþareti olmadan dönürür
    /// </summary>
  ...
  public
    /// <summary>
    ///   IndexOf('xyz') > -1 ifadesinin kýsaltýlmýþ hali
    /// </summary>
    function Contains(AVariant: Variant) : Boolean;
    /// <summary>
    ///   TStrings e variant deðer ekleyebilmeyi saðlar.böylelikle deðiþik
    ///   deðer tiplerini dönüþtürmeden saklanmasý saðlanýr.
    /// </summary>
    function AddVar(AVariant: Variant): Integer;
    /// <summary>
    ///   TStrings i bir integer listesi olarak kullanmayý saðlayan yöntem.
    /// </summary>
    function AsInteger(AIndex: Integer): Integer;
    /// <summary>
    ///   Bu iþlevi listede sadece integer deðerler içeriyorsa kullanýn.
    ///  Aksi takdirde Exception fýrlatabilir!
    /// </summary>
    function FindMaxInteger: Integer;
    /// <summary>
    ///   a,b,c gibi ayrýlmýþ string i listeye atýlabilmesini saðlayan yöntem.
    /// </summary>
    ...
    /// <summary>
    ///   Liste içerisindekilerini a,b,c gibi alýnabilmesini saðlar.
    /// </summary>
    function Join(ADelimiter: string): string;
    /// <summary>
    /// Nesne yoksa yeni bir tane ekler eðer varsa var olan nesneyi deðiþtirir
    /// </summary>
    function AddOrUpdateObject(AValue: string;AObject: TObject): Integer;
    /// <summary>
    /// Adý verilerek nesnenin elde edilmesini saðlar.
    /// </summary>
    property ObjectByName[Index: string] : TObject read GetObjectByName write SetObjectByName;
    /// <summary>
    /// Adý verilen elemanýn deðerini tam sayý olarak döndürür.  
    /// </summary>
    function ValueAsInteger(AName: string;ADefault: Integer): Integer;
    /// <summary>
    /// AName=AValue olarak eklemeyi saðlar.
    /// </summary>
    function AddNameValue(AName, AValue: Variant): Integer;
    /// <summary>
    /// Verilen deðer liste içinde yoksa ekler varsa eklemez.
    /// </summary>
    ...
    /// <summary>
    /// Verilen eski adý yeni adý ile deðiþtirir
    /// </summary>
    /// <param name="AOldName">Deðiþtirilmesi gereken ad</param>
    /// <param name="ANewName">Yeni adý</param>
    /// <returns>Deðiþen adýn indexi</returns>
    function Change(AOldName: string;ANewName: string): Integer;
    /// <summary>
    /// Listedeki elemana atanmýþ nesneyi (Object) TStrings olarak döndürür.
    /// Atanan nesnenin tipi TStrings türetilmiþ olmalý
    /// </summary>
    /// <remarks>
    ///  Bu özelliði kullanan yöntemler ve procedure'ler iþleri bittiklerinde
    ///  ClearObjects yöntemini kesinlikle çaðýrmalýdýrlar.!
    /// </remarks>
    property SubList[IndexOrName: Variant]: TStrings read GetSubList write SetSubList;
    /// <summary>
    /// Listedeki elemanlara atanmýþ tüm nesnelerin Free yöntemlerini çaðýrýr
    /// Eðer nesne TStrings türünde ise onunda ClearObjects yöntemini çaðýrýr
    /// </summary>
    procedure ClearObjects;
    /// <summary>
    /// Listedeki tüm elemanlarýn Object özelliði için yeni bir TStringList
    /// nesnesi baþlatýr ve atar
    /// </summary>
    /// <remarks>
    /// Eðer elemanýn nesnesi nil ise bu iþlemi yapar 
    /// </remarks>
  ...
  public
    /// <summary>
    ///   Liste içerisindekilerini sýralar
    /// </summary>
  ...
  public
    ...
    /// <summary>
    ///   Belirtilen alandaki tüm deðerleri verilen TStrings tipindeki deðiþkene
    ///  aktarýr.
    /// </summary>
    procedure FillStringList(AField: string;AList: TStrings;AClearList: Boolean = True);
    /// <summary>
    /// Fields[x] yerine DataSetten AsString olarak eriþmeyi saðlayan özellik
    /// </summary>
    /// <returns>Alaný deðerini string olarak döndürür</returns>
    property AsString[IndexOrName: Variant]: string read GetFieldAsString write SetFieldAsString;
    /// <summary>
    /// Fields[x] yerine DataSetten AsInteger olarak eriþmeyi saðlayan özellik
    /// </summary>
    /// <returns>Alanýn deðerini integer olarak döndürür</returns>
    property AsInteger[IndexOrName: Variant]: Integer read GetFieldAsInteger write SetFieldAsInteger;
    /// <summary>
    /// Fields[x] yerine DataSetten AsBoolean olarak eriþmeyi saðlayan özellik
    /// </summary>
    /// <returns>Alanýn deðerini boolean olarak döndürür</returns>
    property AsBoolean[IndexOrName: Variant]: Boolean read GetFieldAsBoolean write SetFieldAsBoolean;
    /// <summary>
    /// Fields[x] yerine DataSetten AsFloat olarak eriþmeyi saðlayan özellik
    /// </summary>
    /// <returns>Alanýn deðerini float olarak döndürür</returns>
    property AsFloat[IndexOrName: Variant]: Double read GetFieldAsFloat write SetFieldAsFloat;
    /// <summary>
    /// Alanýn bir sonraki satýrdaki deðerini döndürür.Eðer DataSet'in
    /// Eof bayraðý aktifse Null döner.
    /// </summary>
    /// <returns>Alanýn deðerini Variant olarak döndürür</returns>
  ...
  public
    /// <summary>
    ///   Alan içindeki deðerin null veya boþ olup olmadýðýný döndüren yöntem
    /// </summary>
    function IsNullOrEmpty : Boolean;
    /// <summary>
    ///   TDataSetExtension sýnýfýnda tanýmlanan yöntemin TField içinde yapýlmýþ
    ///  þekli.Nitekim kullanmasý TDataSet'te kullanýlmasýndan daha kolay.
    /// </summary>
  ...
  public
    /// <summary>
    ///   Veritabani.SorguBaslat (eski adý ile _query_exec) yönteminin TADOQuery
    ///  sýnýfýnýn içinden çalýþtýrýlmasýný saðlayan yöntem.
    /// </summary>
  ...
  public
    /// <summary>
    ///   Veritabani.KomutBaslat (eski adý ile _query_cmd) yönteminin TADOCommand
    ///  sýnýfýnýn içinden çalýþtýrýlmasýný saðlayan yöntem.
    /// </summary>
  ...
  public
    /// <summary>
    ///   Stream içeriðini 00ABCDEF00... gibi hex rakamlarla dize olarak
    ///   aktarýlmasýný saðlar.
    /// </summary>
    function ToHexString(AByteCountPerLine: Byte): string;
    /// <summary>
    ///   Verilen hex dizenin stream'a yazýlmasýný saðlar
    /// </summary>
  ...
  public
    /// <summary>
    ///   Çaðrýldýðýnda bu nesne ekrana çizilmez
    /// </summary>
...
begin
  ...
  try
    ...
    while not DataSet.Eof do begin
      if not IsNullOrEmpty then {EA : 18.06.2009 14:09 Null kayýtlar gereksiz yer kaplýyorlar }
        AList.Add(AsString);
      DataSet.Next;
    end;
  finally
...
begin
  ...
  Result := Veritabani.BasitKomutÇalýþtýr(TFDConnection(Self.Connection),ASql,params,paramValues,AReturnsResult);
end;
