...
begin
   ...
   for i := 0 to ArsivdenAlDlg.Basliklar.Count-1 do begin          //  Ücret,0  Basliklardaki saðdaki rakamlar aktarýmda kaç tablo ve hangilerinin kullanýlacaðýný gösteriyor 0 tablolardaki 0 ncý tablodan alýnacaðýný gösteriyor
       ArsivdenAlDlg.cxCheckListBox1.Items.Add;      //  Ücret,0   Aþaðýda Text kýsmýna Ücreti DisplayName kýsmýna da 0 atýyoruz
...
//       ArsivdenAlDlg.cxCheckListBox1.Items[i].DisplayName := copy(Basliklar.Strings[i],pos(',', Basliklar.Strings[i])+1, 20);
   ...
   if ArsivdenAlDlg.Basliklar.Count = 1 then //Tek bilgi var. Sorulmadan direk arþivden getir
