...
begin
  ...
  end else raise EInvalidOperation.Create('Tablo kayýtlý deðil! : ' + ATable.Name);
end;
...
begin
  ...
  if Assigned(table) then begin
    if (ATable.State in [dsEdit]) then begin
      logId := LogOperation(table.TableName,'Deðiþtirme');
      tempTable := GetTempLogTable('IVFLOGDETAY');
      try
    ...
    end else raise EInvalidOperation.Create('Tablo dsEdit veya dsInsert durumunda olmalýydý.');
  end else raise EInvalidOperation.Create('Tablo kayýtlý deðil! : ' + ATable.Name);
end;
...
begin
  if (ATableName = '') then
    raise EInvalidOperation.Create('ATableName boþ geldi.');
  table := FindTableByName(ATableName);
  if (not Assigned(table)) then
    raise EInvalidOperation.Create('Kayýttan çýkarma baþarýsýz. Tablo bulunamadý : ' + ATableName);
  i := FTableLogs.IndexOf(table);
  if i > -1 then begin
    FTableLogs.Extract(table);
    table.Free;
  end else raise Exception.Create('Kayýttan çýkarma baþarýsýz. Ýç hata : Tablo listede bulunamadý');
end;
...
begin
  ...
  table := FindTableByInstance(ATable);
  if (not Assigned(table)) then
    raise EInvalidOperation.Create('Kayýttan çýkarma baþarýsýz. Tablo bulunamadý : ' + ATable.Name);
  i := FTableLogs.IndexOf(table);
  if i > -1 then begin
    FTableLogs.Extract(table);
    table.Free;
