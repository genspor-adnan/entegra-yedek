...
type
   ...
   //   public
    //class function Edit(ABaðlantýDizesi: TBaðlantýDizesi): Boolean;
...
begin
   ...
   try
      ...
      if (EditRemoteServer.Text = '') or (cboDatabases.Text = '') then begin
         MessageDlg('Sunucu va veritabaný adý seçin!', mtWarning, [mbOK], 0);
         Exit;
      end;
      if ledUserName.Text='' Then begin
         MessageDlg('Kullanýcý Adýný Giriniz',mtWarning,[mbOk],0);
...
{class function TOpenSQLServerForm.Edit(
  ABaðlantýDizesi: TBaðlantýDizesi): Boolean;
begin
  with TOpenSQLServerForm.Create(Application) do
  try
    btnOk.Enabled := False;
    yetkilendirmeComboBox.ItemIndex := strtoint(IIf(ABaðlantýDizesi.TümleþikGüvenlik,inttostr(0),inttostr(1)));
    yetkilendirmeComboBoxChange(nil);
    cboServers.Text := ABaðlantýDizesi.SunucuAdý;
    cboDatabases.Text := ABaðlantýDizesi.Veritabaný;
    ledUserName.Text := ABaðlantýDizesi.KullanýcýId;
    ledPassword.Text := ABaðlantýDizesi.Þifre;
    saglayiciComboBox.ItemIndex := Dize.Hangisi(ABaðlantýDizesi.Saðlayýcý,['SQLOLEDB.1', 'SQLNCLI.1', 'SQLNCLI10.1']);
    EditTimeOut.Text := IntToStr(ABaðlantýDizesi.ZamanAþýmý);
    LocalAsyncCallEx(@SearchForServers,Integer(cboServers.Items));

    if ShowModal = mrCancel then
      Result := False
    else begin
      ABaðlantýDizesi.SunucuAdý := cboServers.Text;
      ABaðlantýDizesi.Veritabaný := cboDatabases.Text;
      ABaðlantýDizesi.TümleþikGüvenlik := yetkilendirmeComboBox.ItemIndex = 0;
      ABaðlantýDizesi.Saðlayýcý := Dize.Sec(saglayiciComboBox.ItemIndex,
        ['SQLOLEDB.1', 'SQLNCLI.1', 'SQLNCLI10.1']);
      ABaðlantýDizesi.KullanýcýId := ledUserName.Text;
      ABaðlantýDizesi.Þifre := ledPassword.Text;
      ABaðlantýDizesi.ZamanAþýmý := StrToIntDef(EditTimeOut.Text,15);
      Result := True;
    end;
  finally
    Free;
