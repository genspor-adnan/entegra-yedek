...
//  Provider=SQLOLEDB.1;Password=;Persist Security Info=False;Packet Size=8192;User ID=sa;Initial Catalog=gen2005;Data Source=.
   ...
   if yetkilendirmeComboBox.ItemIndex = 1 then begin
      Result := Result + 'Password=' + SC.Password+';Persist Security Info=False;' +
...
begin
   ...
   ParseConnectionString(GetGenotipConnectionString);
end;
...
begin
  winAuth := False;
  list := StringToStringList(AConnStr);
  try
    for i := 0 to list.Count - 1 do begin
      case CaseOf(list.Names[i],['Provider','Integrated Security','User ID',
        'Password','Initial Catalog','Data Source']) of
        0 : begin
          saglayiciComboBox.ItemIndex := CaseOf(list.ValueFromIndex[i],['SQLOLEDB.1','SQLNCLI.1','SQLNCLI10.1']);
        end;
        1: begin
          if (list.ValueFromIndex[i] = 'SSPI') then
            winAuth := True;
        end;
        2: begin
          ledUserName.Text := list.ValueFromIndex[i];
        end;
        3: begin
          pass := list.ValueFromIndex[i];
        end;
        4: begin
          cboDatabases.Text := list.ValueFromIndex[i];
        end;
        5: begin
          cboServers.Text := list.ValueFromIndex[i];
        end;
      end;
    end;
  finally
