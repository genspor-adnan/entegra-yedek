...
type
  TLisansDlg = class(TForm)
    ...
    procedure txtS4KeyPress(Sender: TObject; var Key: Char);
...
uses UTablo, uEncrypt, MD5, UMesaj;
...
begin
  ...
  sifre := txtS1.Text + txtS2.Text + txtS3.Text + txtS4.Text;
  if Sifre <> str4 then
  begin
...
procedure TLisansDlg.txtS4KeyPress(Sender: TObject; var Key: Char);
var
ad:String;
begin
if (key=#8) and (Length((sender as Tedit).Text)=0) and (strtoint(Copy((sender as Tedit).name, Length((sender as Tedit).Name ), 1)) > 1) then
begin
  ad:=copy((sender as TEdit).Name,1,length((sender as TEdit).Name)-1)+inttostr(strtoint(copy((sender as TEdit).Name,5,1))-1);
  (FindComponent(ad) as tedit).SetFocus;
  (FindComponent(ad) as TEdit).SelStart:= length( (FindComponent(ad) as TEdit).Text);
end;
end;

procedure TLisansDlg.Label1Click(Sender: TObject);
begin
//if MesajStrAl(
end;
