...
var
  ...
  FindFirstFileExA_MP: function(lpFileName: PAnsiChar; fInfoLevelId: DWORD; var lpFindFileData: TWIN32FindDataA; fSearchOp: DWORD; lpSearchFilter: Pointer; dwAdditionalFlags: DWORD): THandle; stdcall;
  ...
  CopyFileW_MP: function(lpExistingFileName, lpNewFileName: PWideChar; bFailIfExists: BOOL): BOOL; stdcall;
...
var
  Code: Integer;
begin
  Code := GetFileAttributes(PAnsiChar(Directory));
...
begin
  ...
  begin
    Result := FindFirstFileExA_MP(PAnsiChar( String( FileName + '\' + Mask)), FINDEX_INFO_STANDARD, FindFileDataA, FINDEX_SEARCH_NAMEMATCH, nil, CaseFlag);
...
begin
  ...
  begin
    if (Win32Platform = VER_PLATFORM_WIN32_NT) then
      ...
      S := FileName;
      ErrorCode := GetFileAttributesA(PAnsiChar(S))
    end;
...
begin
  ...
  if Win32Platform = VER_PLATFORM_WIN32_NT then
    ...
    FileNameA := FileName;
    Handle := FindFirstFileA(PAnsiChar(FileNameA), FindData)
  end;
...
begin
  ...
  if IsWinNT then
    ...
    Result := GetDriveTypeA(PAnsiChar(string(WS))) = DRIVE_REMOTE;
end;
...
begin
  ...
  begin
    ...
    InfoA.dwTypeData := PAnsiChar( string(ACaption));
...
begin
  if IsUnicode then
    ...
    S := WideString( Text);
    GetTextExtentPoint32(DC, PAnsiChar(S), Length(S), Result)
  end;
end;
...
begin
  ...
  if IsUnicode then
   ...
   CharUpperBuffA(PAnsiChar(string(Result)), Length(Result))
end;
...
begin
  ...
  begin
    ...
    aC := nil;
    if ExeFile <> '' then
      aA := PAnsiChar( string(ExeFile));
    if Parameters <> '' then
      aB := PAnsiChar( string(Parameters));
    if InitalDir <> '' then
      aC := PAnsiChar( string(InitalDir));
    CreateProcessA(
                  aA,
...
begin
  ...
  begin
    ...
    GetNumberFormatA(LOCALE_USER_DEFAULT, 0, PAnsiChar(string(NumberString)), nil, BufferA, SizeOf(BufferA));
    Result := BufferA
  end;
...
begin
  ...
  if IsUnicode then
    ...
    Result := lstrcmpia(PAnsiChar(string(Mask)), PAnsiChar(string(CompareStr)));
end;
...
begin
  ...
  if Assigned(PathMatchSpecA_MP) then
    Result := PathMatchSpecA_MP(PAnsiChar( string(Path)), PAnsiChar( string(Mask)))
  else
    Result := False
end;
...
begin
  ...
  if IsUnicode then
   ...
   CharLowerBuffA(PAnsiChar(string(Result)), Length(Result))
end;
