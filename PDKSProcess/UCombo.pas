...
begin
  ...
  SystemIni.OpenKeyReadOnly('SOFTWARE\GENTEGRE2\GENPER');
  s := '';
  s := SystemIni.ReadString(Anahtar);
  if s = '' then s := Def;
  RegReadString := s;
...
begin
  ...
  if lc = 'L' then
    ...
    SystemIni.RootKey := HKEY_CURRENT_USER;
  SystemIni.OpenKey('SOFTWARE\GENTEGRE2\GENPER', True);
  try
