/****** Object:  UserDefinedFunction [dbo].[fn_VardiyaAdi_Bas_Bit]    Script Date: 10/18/2010 11:36:00 ******/
SET ANSI_NULLS ON
GO
...
ALTER        function [dbo].[fn_VardiyaAdi_Bas_Bit] ( @PERKOD VARCHAR(20),@GIRIS DATETIME, @CIKIS DATETIME )
...
SELECT top 1 @VardiyaCikis = VARDIYAADI
  FROM dbo.PERS_VARDIYA WHERE 
  PERKOD=@PERKOD
  AND  CONVERT(DATETIME,VARDIYA_BASTARIH,120) <=   CONVERT(DATETIME,@GIRIS,120)
  AND  CONVERT(DATETIME,VARDIYA_BITTARIH,120) >=   CONVERT(DATETIME,@GIRIS,120)
  AND ISNULL(GUN,'')=''
