...
type
  ...
  private
    ...
    FOnImageScaleChange: TNotifyEvent;
    procedure DoScroll;
    procedure DoImageScaleChange;
  ...
  published
    ...
    property OnImageScaleChange: TNotifyEvent read FOnImageScaleChange write FOnImageScaleChange;
  end;
...
begin
  ...
  begin
    ANewWidth := Round(FImage.Width * FImageScale / 100);
    ANewHeight := Round(FImage.Height * FImageScale / 100);
  end;
...
begin
  ...
  if FImageDrag and (Button = mbLeft) then
  begin
    FImageDragging := True;
...
begin
  if FImageDragging and (Button = mbLeft) then
  begin
    FImageDragging := False;
...
procedure TATImageBox.DoImageScaleChange;
begin
  if Assigned(FOnImageScaleChange) then
    FOnImageScaleChange(Self);
end;
...
procedure Register;
begin
  RegisterComponents('GenSoft', [TATImageBox]);
end;

procedure TATImageBox.ImageDblClick(Sender: TObject);
begin
