...
for i := 1 to Length(s) do
begin
// Her bir karakterin sayýsal karþýlýðýna X ve dizi indisi ekleniyor ve  // X ile XOR yapýlýyor
r := r + IntToHex((Ord(s[i]) + (X + i)) xor X, 4)
end;
// Þifreli Metinin sonuna X 170 ile XOR yapýlarak Ekleniyor.
Result := r + IntToHex(X xor 170, 2);
...
// Þifrelenmiþ metinin sonundaki X deðeri Alýnýyor
X := StrToInt('$' + Copy(s, length(s) - 1, 2));
// Þifrelenmiþ metinin sonuna 170 ile XOR yapýlarak eklendiði için XOR ile // yeniden açýlýyor.
X := X xor 170;
// Þifrelenmiþ metnin sonundaki X bilgisi siliniyor.
Delete(s, length(s) - 1, 2);
// Her bir 4 karaktekter 1 karaktere karþýlýk geliyor. Bu yüzden döngüde // karakter sayýsý/4 kadar çevrim oluyor
for i := 1 to Length(s) div 4 do
begin
// 4'er karakter alýnarak tamsayýya çeviriliyor ardýndan X ile XOR  // yapýlarak X ve dizinin indisi çýkarýlýyor. Sonucunda kalan karakter
// deþifre metine ekleniyor.
r := r + chr((StrToInt('$' + copy(s, (4 * (i - 1) + 1), 4)) xor X) - (X + i));
end;
finally
