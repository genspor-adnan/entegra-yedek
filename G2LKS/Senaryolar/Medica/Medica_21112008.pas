var
  ...
  sayac                 : Integer;
  sayacarttir		: boolean;
  secilifistarih,fistarih  : string;
  ...
  with baslikxml.root do
    begin
	  with new do begin 
		...
		with new do begin name:='DATE'; text:=fistarih;  end;		
...
begin
  ReadSettings;  
  fistarih:= formatdatetime('dd.mm.yyyy',FaturaListesi.Fieldbyname('FATURATARIH').AsDateTime );
		...
		begin
			  with transactions.new do begin
			   name:='TRANSACTION';
				...
				with new do begin name:='CREDIT'; text:=faturaHizmet.fieldbyname('TUTAR').Asstring;  end;
	 ...
	 begin
		  with transactions.new do begin
		   name:='TRANSACTION';
			...
			with new do begin name:='CREDIT'; text:=faturaKDV.fieldbyname('TUTAR').Asstring;  end;
...
begin
  ...
  if not(fistarih=secilifistarih) then
   begin
     fistarih:=secilifistarih;
     XmlDosyaBaslat;
     sayac := 0;	
   end;
     ...
     SayacArttir:=True;
     InvoiceTransactions;
     if SayacArttir then  sayac := sayac + 1;
