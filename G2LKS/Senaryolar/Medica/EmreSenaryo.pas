...
begin
  ...
  if (faturaOdeme.FieldByName('MUHKODU').AsString = '') then begin // Kime deðerine göre ek kontrol
  ...
  with baslikxml.root do
    begin
	  with new do begin 
		...
		with new do begin name:='DATE'; text:='4';  end;		
		with new do begin name:='AUXIL_CODE'; text:='4';  end;		
		with new do begin name:='NOTES1'; text:='4';  end;		
...
begin
 baslikxml.savetofile('C:\EMRE123.XML');
 baslikxml.free;
...
{  if exportPath = '' then begin
  ...
  faturaHizmet := query_exec(SystemConnection,'p_G2LKS_FaturaHizmetBilgisi %dosyano%,%gelisno%,%kartno%',
