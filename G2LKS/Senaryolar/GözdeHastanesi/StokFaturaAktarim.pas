  ...
  with baslikxml.root do
    begin
	  with new do begin 
        ...
        with new do begin name:='DOC_NUMBER'; text:=FaturaListesi.FieldByName('FATURANO').AsString;  end;	
		...
		with new do begin name:='POST_FLAGS'; text:='245';  end;	
		with new do begin name:='VAT_RATE'; text:='18';  end;		
		...
		with new do begin name:='NOTES1'; text:=FaturaListesi.FieldByName('NOTLAR').AsString;  end;
		with new do begin name:='TC_NET'; text:=FaturaListesi.FieldByName('FATURA_TUTARI').AsString;  end;			
		with new do begin name:='RC_XRATE'; text:='4';  end;					

		with new do begin name:='CURRSEL_TOTALS'; text:='1';  end;			
		
		with new do begin name:='DOC_DATE'; text:=formatdatetime('dd.mm.yyyy',FaturaListesi.FieldByName('FATURATARIH').AsDatetime);  end;		
...
begin
  ...
  baslikxml.Root.name := 'PURCHASE_INVOICES';
  //XmlDosyaBaslat;
  DeleteFileS(exportPath,'Stok_'+FormatDateTime('YYYYMMDD',now)+'.xml'); 
end;

// Faturalarýn dönüþümü bittiði zaman çaðrýlýr.
procedure OnFinalize;
begin
 baslikxml.savetofile(exportPath+'\'+'Stok_'+ FormatDateTime('YYYYMMDDHHNN',now)+'.xml');
 baslikxml.free;
...
// fatura ile ilgili tüm satýrlarý kontrol ettirip herhangi bir hatasý varsa fiþe bu fatura aktarýlmasýn.
  result:=True;
  faturaHizmet := query_exec(SystemConnection,'p_G2LKS_StokFaturaHizmetBilgisi %gelisno%',['%gelisno%'], [GelisNo],nil);
  faturaHizmet.open;	
  faturaHizmet.First;
		...
		begin
			  with transactions.new do begin
			   name:='TRANSACTION';
				...
				with new do begin name:='QUANTITY'; text:=faturaHizmet.fieldbyname('ADET').Asstring;  end;				
				with new do begin name:='PRICE';    text:=faturaHizmet.fieldbyname('BIRIMFIYAT').Asstring;   end;	
				with new do begin name:='TOTAL';    text:=faturaHizmet.fieldbyname('TUTAR').Asstring;  end;
				with new do begin name:='UNIT_CODE'; text:=faturaHizmet.fieldbyname('BIRIM').Asstring;  end;	
				...
				with new do begin name:='VAT_BASE'; text:= floattostr(faturaHizmet.FieldByName('KDVTUTAR').AsFloat-faturaHizmet.FieldByName('KDV').AsFloat);  end;					
				with new do begin name:='BILLED'; text:='1';  end;								
				with new do begin name:='TOTAL_NET'; text:=floattostr(faturaHizmet.FieldByName('KDVTUTAR').AsFloat-faturaHizmet.FieldByName('KDV').AsFloat);  end;					
				with new do begin name:='EDT_CURR'; text:='1';  end;								
				with new do begin name:='EDT_PRICE'; text:= floattostr(faturaHizmet.FieldByName('KDVTUTAR').AsFloat-faturaHizmet.FieldByName('KDV').AsFloat);  end;									
			  end;
		end;  		 
	FaturaHizmet.Next;
	end;
