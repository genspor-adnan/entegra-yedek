...
uses
  ...
  Dialogs, DBGrids, Grids, ExtCtrls, CheckLst, Menus, Buttons, jpeg,ULKSTransformator;
...
type
  TVersiyonDlg = class(TForm)
    ...
    procedure Komutugster1Click(Sender: TObject);
...
begin
   ...
   senaryo := AnsiUpperCase(Copy(Ver,1,3)) = 'SEN';
   Ver := copy(ver,1,3)+copy(ver,5,2)+copy(ver,8,2);
   if not senaryo then begin
     m := TMemo(VersiyonDlg.FindComponent(ver));
     Memo1.Text := m.text;
     Memo1.Tag := 0;
   end else begin
     Memo1.Lines.LoadFromFile(ExtractFilePath(ParamStr(0)) + ver + '.ver');
     Memo1.Tag := 1;
   end;
end;

procedure TVersiyonDlg.BitBtn1Click(Sender: TObject);
var kom:TStringList;
    i:smallint;
    ...
    begin
      ...
      try
        ...
        ShowMessage('Bu deðiþiklik daha önce uygulanmýþ..');
      end;
   end;
var
   scriptEngine: TLksTransformator;
begin
  if Memo1.Tag = 0 then begin
     ...
     scriptEngine := TLksTransformator.Create(Memo1.Text);
     try
       scriptEngine.Run(nil);
       Komutugster1Click(self);
     finally
       scriptEngine.Free;
     end;
...
procedure TVersiyonDlg.Komutugster1Click(Sender: TObject);
begin
//   GENOTIPIni.WriteBool('Versiyonlar',Ver,True);
...
begin
 ...
 //   GENOTIPINI.ReadSectionValues('Versiyonlar',slist);
...
begin
   if (Key in ['A'..'Z'])or(Key in ['a'..'z'])then
      raise exception.Create('Komut deðiþtirilemez..')
end;
