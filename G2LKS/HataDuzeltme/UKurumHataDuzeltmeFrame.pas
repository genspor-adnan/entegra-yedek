...
type
  TKurumHataDuzeltmeFrame = class(TFrame)
    JvPanel1: TJvPanel;
    hataStaticText: TStaticText;
    Bevel1: TBevel;
    kodDBText: TDBText;
    KurumTableDataSource: TDataSource;
    KurumTable: TADOQuery;
...
procedure TKurumHataDuzeltmeFrame.ShowSolution(var Msg: TMessage);
begin
  hataStaticText.Caption := HataKontrolForm.Hata;
  KurumTable.Close;
  KurumTable.Parameters[0].Value := HataKontrolForm.Params;
  KurumTable.Open;
end;
...
procedure TKurumHataDuzeltmeFrame.araButtonClick(Sender: TObject);
var
  firmNo  : integer;
      ...
      begin
        try
          Result := ShowAraForm(Tablo.lksConnection,
            ...
            KurumTable.FieldByName('CARIKODU').AsString);
          if ((Length(Result) > 0) and (Result[0] <> '')) then
            begin
              KurumTable.Edit;
              KurumTable.FieldByName('CARIKODU').AsString := Result[0];
              KurumTable.FieldByName('MUHASEBEKODU').AsString := Result[2];
            end;
        except
...
initialization
  TSolutionFrameRegistration.RegisterSolutionFrame(TKurumHataDuzeltmeFrame,200);
end.
