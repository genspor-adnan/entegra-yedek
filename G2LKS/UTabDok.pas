...
uses
  ...
  Db, UCombo, Menus, stdctrls, buttons, comctrls, ImgList, ADODB,
...
type
  TTabloDokum = class(TDataModule)
    ...
    procedure TabKosulRecordsetCreate(DataSet: TCustomADODataSet;
...
uses UAnaForm, UTablo, UMesaj, qrexpr, URapSyf, UDokum,
  FetaUtil,FetaKurulusSiniflari,FetaClassExtensions,ADOInt; //  UFastRap,  UAyar,
  // UAnaListe,
...
procedure TTabloDokum.TabDokumRecordsetCreate(DataSet: TCustomADODataSet;
  const Recordset: _Recordset);
begin
...
procedure TTabloDokum.TabKosulRecordsetCreate(DataSet: TCustomADODataSet;
  const Recordset: _Recordset);
begin
  Recordset.Properties['Update Criteria'].Value := adCriteriaKey;
end;
...
begin
  ...
  if FormAdiGetir = 'DokumDlg' then begin
    ...
    TabDokum2.Open;
    if TabloDokum.TabDokum2.RecordCount > 0 then
...
begin
  ...
  if (FormAdiGetir = 'DokumDlg') and (s = 'Liste') then begin //TButton(PopupSagTusMenu.PopupComponent).Caption
    ...
    RapTablo.Ayarlar.Open;
    if RapTablo.Ayarlar.RecordCount < 1 then
...
begin
  TabKosul3.Last;
  if TabKosul3.RecordCount = 0 then
    SNo := 1
  else
...
begin
  ...
  Query2.SQL.Text := 'Select GELISNO, TARIH, TUR, TAHSIL, VADE From TAHSILAT Where DOSYANO = ''' + Dosyano + '''' +
    ' and GELISNO = ' + GelisNo;
  Query2.Open;
  if Query2.RecordCount = 0 then begin // Hiç tahsilat bilgisi yoksa
