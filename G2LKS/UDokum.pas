...
uses
  ...
  ADODb, StdCtrls, Outline, DBCtrls, Grids, DBGrids, Mask, Buttons,
  ...
  cxGridDBTableView, cxGrid, UQuantGrid, cxGridCustomPopupMenu,
  ...
  dxSkinTheAsphaltWorld, dxSkinVS2010, dxSkinWhiteprint, dxDateRanges,
...
type
  ...
  TDokumDlg = class(TFrame, IIcerikBilgiFrame, IBilgiFrame, IPopupDialog)  //; IAracCubuguDestegi
    ...
    procedure TabDokumRecordsetCreate(DataSet: TCustomADODataSet;
...
//    ComboList : TComponentList;
  ...
  public
    ...
    procedure ExceleYazdir(Table1: TADOQuery; Grid1: TDBGrid);
...
uses FetaUtil, {$IFNDEF AGENT} UAnaForm, {$ENDIF} UListe, UGrid, UMesaj, UFastRap,
     ...
     FetaClassExtensions, URaporAraclari, JvJVCLUtils, UGenelAnaSekmeFrame, UDokumGirisFrame, PrjConst,LocOnFly;
...
var
  ...
  YeniKayit, Dokum_Degis_Yetki:Boolean;
...
begin
  kom := '(';
  if (EQUAL = 'Ba?layan') or (EQUAL = '??inde ge?en') then
    kom := kom + TabAd + '.' + FIELD + ' LIKE '
  else if EQUAL = 'G?n/Ay' then
  ...
  with Veritabani.SorguBaslat(Tablo.cnn,'Select TOP 0 '+Field+' FROM ' + TabAd,[],[]) do
  try
  ...
  if EQUAL = '??inde ge?en' then kom := kom + '%';
...
{      Yils :='';
  ...
  kom := kom + VALUE;
  if (EQUAL = 'Ba?layan') or (EQUAL = '??inde ge?en') then kom := kom + '%';
  if (FieldTipi = FtString) or (FieldTipi = FtMemo) or (FieldTipi = FtDate) then kom := kom + '''';
  kom := kom + ')';
  komut.Add(kom);
  komutsatiri := TRUE;
...
begin
  if TabDokum.AsString['RAPORADI'] = '' then
    raise Exception.Create('Rapor Ad? Dolu Olmal?!');
...
procedure TDokumDlg.TabDokumRecordsetCreate(DataSet: TCustomADODataSet; const Recordset: _Recordset);
begin
...
begin
   if not BoslukKontrol(TabKosul.AsString['KOD_ADI'], 'Kod Ad?') then Abort;
   if not BoslukKontrol(TabKosul.AsString['ACIKLAMA'], 'A??klama') then Abort;
   if not BoslukKontrol(TabKosul.AsString['ICERIKTURU'], '??erik T?r?') then Abort;
end;
...
//      index1 := Table1.IndexDefs.Items[0].Fields;
    ...
    begin //?ndex Alan Say?s?n? belirle 'kod;s?rano'
      inc(IndexAlanSay);
...
begin
  ...
  FList.Assign(TabDokum.FieldByName('FIELDLIST'));
   {Field'lerden Table Adlar? belirlenir}
  TabKosul.First;
...
//          if simge <> '.' then Delete(TabAd,1,Rev_Pos(simge, TabAd));
  ...
  if TableAdlari.Count = 0 then {bo?sa}
    TableAdlari.Add('STOK');

  if TableAdlari.Count > 1 then {Tablolar aras?ndaki ba?? kur}
...
begin
  ...
  Tablo.CNN.ExecSQL('UPDATE DOKUMLER SET SAYAC= ISNULL(SAYAC,0) + 1 , SONTARIH= GETDATE(), SONKULLANAN='''+Kullanan+''' WHERE RAPORADI ='''+TabDokum.Fieldbyname('RAPORADI').AsString +'''');
end;
...
begin
   Veritabani.BasitKomut?al??t?r(Tablo.cnn,'delete from DOKUMLER  where MODUL>''-'' and  STANDART=0 AND DOKUMLER.RAPORADI in ('+
                 ' select D2.RAPORADI from DOKUMLER D2 where   D2.STANDART=1)',[],[]);
   Veritabani.BasitKomut?al??t?r(Tablo.cnn,'delete from [AYARLARYENI] where DOKUMID not in (select ID from DOKUMLER) ',[],[]);
   Veritabani.BasitKomut?al??t?r(Tablo.cnn,'delete from [KOSULLAR] where DOKUMID not in (select ID from DOKUMLER) ',[],[]);
end;
...
begin
   // Analist De?i?kenleri Klas?r?n? ekle
...
begin
   ...
   Veritabani.BasitKomut?al??t?r(Tablo.cnn,'UPDATE DOKUMLER SET STANDART=0,SAYAC=0,RAPORNO=0,EKLEYEN='+Kullanan+',EKLEMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''',DEGISTIREN='+Kullanan+',DEGISTIRMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''' WHERE ID = &ID', ['&ID'],[IntToStr(DID)]);
   ...
   if Tablo.Query3.RecordCount>0 then begin
      ...
      Veritabani.BasitKomut?al??t?r(Tablo.cnn,'UPDATE AYARLARYENI SET DOKUMID='+IntToStr(DID)+', EKLEYEN='+Kullanan+',EKLEMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''',DEGISTIREN='+Kullanan+',DEGISTIRMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''' WHERE ID = &ID', ['&ID'],[IntToStr(AID)]);
   end;
   ...
   while not Tablo.Query3.eof do begin
     ...
     Veritabani.BasitKomut?al??t?r(Tablo.cnn,'UPDATE KOSULLAR SET DOKUMID='+IntToStr(DID)+', EKLEYEN='+Kullanan+',EKLEMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''',DEGISTIREN='+Kullanan+',DEGISTIRMETARIHI='''+FormatDateTime('mm/dd/yyyy hh:nn',Tablo.GENINI.BugunTrhSaat)+''' WHERE ID = &ID', ['&ID'],[IntToStr(AID)]);
     Tablo.Query3.next;
   end;
...
//      ComboList.Add(TComponent(CurrentInstance));
  ...
  begin
    ...
    begin
      ...
      Height := 21;
//      delete(ad, 1, 3);
      if (ComboIcerik = 'bu g?n') or (ComboIcerik = 'BU G?N') or (ComboIcerik = 'bug?n') or (ComboIcerik = 'BUG?N')  then
  ...
  begin
    ...
    begin
      ...
      if ( UpperCase(ComboIcerik) = 'SAAT') or (ComboIcerik = '??MD?') or (ComboIcerik = '?imdi') or (ComboIcerik = '?imdi')  then
  ...
  try
    ...
    begin
      ...
      case ATabKosul.FieldByName('ICERIKTURU').AsInteger of
        ...
        2 : SpinCreate('Spin_' + ATabKosul.FieldByName('ID').AsString, ATabKosul.FieldByName('DEGER').AsString, 220, i * 5);  //tamsay?
...
begin
  FFrameBilgi := AValue;
  AValue.Etiketler.AsBoolean['D?k?mEkran?'] := True;
end;
...
begin
   case TMenuItem(Sender).Tag of
      ...
      //2 : Tablo.StokSihirbazBaslat('D', 0, qryListe.FieldByName('STOKID').AsInteger,-1);
   end;
end;
...
//    else
    ...
    try
      TComboBox(Sender).Items.FillFromSQL(Tablo.cnn,SQLSt,[],[]);
    except
    end;
    exit;
  end;
...
begin                       {
  if not qryListe.Active then
    raise Exception.Create('?nce sonu?lar? listeleyiniz');
...
begin
   ...
   if OpenDialog1.Execute then begin
      ...
      if TabDokum2.RecordCount >0 then
         if MessageDlg('D?k?mlerde ayn? adla rapor bulundu. ?zerine kaydedilsin mi?',
...
var
  ...
  j : Integer;
begin
  SaveDialog1.Title := 'Kaydedilecek Dosya Ad?';
  if SaveDialog1.Execute then
  begin
    AssignFile(F, SaveDialog1.FileName);
    if FileExists(SaveDialog1.FileName) then
      if MessageDlg(OpenDialog1.FileName + ' adl? dosya zaten var. ?zerine yaz?ls?n m??',
    ...
    try
      ...
      on EInOutError do
        MessageDlg('Dosya olu?turmada hata var!!!', mtError, [mbOk], 0);
    end;
    ...
    begin
      ...
      begin
        ...
        else if Fmt[Length(Fmt)] = '?' then
        begin
          ss := copy(qryListe.Fields[i].AsString, 1, 7);
          DisNo := StrToInt(copy(qryListe.Fields[i].AsString, 9, 1));
          if (ss = 'SA? ?ST') or (ss = 'Sa? ?st') then
            Ekle := 0
          else if (ss = 'SOL ?ST') or (ss = 'Sol ?st') then
            Ekle := 8
          else if (ss = 'SA? ALT') or (ss = 'Sa? Alt') then
            Ekle := 16
...
procedure TDokumDlg.ExceleYazdir(Table1: TADOQuery; Grid1: TDBGrid);
...
begin
  str := '';
  MesajStrAl('', 'Excel Dosya Ad?n? Dizinli Giriniz :', 'E', nil, str, '', 'E', nil, str);
  if str = '' then exit;
...
begin
  ...
  try
    ...
    sheet := book.worksheets[1];
     ...
     raise exception.Create('Excel A??lamad?..');
  end;
  try
...
begin
 ...
 { Veritabani.BasitKomut?al??t?r(Tablo.cnn,'Delete from KULHAR '+
...
begin
  ...
  with Veritabani.SorguBaslat(Tablo.cnn,'Select KULLANICI From KULLANICI',[],[]) do
  try
    Open;
    while not Eof do begin
      if KullanAdi <> AsString[0] then
      begin
       { Veritabani.BasitKomut?al??t?r(Tablo.cnn,
...
begin
  ...
  if SaveDialog1.Execute then
  begin
    with Veritabani.SorguBaslat(Tablo.cnn, StringReplace(TabKomut.SQL.Text, '??lem Say?', DokumAdi, [rfReplaceAll]),[],[]) do
    try
      Open;
      AssignFile(F, SaveDialog1.FileName);
      try
...
begin
  ...
  while not Tablo.Query5.eof do begin
    ...
    if Tablo.Query3.Fields[0].AsInteger > 1 then begin //daha ?nce fatura kesilmi? mi bakal?mmmm
      Tablo.Query4.Close;
      ...
      if Tablo.Query4.RecordCount > 0 then
        StrPCopy(ss, 'Hasta:' + Tablo.Query4.FieldByName('AD').AsString + ' ' +
          ...
          ' Fat.Tutar?:' + Format('%-10m', [Tablo.Query4.FieldByName('FATURA_TUTARI').AsCurrency]) +
    ...
    if Eklenebilir then begin
      ...
      Tablo.Query1.SQL.Text := 'Insert Into FATBASLIK (DOSYANO, GELISNO, KARTNO,BELGE,KIME,FATURATARIH,FATURANO,BASLIK,' +
        ...
        ''',''Hari?'',' + Lotno + ',100,' + Tablo.Query5.FieldByName('TUTAR').AsString + ',''' + Kullanan + ''','+IntToStr(SubeId)+')';
      Tablo.Query1.ExecSQL;
...
begin
  if Application.MessageBox(PChar(SSilmeSorusu), PChar(SGenotipOnay), MB_YESNO) = IDYES then begin
      if Veritabani.VeriVarMi(Tablo.cnn,'select ID from KOSULLAR where DOKUMID = $PID', ['PID'], [TabDokum.Fields[0].AsInteger]) then
         raise exception.Create('?nce Ko?ullar? Silmelisiniz...');
      Veritabani.BasitKomut?al??t?r(Tablo.cnn,'DELETE FROM AYARLARYENI WHERE DOKUMID = &ID', ['&ID'],[TabDokum.Fields[0].AsInteger]);
      Veritabani.BasitKomut?al??t?r(Tablo.cnn,'DELETE FROM DOKUMLER WHERE ID = &ID', ['&ID'],[TabDokum.Fields[0].AsInteger]);
...
//  DokumSartDlg.TabDokum.FindKey([TabDokum.FieldByName('RAPORADI').AsString]);
  if TabDokum.FieldByName('SQL').AsString <> '' then begin
    ...
    komut.Text := StringReplace(komut.Text,'%Kullan?c?Kodu%',Kullanan,[rfReplaceAll]);
    komut.Text := StringReplace(komut.Text,'%Kullan?c?Ad?%',KullanAdi,[rfReplaceAll]);
